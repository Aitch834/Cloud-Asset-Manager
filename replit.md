...
- **NVZPage.tsx** (`/nvz`): NVZ Compliance module. Two tabs — NVZ Summary (per-field cards with rolling 12-month total N and organic N vs limits, traffic-light progress bars, NVZ field settings dialog) and Application Log (full CRUD table of fertiliser/manure applications). Enforces UK NVZ rules: 170 kg organic N/ha/year limit, 250 kg total N/ha/year limit, closed-period warnings for slurry/digestate (arable: 1 Aug–31 Jan; grassland: 15 Oct–31 Jan). Schema: added `isNvz` + `nvzLandType` to `fieldsTable`; new `nvzFertiliserApplicationsTable` with fieldId, applicationDate, productName, productType, nitrogenKgHa, areaAppliedHa, totalNitrogenKg, applicationMethod, notes. API routes: GET/POST/DELETE `/nvz-applications`, GET `/nvz/field-summary` (rolling 12-month N totals per field).
...
- **SoilTestsPage.tsx** (`/soil`): Dedicated soil testing module. Two tabs — Test Records (expandable cards per test with per-nutrient AHDB results table; inline add/delete nutrients; "Add Nutrient Result" inline form with nutrient selector, value, unit, AHDB index, status) and Print/Export (per-test printable soil register grouped by field with year filter). API: individual nutrient routes added — `POST /farms/:farmId/soil-tests/:recordId/results`, `DELETE /farms/:farmId/soil-tests/:recordId/results/:resultId`. Uses `useQueries` (not map over `useQuery`) for dynamic per-test detail fetching.
- **BiosecurityPage.tsx** (`/visitors`, `/pest-control`, `/cleaning`): Three-tab biosecurity management page — Visitor Log (full CRUD with biosecurity/health declaration checkboxes, arrival/departure datetime, escorted by, vehicle reg; boolean badges in table), Pest Control (treatment/follow-up dates with overdue/upcoming amber/red badge logic, product and method tracking), Cleaning & Disinfection (dilution rate, contact time, next-due date alerts, verified-by field). Receives `defaultTab` prop so sidebar routes pointing to `/visitors`, `/pest-control`, `/cleaning` open on the correct tab. Replaces three separate generic ModulePage wrappers.
- **MedicinePage.tsx** (`/medicine`): Dedicated medicine records module. Two tabs — Records (table with withdrawal period tracking; amber row highlight + active-withdrawal alert banner for any record still within withdrawal period; auto-calculated withdrawal end date from administered date + days; herd selector, dosage, admin route, batch number, vet name, reason) and Print/Export (full medicine register printout — Red Tractor Livestock Standards require 5-year retention; withdrawal cells highlighted amber for records still active). Replaces old generic ModulePage wrapper.

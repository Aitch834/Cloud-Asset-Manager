...
- **Dairy Bulk Tank model** (lib/db/src/schema/livestock.ts): Three-table model separating tank registry, monitoring, and collection events. `dairy_bulk_tanks` (tank registry — name, location, capacity, FK from other dairy tables). `dairy_bulk_tank_records` (tank monitoring events — temperature checks, cleaning with product/batch, ABT results, maintenance; gains `tankId` FK). `dairy_milk_collections` (tanker collection events — date, tank, volume, milk buyer, tanker registration, driver name, collection reference, pre-collection ABR result). Previously a single `dairy_bulk_tank_records` table conflated tank monitoring with collection events and had no concept of multiple tanks. API routes: GET/POST/PUT/DELETE at `/farms/:farmId/dairy/tanks`, `/dairy/bulk-tank-records`, `/dairy/milk-collections`. Dashboard UI: three-section BulkTankTab (collapsible tank registry, monitoring records, milk collections each with their own add/edit dialogs).

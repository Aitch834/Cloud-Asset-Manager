...
// ─── Soil / Leaf Analysis Sample Label ───────────────────────────────────────

export interface SoilSampleLabelData {
...
function buildSoilLabelHtml(data: SoilSampleLabelData, qrDataUrl: string): string {
  const details: [string, string][] = [];
  ...
  const instructions = data.instructions
    ? `<div class="instructions-bar">Note: ${escapeHtml(data.instructions)}</div>`
    : "";
...
export async function printSoilSampleLabel(data: SoilSampleLabelData, count = 4): Promise<void> {
  ...
  for (let i = 0; i < totalCells; i++) {
    cells.push(i < count
      ? `<div class="label">${labelInner}</div>`

...
import { CROP_PROFILES } from "../irrigationData";
import { computeScenarios, computeSMD, getKc, getSmdStatus } from "../irrigationSMD";
...
describe("dashboard irrigation calculations match shared fixtures", () => {
  ...
  it("matches crop-coefficient interpolation", () => {
    ...
    const crop = CROP_PROFILES[input.crop];
    for (const [date, expected] of input.expected) {
      expect(getKc(crop, input.plantingDate, input.harvestDate, date as string)).toBeCloseTo(expected as number, 10);
    }
  });

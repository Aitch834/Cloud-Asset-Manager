...
export function CropsTab({ farmId }: { farmId: number }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? <Loader2 className="animate-spin w-5 h-5" /> : <DataTable cols={[{ key: "cropName", label: "Crop" }, { key: "variety", label: "Variety" }, { key: "sowingDate", label: "Sowing", fmt: r => fmtDate(r.sowingDate) }, { key: "expectedHarvestDate", label: "Expected Harvest", fmt: r => fmtDate(r.expectedHarvestDate) }, { key: "growingMethod", label: "Method" }, { key: "status", label: "Status" }]} rows={crops as Record<string, unknown>[]} onView={setViewRecord} onEdit={openEdit} onDelete={r => del.mutate(r.id as number)} />}
      {viewRecord && (
        <Dialog open onOpenChange={() => setViewRecord(null)}>
          <DialogContent style={{ maxWidth: "42rem" }}>

...
function BulkTankTab({ farmId }: { farmId: number }) {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["goat-dairy-tank", farmId] }); setOpen(false); toast({ title: editing ? "Updated" : "Added" }); },
  });
  const del = useMutation({
    mutationFn: (id: number) => fetch(api(`farms/${farmId}/goat-dairy/bulk-tank-records/${id}`), { method: "DELETE" }).then(r => r.json()),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["goat-dairy-tank", farmId] }); toast({ title: "Deleted" }); },
  });
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent style={{ maxWidth: "36rem" }}>
          ...
          <div className="grid grid-cols-2 gap-3 py-2">
            ...
            {form.tankCleaned && <><div><Label>Cleaning Product</Label><Input value={form.cleaningProductUsed || ""} onChange={e => set("cleaningProductUsed", e.target.value)} /></div><div><Label>Product Batch</Label><Input value={form.cleaningProductBatch || ""} onChange={e => set("cleaningProductBatch", e.target.value)} /></div></>}
...
function CaeTab({ farmId }: { farmId: number }) {
  const qc = useQueryClient();
  const { toast } = useToast();
  const [open, setOpen] = useState(false);

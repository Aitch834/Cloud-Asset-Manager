...
export function BulkTankTab({ farmId }: { farmId: number }) {
  ...
  const saveCollM = useMutation({ mutationFn: (d: Record<string, unknown>) => editingColl ? fetch(api(`farms/${farmId}/goat-dairy/milk-collections/${editingColl.id}`), { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) }).then(r => r.json()) : fetch(api(`farms/${farmId}/goat-dairy/milk-collections`), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) }).then(r => r.json()), onSuccess: () => { qc.invalidateQueries({ queryKey: ["goat-dairy-milk-collections", farmId] }); setCollDialog(false); toast({ title: editingColl ? "Collection updated" : "Collection recorded" }); } });
  const delCollM = useMutation({ mutationFn: (id: number) => fetch(api(`farms/${farmId}/goat-dairy/milk-collections/${id}`), { method: "DELETE" }).then(r => r.json()), onSuccess: () => { qc.invalidateQueries({ queryKey: ["goat-dairy-milk-collections", farmId] }); toast({ title: "Collection deleted" }); } });
                ...
                <tbody>
                  {colls.map(c => (
                    <tr key={c.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="py-2">
                        <div className="flex items-center gap-1">
                          <RecordAttachments recordType="goat-dairy-milk-collection" recordId={c.id} farmId={farmId} compact />

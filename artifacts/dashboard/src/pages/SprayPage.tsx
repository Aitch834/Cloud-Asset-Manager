  ...
  return (
    <AppLayout title="Spray Records">
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        {tab === "print" && <PrintTab applications={applications} />}
      </div>
    </AppLayout>
  );
}
...
function PrintTab({ applications }: any) {
  ...
  const handlePrint = () => {
    ...
    win.document.write(`<html><head><title>Spray Records</title><style>body{font-family:Arial,sans-serif;font-size:11px;margin:2cm}h1{font-size:15px;border-bottom:2px solid #333;padding-bottom:6px}table{width:100%;border-collapse:collapse;margin-top:12px}th{background:#f3f4f6;padding:5px 6px;text-align:left;font-size:10px;border:1px solid #d1d5db}td{padding:4px 6px;border:1px solid #e5e7eb;vertical-align:top}tr:nth-child(even){background:#f9fafb}.footer{margin-top:24px;font-size:9px;color:#9ca3af}</style></head><body><h1>Spray Application Records — Red Tractor Compliance</h1><p style="font-size:10px;color:#6b7280">Printed: ${today} | BDE Farm Trac</p><table><thead><tr><th>Date</th><th>Field</th><th>Product</th><th>Rate</th><th>Area</th><th>Water Vol.</th><th>Wind</th><th>Temp</th><th>Operator</th><th>Certificate</th><th>Reason</th></tr></thead><tbody>${rows}</tbody></table><div class="footer">Generated by BDE Farm Trac — Red Tractor Compliance Platform | ${today}</div></body></html>`);

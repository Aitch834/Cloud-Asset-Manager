...
<script>
(function () {
  /* @page fixes size: A4 portrait with 16mm top+bottom margins.
     Content height = (297 - 16 - 16) mm × (96 px/in ÷ 25.4 mm/in) ≈ 1001 px.
     matchMedia('print') change fires AFTER print styles are applied, so
     el.offsetTop is measured in the print layout, not the screen layout.
     beforeprint is kept as a fallback for browsers where matchMedia change
     may not fire, but matchMedia is preferred for layout accuracy. */
  var PAGE_H = Math.round(265 * 96 / 25.4);
  function annotate() {
    document.querySelectorAll('a.toc-link').forEach(function (a) {
      var id = (a.getAttribute('href') || '').replace(/^#/, '');
      var el = id ? document.getElementById(id) : null;
      if (el) { a.setAttribute('data-page-num', String(Math.floor(el.offsetTop / PAGE_H) + 1)); }
    });
  }
  if (window.matchMedia) {
    window.matchMedia('print').addEventListener('change', function (mq) {
      if (mq.matches) annotate();
    });
  }
  window.addEventListener('beforeprint', annotate);
})();
</script>
</body>
</html>`;

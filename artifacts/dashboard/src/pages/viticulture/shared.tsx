...
async function svgToPngDataUrl(svg: string): Promise<string | null> {
  ...
  try {
    ...
    // Render at 2x so block labels remain legible in the PDF.
    canvas.width = 640;
    canvas.height = 380;
...
export async function downloadVineRegisterPdf(
  ...
  // The browser-print export includes the same optional map in its header.
  // Keep this best-effort so a boundary request or SVG conversion failure
  // never prevents growers from downloading the register itself.
  let blockMapDataUrl: string | null = null;
  if (farmId && blocks && blocks.length > 0) {
    try {
      const res = await fetch(`/api/farms/${farmId}/vineyard-blocks/boundaries`, { credentials: "include" });
      if (res.ok) {
        const { boundaries } = await res.json() as {
          boundaries: Array<{ blockId: number; polygonPoints: LatLng[] }>;
        };
        if (Array.isArray(boundaries) && boundaries.length > 0) {
      ...
      // The map is optional; continue with the register when it is unavailable.
    }
  }
  ...
  // ── Optional block map in the report header ───────────────────────────────
  if (blockMapDataUrl) {
...
export async function downloadVineHarvestPdf(
  records: Record<string, unknown>[],
  blocks: Record<string, unknown>[],
  farmName: string,
  farmMeta: Record<string, unknown> | null | undefined,
  ...
  // ── Header ─────────────────────────────────────────────────────────────────
  doc.setFillColor(124, 61, 18); // #7c3d12
  doc.rect(0, 0, pageW, 22, "F");
  doc.setFontSize(15); doc.setFont("helvetica", "bold"); doc.setTextColor(255, 255, 255);
  doc.text("BDE Farm Trac", 14, 10);
  doc.setFontSize(8); doc.setFont("helvetica", "normal");
  ...
  doc.roundedRect(14, y, pageW - 28, 8, 1, 1, "FD");
  doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(124, 61, 18);
  ...
  const average = (field: string, dp: number) => {
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      11: { cellWidth: 20 },
      12: { cellWidth: 24 },
      13: { cellWidth: "auto" as unknown as number },
    },
    margin: { left: 14, right: 14 },
  });

  if (records.length > 0) {
    const finalY = ((doc as unknown as { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY ?? y) + 3;
    autoTable(doc, {
      body: [[
        {
      ...
      columnStyles: {
        4: { cellWidth: 18, halign: "right" }, 5: { cellWidth: 15 }, 6: { cellWidth: 14, halign: "right" },
        7: { cellWidth: 13, halign: "right" }, 8: { cellWidth: 15, halign: "right" }, 9: { cellWidth: 18, halign: "right" },
        10: { cellWidth: 24 }, 11: { cellWidth: 20 }, 12: { cellWidth: 24 }, 13: { cellWidth: "auto" as unknown as number },
      },
      margin: { left: 14, right: 14 },
      theme: "plain",
    });
...
export function DataTable({ cols, rows, onEdit, onDelete, onView, deleteMutation, sortKey, sortDir, onSort, rowClassName, getRowActionLabel }: {
  ...
  getRowActionLabel?: (r: Record<string, unknown>) => string;
}) {
  const [pending, setPending] = useState<Record<string, unknown> | null>(null);
  useEffect(() => { if (deleteMutation?.isSuccess) setPending(null); }, [deleteMutation?.isSuccess]);
  const actionLabel = (action: "View" | "Edit" | "Delete", row: Record<string, unknown>) => {
            ...
            <tr key={i} className={`border-b last:border-0 ${rowClassName ? rowClassName(row) : ""}`}>
              ...
              {(onEdit || onDelete || onView) && (
                <td className="py-2 text-right space-x-1 whitespace-nowrap">
                  {onView && <Button size="icon" variant="ghost" aria-label={actionLabel("View", row)} title="View" onClick={() => onView(row)}><Eye className="w-3.5 h-3.5" /></Button>}
                  {onEdit && <Button size="icon" variant="ghost" aria-label={actionLabel("Edit", row)} title="Edit" onClick={() => onEdit(row)}><Pencil className="w-3.5 h-3.5" /></Button>}
                  {onDelete && <Button size="icon" variant="ghost" aria-label={actionLabel("Delete", row)} title="Delete" onClick={() => setPending(row)}><Trash2 className="w-3.5 h-3.5 text-red-500" /></Button>}
                </td>
              )}
            </tr>
          ))}</tbody>

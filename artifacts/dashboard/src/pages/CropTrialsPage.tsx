...
function TrialDetailView({ trial, farmId, onBack, fields }: { trial: Trial; farmId: number; onBack: () => void; fields: any[] }) {
  ...
  const addCommMut = useMutation({
    mutationFn: (body: any) => fetch(`/api/farms/${farmId}/crop-trials/${trial.id}/communications`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => ""); throw new Error(t || `Request failed (${r.status})`); } return r; }).then(r => r.json()),
  ...
  const deleteCommMut = useMutation({
    mutationFn: (id: number) => fetch(`/api/farms/${farmId}/crop-trials/${trial.id}/communications/${id}`, { method: "DELETE" }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => ""); throw new Error(t || `Request failed (${r.status})`); } return r; }),
  ...
  const addPlotMut = useMutation({
    mutationFn: (body: any) => fetch(`/api/farms/${farmId}/crop-trials/${trial.id}/plots`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => ""); throw new Error(t || `Request failed (${r.status})`); } return r; }).then(r => r.json()),
  ...
  const deletePlotMut = useMutation({
    mutationFn: (plotId: number) => fetch(`/api/farms/${farmId}/crop-trials/${trial.id}/plots/${plotId}`, { method: "DELETE" }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => ""); throw new Error(t || `Request failed (${r.status})`); } return r; }),
    onSuccess: () => { toast({ title: "Plot deleted" }); invalidate(); setDeletePlotId(null); setSelectedPlot(null); },
    onError: () => toast({ title: "Delete failed", variant: "destructive" }),
  });
  const addTreatmentMut = useMutation({
    mutationFn: (body: any) => fetch(`/api/farms/${farmId}/crop-trials/${trial.id}/plots/${selectedPlot!.id}/treatments`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => ""); throw new Error(t || `Request failed (${r.status})`); } return r; }).then(r => r.json()),
    onSuccess: () => { toast({ title: "Treatment recorded" }); invalidate(); setAddTreatmentOpen(false); },
    onError: () => toast({ title: "Save failed", variant: "destructive" }),
  });
  const addObsMut = useMutation({
    mutationFn: (body: any) => fetch(`/api/farms/${farmId}/crop-trials/${trial.id}/plots/${selectedPlot!.id}/observations`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => ""); throw new Error(t || `Request failed (${r.status})`); } return r; }).then(r => r.json()),
    onSuccess: () => { toast({ title: "Observation saved" }); invalidate(); setAddObsOpen(false); },
    onError: () => toast({ title: "Save failed", variant: "destructive" }),
  });
  const addYieldMut = useMutation({
    mutationFn: (body: any) => fetch(`/api/farms/${farmId}/crop-trials/${trial.id}/plots/${selectedPlot!.id}/yields`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => ""); throw new Error(t || `Request failed (${r.status})`); } return r; }).then(r => r.json()),
...
export default function CropTrialsPage() {
  ...
  const createMut = useMutation({
    mutationFn: (body: any) => fetch(`/api/farms/${farmId}/crop-trials`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => ""); throw new Error(t || `Request failed (${r.status})`); } return r; }).then(r => r.json()),
    onSuccess: () => { toast({ title: "Trial created" }); qc.invalidateQueries({ queryKey: ["crop-trials", farmId] }); setAddOpen(false); },
    onError: () => toast({ title: "Save failed", variant: "destructive" }),
  });
  const updateMut = useMutation({
    mutationFn: ({ id, body }: { id: number; body: any }) => fetch(`/api/farms/${farmId}/crop-trials/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => ""); throw new Error(t || `Request failed (${r.status})`); } return r; }).then(r => r.json()),
  ...
  const deleteMut = useMutation({
    mutationFn: (id: number) => fetch(`/api/farms/${farmId}/crop-trials/${id}`, { method: "DELETE" }).then(async r => { if (!r.ok) { const t = await r.text().catch(() => ""); throw new Error(t || `Request failed (${r.status})`); } return r; }),

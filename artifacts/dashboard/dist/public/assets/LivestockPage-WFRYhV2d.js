import { s as createLucideIcon, c as useQueryClient, a as useToast, m as useQuery, r as reactExports, O as useMutation, j as jsxRuntimeExports, I as Input, d as Button, S as Plus, e as LoaderCircle, n as Card, o as CardContent, A as ArrowRight, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, H as DialogDescription, L as Label, X as DialogMutationError, b as useAppStore, R as Redirect, T as FlaskConical, aJ as ErrorBoundary } from "./index-DxRio-2w.js";
...
import { F as FileText, D as Droplets, C as ClipboardCheck } from "./shield-alert-BEDUX2R0.js";
import { S as Search } from "./search-CBNSHmH4.js";
import { P as Paperclip } from "./paperclip-j82uenYy.js";
...
import { A as AppLayout, c as ClipboardList, j as Truck, q as Stethoscope, e as ChartColumn, B as BookOpen, R as RotateCcw } from "./AppLayout-EUHbgKOo.js";
import { T as TabBar, a as TabButton } from "./tab-button-Ca7Ppcdb.js";
import { Q as QrCode, a as QRCodeSVG } from "./index-Cga6o8HX.js";
import { u as useUpload } from "./use-upload-DKimebuB.js";
...
import { u as useFarmMembers, m as memberFullName } from "./use-farm-members-VIQpw2iF.js";
...
import { R as ResponsiveContainer, X as XAxis, Y as YAxis, T as Tooltip, B as Bar, C as Cell } from "./generateCategoricalChart-Bew74kXU.js";
...
function LambingSection({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/lambing-records/${id}`, { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
<p class="footer">Red Tractor Beef &amp; Dairy: BVD monitoring records must be maintained and available at audit. Persistent Infectees (PIs) must be removed promptly. Retain records for a minimum of 3 years. Printed: ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-GB")}</p>
  ...
  async function del(id) {
    ...
    await fetch(`/api/farms/${farmId}/bvd-tests/${id}`, { method: "DELETE" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
<p class="footer">Red Tractor requires a record of every on-farm emergency killing. The person carrying out the slaughter must hold a valid WASK/WATOK certificate, or where a barbiturate injection is used, the attending vet must be RCVS registered. Records must be retained for a minimum of 3 years. Printed: ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-GB")}</p>
  ...
  async function del(id) {
    ...
    await fetch(`/api/farms/${farmId}/casualty-slaughter/${id}`, { method: "DELETE" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function IsolationRegisterSection({ farmId }) {
  ...
  const hcs = hcQ.data ?? [];
  const createMut = useMutation({ mutationFn: (b) => fetch(`/api/farms/${farmId}/isolation-records`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
  ...
  const updateMut = useMutation({ mutationFn: ({ id, b }) => fetch(`/api/farms/${farmId}/isolation-records/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
  ...
  const deleteMut = useMutation({ mutationFn: (id) => fetch(`/api/farms/${farmId}/isolation-records/${id}`, { method: "DELETE" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
  ...
  const createHcMut = useMutation({ mutationFn: ({ recId, b }) => fetch(`/api/farms/${farmId}/isolation-records/${recId}/health-checks`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
  ...
  const deleteHcMut = useMutation({ mutationFn: ({ recId, checkId }) => fetch(`/api/farms/${farmId}/isolation-records/${recId}/health-checks/${checkId}`, { method: "DELETE" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }

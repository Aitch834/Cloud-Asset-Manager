import { m as useQuery, j as jsxRuntimeExports, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, H as DialogDescription, d as Button, c as useQueryClient, a as useToast, r as reactExports, S as useMutation, I as Input, T as Plus, n as Card, p as CardContent, e as LoaderCircle, J as DialogFooter, N as DialogMutationError, L as Label, U as FlaskConical, A as ArrowRight } from "./index-szmq1rP5.js";
...
import { F as FileText, C as ClipboardCheck, D as Droplets } from "./shield-alert-DPVDiVqR.js";
import { U as Upload } from "./upload-5kKBOKKs.js";
import { c as ClipboardList, k as Stethoscope, B as BookOpen, R as RotateCcw } from "./AppLayout-B_c4q11G.js";
...
import { e as escapeHtml, p as printProReport, o as openPrintWindow, c as buildProReport } from "./print-report-ClU8-1P0.js";
...
import { L as ListChecks } from "./list-checks-BDm_IHhc.js";
...
import { b as useFarmName, a as useRawFarmName } from "./use-farm-name-J933M3cO.js";
...
function PrintHerdRegisterDialog({ farmId, herds, onClose }) {
  ...
  const handleHerdPrint = () => {
    const rows = herds.length === 0 ? `<tr><td colspan="6" style="text-align:center;color:#9ca3af;font-style:italic;padding:12px">No herds recorded</td></tr>` : herds.map((h) => `<tr>
          ...
          <td>${escapeHtml(h.type ? herdSpeciesDisplayLabel(h.type) + (herdProductionSubtype(h.type, h.productionType) ? ` (${herdProductionSubtype(h.type, h.productionType)})` : "") : "—")}</td>
...
function TbTestsSection({ farmId }) {
  ...
  function printReport() {
    const rows = records.map((r) => `<tr><td>${escapeHtml(formatDate$1(r.testDate))}</td><td>${escapeHtml(r.testType.replace(/-/g, " "))}</td><td>${escapeHtml(r.species)}</td><td>${escapeHtml(r.herdFlockRef ?? "—")}</td><td>${escapeHtml(r.animalsTested ?? "—")}</td><td>${escapeHtml(r.reactors)}</td><td>${escapeHtml(r.inconclusives)}</td><td>${escapeHtml(r.outcome.toUpperCase())}</td><td>${r.movementRestriction ? "YES" : "No"}</td><td>${escapeHtml(formatDate$1(r.nextTestDueDate))}</td><td style="text-align:center;color:${r.movementId ? "#166534" : "#9ca3af"};font-weight:${r.movementId ? "700" : "400"}">${r.movementId ? "✓ Linked" : "—"}</td></tr>`).join("");
...
function WelfareOutcomeSection({ farmId }) {
  ...
  function printReport() {
    const rows = records.map((r) => `<tr><td>${escapeHtml(formatDate$1(r.assessmentDate))}</td><td>${escapeHtml(r.species)}</td><td>${r.assessorType === "internal" ? "Internal" : "External"}</td><td>${escapeHtml(r.assessorName)}</td><td>${escapeHtml(r.herdFlockRef ?? "—")}</td><td>${escapeHtml(r.sampleSize ?? "—")}</td><td>${escapeHtml(r.lamenessScore ?? "—")}</td><td>${escapeHtml(r.overallOutcome.toUpperCase())}</td><td>${escapeHtml(formatDate$1(r.nextAssessmentDue))}</td></tr>`).join("");

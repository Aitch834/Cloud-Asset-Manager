...
import { r as reactExports, b as useAppStore, _ as __vitePreload, j as jsxRuntimeExports, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, H as DialogDescription, J as DialogFooter, c as Button, l as useQuery, R as Redirect, m as Card, n as CardContent, t as useQueryClient, O as useMutation, S as Plus, d as LoaderCircle, L as Label, I as Input, C as Checkbox, a as useToast, K as Map } from "./index-BKcsQW2l.js";
import { L as LabSelector } from "./LabSelector-cqZJ7Qfa.js";
...
import { A as AppLayout, W as Warehouse, T as TrendingUp, R as RotateCcw } from "./AppLayout-B8_1RkJL.js";
...
function CropsTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/horticulture-crops/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["horti-crops", farmId] }) });
  const openEdit = (r) => {
    setEditing(r);
...
function WaterTestsTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/horticulture-water-tests/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["horti-water", farmId] }) });
  function openAdd() {
    setEditing(null);
    setMode("log");
    setForm({});
...
function HarvestTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/horticulture-harvest-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["horti-harvest", farmId] }) });
  const openEdit = (r) => {
    setEditing(r);
    setForm(Object.fromEntries(Object.entries(r).map(([k, v]) => [k, v == null ? "" : String(v)])));
    setOpen(true);
...
function AllergenTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/allergen-management/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["horti-allergen", farmId] }) });

import { q as createLucideIcon, r as reactExports, l as useQuery, j as jsxRuntimeExports, b as useAppStore, t as useQueryClient, a as useToast, O as useMutation, c as Button, S as Plus, Q as React, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, I as Input, J as DialogFooter } from "./index-CG7Bde15.js";
import { d as downloadCsvFile } from "./csv-Dr539t8b.js";
import { u as useSafeUser } from "./use-safe-clerk-DE9vZTtv.js";
import { U as Users, A as AppLayout, C as CalendarDays, T as TrendingUp, I as Info, o as Bell, Z as Zap } from "./AppLayout-DW85f8T1.js";
...
import { R as ResponsiveContainer, X as XAxis, Y as YAxis, T as Tooltip, B as Bar, C as Cell } from "./generateCategoricalChart-DiHRC4dV.js";
...
import { U as UserCheck } from "./user-check-D7kb60sl.js";
import { P as PoundSterling } from "./shield-alert-1PMzBpNJ.js";
import { S as ShieldCheck } from "./shield-check-BdnLiSiq.js";
import { C as ChartNoAxesColumn } from "./chart-no-axes-column-C2wdnrHT.js";
import { A as ArrowLeftRight } from "./arrow-left-right-Cjp4aeWi.js";
import { T as TriangleAlert } from "./triangle-alert-BnOSZuM5.js";
import { C as ChevronLeft } from "./chevron-left-Cq0sRXPC.js";
...
import { M as MessageSquare } from "./message-square-D8Oq5sPk.js";
...
function TimesheetsTab({ farmId, staffNames, staffMembers }) {
  ...
  const addMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/labour/timesheets`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const editMut = useMutation({
    mutationFn: ({ id, body }) => fetch(`/api/farms/${farmId}/labour/timesheets/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const delMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/labour/timesheets/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
...
function RotaTab({ farmId, staffNames, staffMembers }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
    ...
    pendingHolidayLog && /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: true, onOpenChange: (o) => {
      if (!o) setPendingHolidayLog(null);
...
function AbsenceTab({ farmId, staffNames, onPendingCount, staffMembers }) {
  ...
  const [yearFilter, setYearFilter] = reactExports.useState(String(thisYear));
  const [staffFilter, setStaffFilter] = reactExports.useState("all");
...
<div class="footer-note">BDE Farm Trac — Barnett Davies Enterprises Ltd · Confidential HR Document · Form FT-LR-01 · bdefarmtrac.co.uk</div>
  ...
  const addMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/labour/absences`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const editMut = useMutation({
    mutationFn: ({ id, body }) => fetch(`/api/farms/${farmId}/labour/absences/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const delMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/labour/absences/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      toast({ title: "Deleted" });
      qc.invalidateQueries({ queryKey: ["labour-absences", farmId] });
    },
  ...
  const addEntMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/labour/entitlements`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const editEntMut = useMutation({
    mutationFn: ({ id, body }) => fetch(`/api/farms/${farmId}/labour/entitlements/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
...
function PaySummaryTab({ farmId, staffNames, staffMembers }) {
  ...
  const addRateMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/labour/rates`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const editRateMut = useMutation({
    mutationFn: ({ id, body }) => fetch(`/api/farms/${farmId}/labour/rates/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const delRateMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/labour/rates/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
...
function WorkingTimeTab({ farmId, staffNames, staffMembers }) {
  ...
  const toggleOptOut = async (name, current) => {
    ...
    if (ent) {
      ...
      await fetch(`/api/farms/${farmId}/labour/entitlements`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ staffName: name, year: (/* @__PURE__ */ new Date()).getFullYear(), entitlementDays: "28", carriedOverDays: "0", wtrOptOut: !current }) });
    }
    qc.invalidateQueries({ queryKey: ["labour-entitlements", farmId] });
...
function StaffHoursCrossRefTab({ farmId, staffNames }) {
  ...
  const delAnnot = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/labour/crossref-annotations/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["labour-crossref-annotations", farmId] });

import { b as useAppStore, c as useQueryClient, r as reactExports, m as useQuery, j as jsxRuntimeExports, d as Button, S as useMutation } from "./index-D5ac2D7t.js";
import { u as usePersistedTab } from "./use-persisted-tab-D-ESQCO1.js";
import { u as usePersistedNumberFilter } from "./use-persisted-filter-D3h07CB9.js";
...
import { A as AppLayout, e as ChartColumn, T as TrendingUp } from "./AppLayout-CBaXLtVv.js";
...
import { P as PoundSterling } from "./shield-alert-DOmCjdKc.js";
...
const AGRI_ENV_GRANT_CSV_HEADERS = [
  "Claimed this year",
  "Total grant value",
  "All-time claimed",
  "Remaining grant value"
];
const PL_CSV_HEADERS = [
  "Line Item",
...
function buildAgriEnvProjectCsvRows(projects) {
  return projects.map((project) => {
    const remaining = project.totalGrantValuePence != null ? project.totalGrantValuePence - (project.allTimeClaimedPence ?? 0) : null;
    return [
      `  ↳ ${project.schemeName}`,
...
function buildPlAgriEnvProjectCsvRows(projects) {
...
function SubsidiesTab({ farmId, year, onRegisterExport }) {
  ...
  const schemes = reactExports.useMemo(() => data?.schemes ?? [], [data?.schemes]);
  const subsidyTx = reactExports.useMemo(
    () => (data?.subsidyTransactions ?? []).filter((t) => ["Agri-Environment Scheme", "Grant / Subsidy"].includes(t.category ?? "")),
    [data?.subsidyTransactions]
  );
  const agriEnvProjects = reactExports.useMemo(() => data?.agriEnvProjects ?? [], [data?.agriEnvProjects]);
  const agriEnvMilestones = reactExports.useMemo(() => data?.agriEnvMilestones ?? [], [data?.agriEnvMilestones]);
...
function BusinessReportsPage() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(AppLayout, { title: "Business Reports", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          Button,
          {
            ...
            onClick: () => printElementReport(document.getElementById("business-report-content"), {
              title: {
                "gross-margin": "Gross Margin Report",
                pl: "Profit & Loss Statement",
                "input-costs": "Input Costs Report",
                "grain-position": "Grain Position Report",
                subsidies: "Subsidy Income Report",
                "year-on-year": "Year-on-Year Report",
                assets: "Asset Register",
                benchmarking: "Farm Benchmarking Report"
              }[tab],
              subtitle: showYearSelector ? `Financial year ${year}` : void 0,
              ...
              redTractorId: farm?.redTractorId ? String(farm.redTractorId) : void 0,
              landscape: true
            }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "business-report-content", style: { marginTop: "1.25rem" }, children: [

import { s as createLucideIcon, r as reactExports, m as useQuery, j as jsxRuntimeExports, L as Label, e as LoaderCircle, I as Input, b as useAppStore, R as Redirect, n as Card, o as CardContent, c as useQueryClient, a as useToast, S as useMutation, d as Button, T as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, C as Checkbox, N as DialogMutationError } from "./index-CFhHVfpn.js";
import { R as RaiseTaskDialog } from "./RaiseTaskDialog-BsOBB6RH.js";
import { S as Sprout, e as ChartColumn, G as Gauge, T as TrendingUp, I as Info, A as AppLayout, c as ClipboardList } from "./AppLayout-CJMxwOtu.js";
...
import { T as Thermometer } from "./thermometer-lio_rChC.js";
import { C as CloudRain } from "./cloud-rain-CGkSx248.js";
import { g as generateCategoricalChart, X as XAxis, Y as YAxis, f as formatAxisMap, R as ResponsiveContainer, T as Tooltip, _ as ReferenceLine } from "./generateCategoricalChart-6hzxYk6d.js";
import { A as Area } from "./Area-BpHsezBH.js";
...
import { T as Tractor } from "./tractor-BAmFM0Ow.js";
import { T as TriangleAlert } from "./triangle-alert-BSP6LwrX.js";
import { F as FileCheck } from "./file-check-DjXtFHk6.js";
...
function computeScenarios(opts) {
  ...
  function buildScenario(label, smdBeforeIrrig, irrigMm) {
    const smdAfter = Math.max(0, Math.min(fieldCapacityMm, smdBeforeIrrig - irrigMm));
    const irrigCostPerHa = irrigMm * costPerMmHa;
    const irrigCostTotal = irrigCostPerHa * fieldAreaHa;
    const loss = yieldLoss(smdAfter);
    const stressRange = Math.max(1, fieldCapacityMm - criticalSmdMm);
    const etaEtm = smdAfter <= criticalSmdMm ? 1 : Math.max(0, 1 - (smdAfter - criticalSmdMm) / stressRange);
    const yieldLossRevLoss = loss.tha * fieldAreaHa * cropPricePerTonne;
    const skipLoss = yieldLoss(smdBeforeIrrig);
    const skipRevLoss = skipLoss.tha * fieldAreaHa * cropPricePerTonne;
  ...
  const irrigateNow = buildScenario("Irrigate now", currentSmdMm, irrigateMm);
  const smdIn7 = Math.min(
    fieldCapacityMm,
    Math.max(0, currentSmdMm + currentDailyEtcMm * 7 - expectedRainfall7dMm)
  );
  const wait7 = buildScenario("Wait 7 days", smdIn7, irrigateMm);
  const smdIn14 = Math.min(
    fieldCapacityMm,
    Math.max(0, currentSmdMm + currentDailyEtcMm * 14 - expectedRainfall7dMm)
  );
  const skip = buildScenario("Don't irrigate", smdIn14, 0);
...
function ScenarioCard({ result, accent, icon }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `rounded-lg border p-4 space-y-3 ${accent}`, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-muted-foreground", children: "SMD after" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "font-medium text-right", children: [
        result.projectedSmdAfterMm.toFixed(1),
        " mm"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-muted-foreground", children: "Yield loss est." }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "font-medium text-right", children: [
...
function IrrigationAdvisorTab({ farmId }) {
  ...
  const smdSeries = reactExports.useMemo(() => {
    ...
    if (useManual || !data?.hasWeatherStation) {
      dailyReadings = manualRows.map((r) => ({
        date: r.date,
        tmax: r.tmax ? parseFloat(r.tmax) : void 0,
        tmin: r.tmin ? parseFloat(r.tmin) : void 0,
        rainfall: r.rainfall ? parseFloat(r.rainfall) : void 0
      }));
    } else {

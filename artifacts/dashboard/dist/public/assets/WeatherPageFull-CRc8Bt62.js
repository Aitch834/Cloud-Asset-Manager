import { q as createLucideIcon, b as useAppStore, r as reactExports, j as jsxRuntimeExports, a as useToast, t as useQueryClient, l as useQuery, O as useMutation, c as Button, S as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, d as LoaderCircle, M as MapPin, L as Label, I as Input, J as DialogFooter } from "./index-CH03FTk5.js";
import { A as AppLayout, T as TrendingUp, j as Truck } from "./AppLayout-BxE6YHy9.js";
...
import { R as ResponsiveContainer, X as XAxis, Y as YAxis, T as Tooltip } from "./generateCategoricalChart-CCMAiNYM.js";
...
function ReadingsTab({ farmId }) {
  ...
  const createMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/weather-readings`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/weather-readings/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      toast({ title: "Deleted" });
      invalidate();
      setDeleteId(null);
...
function VehicleReadingsTab({ farmId }) {
  ...
  const createMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/vehicle-weather-readings`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
    onSuccess: () => {
      toast({ title: "Reading saved" });
      invalidate();
      setAddOpen(false);
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/vehicle-weather-readings/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      toast({ title: "Deleted" });
      invalidate();
      setDeleteId(null);
...
function DevicesTab({ farmId }) {
  ...
  const createMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/vehicle-weather-devices`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, body }) => fetch(`/api/farms/${farmId}/vehicle-weather-devices/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/vehicle-weather-devices/${id}`, { method: "DELETE" }),
    onSuccess: () => {
...
function WeatherPageFull() {
  ...
  const [tab, setTab] = reactExports.useState("readings");

import { b as useAppStore, r as reactExports, l as useQuery, j as jsxRuntimeExports, m as Card, T as FlaskConical, t as useQueryClient, a as useToast, O as useMutation, c as Button, S as Plus, d as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, H as DialogDescription, L as Label, I as Input, Q as React } from "./index-DKBMd7uR.js";
import { A as AppLayout, W as Warehouse, I as Info, c as ClipboardList } from "./AppLayout-BPk5oW6I.js";
import { T as TabBar, a as TabButton } from "./tab-button-B9ah5hIq.js";
import { B as Badge } from "./badge-Dh2rvif7.js";
import { CropsTab, WaterTestsTab, HarvestTab, IntakeTab, PackhouseTab, AllergenTab } from "./FreshProducePage-3Rt6Br-d.js";
...
import { L as Leaf, T as TriangleAlert } from "./triangle-alert-DQLNjx6y.js";
...
import "./layout-grid-BVhxX71P.js";
import "./archive-9tJopSYS.js";
...
function SyntheticHistoryPanel({
  ...
  const removePersisted = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp-synthetic-history/${id}`), { method: "DELETE", credentials: "include" }),
...
function BlockStatusTab({ farmId, farmName }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp-block-status/${id}`), { method: "DELETE", credentials: "include" }),
...
function InputLogTab({ farmId, farmName }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp-input-log/${id}`), { method: "DELETE", credentials: "include" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["ofp-input-log", farmId] });
...
function CertificatesTab({ farmId, farmName }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp-certificates/${id}`), { method: "DELETE", credentials: "include" }),
...
function BuyerDeclarationsTab({ farmId, farmName }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp-buyer-declarations/${id}`), { method: "DELETE", credentials: "include" }),
...
function InputDerogationsTab({ farmId, farmName: _farmName }) {
  ...
  const deleteCase = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp/input-derogations/${id}`), { method: "DELETE", credentials: "include" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["ofp-input-derogations", farmId] });
  ...
  const deleteCorresp = useMutation({
    mutationFn: ({ id, caseId }) => fetch(api(`farms/${farmId}/organic-fp/input-derogation-correspondence/${id}`), { method: "DELETE", credentials: "include" }).then(() => {
      loadCorrespDocs(caseId);
    }),
    onError: () => toast({ title: "Failed to delete", variant: "destructive" })
  });
  const deleteDoc = useMutation({
    mutationFn: ({ id, caseId }) => fetch(api(`farms/${farmId}/organic-fp/input-derogation-documents/${id}`), { method: "DELETE", credentials: "include" }).then(() => {
      loadCorrespDocs(caseId);
    }),
    onError: () => toast({ title: "Failed to delete", variant: "destructive" })
  });

import { a as useToast, c as useQueryClient, b as useAppStore, u as useLocation, r as reactExports, m as useQuery, S as useMutation, j as jsxRuntimeExports, d as Button, T as Plus, e as LoaderCircle, X, A as ArrowRight, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, I as Input, N as DialogMutationError } from "./index-szmq1rP5.js";
...
import { A as AppLayout, I as Info, C as CalendarDays, c as ClipboardList, s as AlertDialog, t as AlertDialogContent, v as AlertDialogHeader, w as AlertDialogTitle, x as AlertDialogDescription, y as AlertDialogFooter, z as AlertDialogCancel, D as AlertDialogAction } from "./AppLayout-B_c4q11G.js";
...
const ACTIVE_AGRI_ENV_PROJECT_STATUSES = /* @__PURE__ */ new Set([
  "active",
  "applied",
  "pending"
]);
function calculateAgriEnvDrawdown(projects, milestones) {
  const eligibleProjects = projects.filter(
    (project) => ACTIVE_AGRI_ENV_PROJECT_STATUSES.has(project.status) && (project.totalGrantValuePence ?? 0) > 0
  );
  ...
  const paidPence = milestones.filter(
    (milestone) => eligibleProjectIds.has(milestone.projectId) && milestone.status === "paid"
  ).reduce((sum, milestone) => sum + (milestone.claimAmountPence ?? 0), 0);
  const submittedPence = milestones.filter(
    (milestone) => eligibleProjectIds.has(milestone.projectId) && milestone.status === "submitted"
  ).reduce((sum, milestone) => sum + (milestone.claimAmountPence ?? 0), 0);
  const paidPercentage = totalPence > 0 ? Math.min(100, Math.round(paidPence / totalPence * 100)) : 0;
  const submittedPercentage = totalPence > 0 ? Math.min(
    100 - paidPercentage,
...
const AE_PROJECT_STATUSES = ["applied", "pending", "active", "completed", "suspended", "withdrawn"];
...
function GrantsPage() {
  ...
  const [mainTab, setMainTab] = usePersistedTab({
    page: "grants",
    farmId,

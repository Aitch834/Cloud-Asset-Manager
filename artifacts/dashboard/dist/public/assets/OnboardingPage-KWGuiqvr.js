import { u as useLocation, a as useToast, b as useAppStore, r as reactExports, j as jsxRuntimeExports, B as Building2, L as Label, I as Input, c as Button, d as LoaderCircle, C as Checkbox } from "./index-C3jhrBt5.js";
import { u as useSafeUser, P as Package } from "./use-safe-clerk-i3mLLQVd.js";
...
import { L as Lock } from "./lock-DOaPkQCn.js";
const SECTORS = [
...
function OnboardingPage() {
  ...
  reactExports.useEffect(() => {
    if (step === 3 && modules.length === 0) {
      ...
      fetch(`${"/dashboard/"}api/billing/modules`).then((r) => r.json()).then((data) => {
    ...
    if (adding && createdFarmId && createdTenantSlug) {
      ...
      if (mod) {
        ...
        if (sectorKey && !farmSectors[sectorKey]) {
          ...
          fetch(`${"/dashboard/"}api/farms/${createdFarmId}`, {
            method: "PUT",
  ...
  async function handleBusinessSubmit() {
    ...
    try {
      ...
      if (res.status === 409) {
        ...
        const retry = await fetch(`${"/dashboard/"}api/tenants`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            name: bizName.trim(),
  ...
  async function handleFarmSubmit() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${"/dashboard/"}api/tenants/current/farms`, {
        method: "POST",
        headers: {
  ...
  async function handleSubscribe() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${"/dashboard/"}api/billing/checkout`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-tenant-slug": createdTenantSlug

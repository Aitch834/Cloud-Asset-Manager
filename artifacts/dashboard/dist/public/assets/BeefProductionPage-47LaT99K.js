import { r as reactExports, l as useQuery, j as jsxRuntimeExports, b as useAppStore, t as useQueryClient, a as useToast, O as useMutation, c as Button, S as Plus, d as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, I as Input, C as Checkbox, L as Label } from "./index-CZUDbqE7.js";
...
import { f as Scale, T as TrendingUp, A as AppLayout, e as ChartColumn, c as ClipboardList } from "./AppLayout-Bva0DUIN.js";
...
function WeighTab({ farmId, onRaiseTask }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/beef-weigh-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["beef-weigh", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
...
function FinishingTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/beef-finishing-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["beef-finishing", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
  const [yearFilter, setYearFilter] = reactExports.useState("all");
...
function DeadweightTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/beef-deadweight-settlements/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["beef-deadweight", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function RTChecklistTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/beef-rt-checklists/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["beef-rt", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));

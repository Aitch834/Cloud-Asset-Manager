import { r as reactExports, m as useQuery, j as jsxRuntimeExports, b as useAppStore, b4 as formatAlertIssuedAt, c as useQueryClient, a as useToast, S as useMutation, d as Button, T as Plus, e as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, I as Input, N as DialogMutationError, C as Checkbox, L as Label } from "./index-D-0vFWNm.js";
import { u as usePersistedTab } from "./use-persisted-tab-Cp0WK2go.js";
import { u as usePersistedNumberFilter, a as usePersistedFilter } from "./use-persisted-filter-Bxm9Nqju.js";
...
import { f as Scale, T as TrendingUp, A as AppLayout, e as ChartColumn, c as ClipboardList } from "./AppLayout-C8XAlai2.js";
import { P as Package } from "./use-safe-clerk-BqAMf2ql.js";
import { T as TrendingDown } from "./trending-down-DwZsEbju.js";
import { R as ResponsiveContainer, X as XAxis, Y as YAxis, T as Tooltip, L as Legend, B as Bar } from "./generateCategoricalChart-zo8U9-p2.js";
...
import { S as ShieldAlert } from "./shield-alert-BTlqutDK.js";
...
import "./triangle-alert-TJBT_jme.js";
...
function BeefEnterpriseReport({ farmId }) {
  ...
  const [year, setYear] = usePersistedNumberFilter({ page: "beef-enterprise-report", filter: "year", farmId, defaultValue: currentYear });
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { ref: reportRef, className: "space-y-5", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between flex-wrap gap-3 no-print", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => printElementReport(reportRef.current, { title: "Beef Enterprise Report", subtitle: `${year} enterprise analysis`, farmName }), className: "h-9 px-3 rounded-lg border border-border bg-background text-sm flex items-center gap-1.5 hover:bg-muted/50", children: [
...
function WeighTab({ farmId, onRaiseTask }) {
  ...
  const { data: rows = [], isLoading } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/beef-weigh-records`), { credentials: "include" }).then((r) => r.json())
  });
  const { data: herdsRaw } = useQuery({
    queryKey: ["herds", farmId],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/herds`), { credentials: "include" }).then((r) => r.json())
  });
  ...
  const save = useMutation({
    mutationFn: (body) => {
      const url = editing ? apiUrl(`farms/${farmId}/beef-weigh-records/${editing.id}`) : apiUrl(`farms/${farmId}/beef-weigh-records`);
  ...
  const del = useMutation({ mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/beef-weigh-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
    ...
    isLoading ? /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { className: "animate-spin" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(DataTable, { cols, rows: filtered, onView: setViewing, onEdit: (r) => {
      ...
      setOpen(true);
    }, onDelete: (r) => del.mutate(r.id), deleteMutation: del }),
...
function FinishingTab({ farmId }) {
  ...
  const [viewing, setViewing] = reactExports.useState(null);
  const [form, setForm] = reactExports.useState({});
  const { data: rows = [], isLoading } = useQuery({ queryKey: ["beef-finishing", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/beef-finishing-records`), { credentials: "include" }).then((r) => r.json()) });
  const { data: herdsRaw2 } = useQuery({ queryKey: ["herds", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/herds`), { credentials: "include" }).then((r) => r.json()) });
  ...
  const save = useMutation({
    mutationFn: (body) => {
      const url = editing ? apiUrl(`farms/${farmId}/beef-finishing-records/${editing.id}`) : apiUrl(`farms/${farmId}/beef-finishing-records`);
      return fetch(url, { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(body) });
    },
    onSuccess: () => {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/beef-finishing-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function DeadweightTab({ farmId }) {
  ...
  const { data: rows = [], isLoading } = useQuery({ queryKey: ["beef-deadweight", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/beef-deadweight-settlements`), { credentials: "include" }).then((r) => r.json()) });
  const save = useMutation({
    mutationFn: (body) => {
      const url = editing ? apiUrl(`farms/${farmId}/beef-deadweight-settlements/${editing.id}`) : apiUrl(`farms/${farmId}/beef-deadweight-settlements`);
      return fetch(url, { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(body) });
    },
    onSuccess: () => {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/beef-deadweight-settlements/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function RTChecklistTab({ farmId }) {
  ...
  const { data: rows = [], isLoading } = useQuery({ queryKey: ["beef-rt", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/beef-rt-checklists`), { credentials: "include" }).then((r) => r.json()) });
  ...
  const save = useMutation({ mutationFn: (body) => {
    const url = editing ? apiUrl(`farms/${farmId}/beef-rt-checklists/${editing.id}`) : apiUrl(`farms/${farmId}/beef-rt-checklists`);
    return fetch(url, { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(body) });
  }, onSuccess: () => {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/beef-rt-checklists/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function ReportsTab({ farmId }) {
  ...
  const weighQ = useQuery({ queryKey: ["beef-weigh", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/beef-weigh-records`), { credentials: "include" }).then((r) => r.json()) });
  const finishQ = useQuery({ queryKey: ["beef-finishing", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/beef-finishing-records`), { credentials: "include" }).then((r) => r.json()) });
  const dwQ = useQuery({ queryKey: ["beef-deadweight", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/beef-deadweight-records`), { credentials: "include" }).then((r) => r.json()) });
...
function BeefProductionPage() {
  ...
  const [tab, setTab] = usePersistedTab({ page: "beef-production", farmId, validIds: ["weigh", "finishing", "deadweight", "rt-checklist", "reports", "enterprise"], defaultTab: "weigh" });
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(AppLayout, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-6 max-w-6xl mx-auto space-y-6", children: [
      ...
      beefAlert?.active && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `flex items-start gap-3 rounded-lg border px-4 py-3 text-sm ${beefAlert.level === "national" ? "bg-red-50 border-red-200 text-red-800" : beefAlert.level === "regional" ? "bg-orange-50 border-orange-200 text-orange-800" : "bg-amber-50 border-amber-200 text-amber-800"}`, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(ShieldAlert, { className: "w-4 h-4 mt-0.5 shrink-0" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-semibold", children: beefAlert.level === "national" ? "National Cattle Disease Alert" : beefAlert.level === "regional" ? "Regional Cattle Disease Alert" : "Cattle Disease Notice" }),
          beefAlert.message && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "ml-2", children: beefAlert.message }),
          (beefAlert.issuedAt || beefAlert.date) && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "ml-2 opacity-70 text-xs", children: formatAlertIssuedAt(beefAlert.issuedAt, beefAlert.date) })
        ] })
      ] }),

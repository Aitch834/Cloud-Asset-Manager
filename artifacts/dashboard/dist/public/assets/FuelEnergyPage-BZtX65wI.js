import { q as createLucideIcon, r as reactExports, b as useAppStore, a as useToast, t as useQueryClient, l as useQuery, O as useMutation, j as jsxRuntimeExports, c as Button, S as Plus, L as Label, $ as X, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, I as Input, M as MapPin } from "./index-D3ay9Ao3.js";
import { A as AppLayout, G as Gauge, j as Truck, Z as Zap, E as Flame, c as ClipboardList, F as Fuel } from "./AppLayout-DRFFu3V0.js";
import { T as TabBar, a as TabButton } from "./tab-button-uiRyLhAC.js";
import { T as Textarea } from "./textarea-J0ndDetl.js";
import { S as Select, a as SelectTrigger, b as SelectValue, c as SelectContent, d as SelectItem, e as SelectGroup, f as SelectLabel } from "./select-DSDiluLt.js";
...
import { P as Pen } from "./pen-CWsQKpPa.js";
...
import { F as Funnel } from "./funnel-Dg9lnJoF.js";
import { D as Droplets, C as ClipboardCheck, F as FileText, P as PoundSterling, S as ShieldAlert } from "./shield-alert-B8JTZTuq.js";
import { C as Camera } from "./camera-BRPUmrnE.js";
import { P as Paperclip } from "./paperclip-hVK_oPxO.js";
import { E as ExternalLink } from "./external-link-BqprZWYQ.js";
import { C as CircleCheck } from "./circle-check-DLf6_bdO.js";
import { C as CircleX } from "./circle-x-mR0df5aY.js";
import { W as Wind } from "./wind-BzwrwInl.js";
import { R as ResponsiveContainer, X as XAxis, Y as YAxis, T as Tooltip, L as Legend, B as Bar } from "./generateCategoricalChart-BwSoQy17.js";
import { B as BarChart } from "./BarChart-BsCNi5Ix.js";
import { C as CartesianGrid } from "./CartesianGrid-Drg42ti3.js";
import { C as ComposedChart, A as Area } from "./ComposedChart-Cuh84PgF.js";
...
function SolarRenewablesTab({ farmId }) {
  ...
  const createInstallMut = useMutation({ mutationFn: (b) => fetch(`${base}/solar-installations`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) }).then((r) => r.json()), onSuccess: () => {
  ...
  const updateInstallMut = useMutation({ mutationFn: (b) => fetch(`${base}/solar-installations/${b.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) }).then((r) => r.json()), onSuccess: () => {
  ...
  const deleteInstallMut = useMutation({ mutationFn: (id) => fetch(`${base}/solar-installations/${id}`, { method: "DELETE" }).then((r) => r.json()), onSuccess: () => {
    toast({ title: "Deleted" });
    qc.invalidateQueries({ queryKey: ["solar-installations", farmId] });
  ...
  const createDocMut = useMutation({
    mutationFn: (body) => fetch(`${base}/solar-installations/${body.installationId}/documents`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const deleteDocMut = useMutation({
    mutationFn: ({ installId, docId }) => fetch(`${base}/solar-installations/${installId}/documents/${docId}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const createGenMut = useMutation({ mutationFn: (b) => fetch(`${base}/solar-generation`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) }).then((r) => r.json()), onSuccess: () => {
    toast({ title: "Reading logged" });
    qc.invalidateQueries({ queryKey: ["solar-generation", farmId] });
    setShowGenDialog(false);
    setGenForm(EMPTY_GEN);
  }, onError: () => toast({ title: "Failed to save", variant: "destructive" }) });
  const updateGenMut = useMutation({ mutationFn: (b) => fetch(`${base}/solar-generation/${b.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) }).then((r) => r.json()), onSuccess: () => {
  ...
  const deleteGenMut = useMutation({ mutationFn: (id) => fetch(`${base}/solar-generation/${id}`, { method: "DELETE" }).then((r) => r.json()), onSuccess: () => {
    toast({ title: "Deleted" });
    qc.invalidateQueries({ queryKey: ["solar-generation", farmId] });
    setDeleteGenId(null);
  }, onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const createPaymentMut = useMutation({ mutationFn: (b) => fetch(`${base}/solar-export-payments`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...b, paymentAmountPence: b.paymentAmountPence ? Math.round(parseFloat(b.paymentAmountPence) * 100) : 0 }) }).then((r) => r.json()), onSuccess: () => {
    toast({ title: "Payment recorded" });
    qc.invalidateQueries({ queryKey: ["solar-payments", farmId] });
    setShowPaymentDialog(false);
    setPaymentForm(EMPTY_PAYMENT);
  }, onError: () => toast({ title: "Failed to save", variant: "destructive" }) });
  const updatePaymentMut = useMutation({ mutationFn: (b) => fetch(`${base}/solar-export-payments/${b.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...b, paymentAmountPence: b.paymentAmountPence ? Math.round(parseFloat(b.paymentAmountPence) * 100) : void 0 }) }).then((r) => r.json()), onSuccess: () => {
  ...
  const deletePaymentMut = useMutation({ mutationFn: (id) => fetch(`${base}/solar-export-payments/${id}`, { method: "DELETE" }).then((r) => r.json()), onSuccess: () => {
    toast({ title: "Deleted" });
    qc.invalidateQueries({ queryKey: ["solar-payments", farmId] });
...
function FuelEnergyPage() {
  const [tab, setTab] = reactExports.useState(() => {
    ...
    const t = p.get("tab");
    const valid = ["tanks", "deliveries", "usage", "inspections", "grid-energy", "solar", "reports"];
    return t && valid.includes(t) ? t : "tanks";
  });
  const openId = (() => {
  ...
  const delTankMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/fuel/tanks/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      invalidate();
  ...
  const delDeliveryMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/fuel/deliveries/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      invalidate();
  ...
  const delUsageMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/fuel/usage/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      invalidate();
  ...
  const delInspMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/fuel/storage-inspections/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      invalidate();
  ...
  const delMeterMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/energy/meters/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      invalidate();
  ...
  const delReadingMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/energy/readings/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      invalidate();
  ...
  const delStockCheckMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/fuel/stock-checks/${id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      invalidate();

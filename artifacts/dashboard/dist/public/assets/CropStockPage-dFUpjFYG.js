import { b as useAppStore, j as jsxRuntimeExports, a as useToast, c as useQueryClient, r as reactExports, m as useQuery, S as useMutation, d as Button, T as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, I as Input, J as DialogFooter, e as LoaderCircle, U as FlaskConical, N as DialogMutationError, aA as Check } from "./index-BjPqi5IF.js";
...
import { A as AppLayout, a as Wheat, c as ClipboardList } from "./AppLayout-Bicu0xBR.js";
...
import { b as buildProReport, o as openPrintWindow } from "./print-report-DB1ygEK5.js";
import { A as ArrowLeftRight } from "./arrow-left-right-C1qboh-F.js";
...
import { D as Database } from "./database-RBueB1G4.js";
...
function StockLevelsTab({ farmId }) {
  ...
  const saveLevelMut = useMutation({
    mutationFn: (body) => editRow ? fetch(`/api/farms/${farmId}/crop-stock-levels/${editRow.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
      return r;
    }) : fetch(`/api/farms/${farmId}/crop-stock-levels`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const deleteLevelMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/crop-stock-levels/${id}`, { method: "DELETE" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const harvestMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/crop-stock-movements`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function CropStockPage() {
  ...
  const [tab, setTab] = usePersistedTab({ page: "crop-stock", farmId, validIds: ["stock", "movements", "stocktakes"], defaultTab: "stock" });

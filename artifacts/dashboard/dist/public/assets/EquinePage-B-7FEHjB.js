import { b as useAppStore, c as useQueryClient, a as useToast, r as reactExports, m as useQuery, S as useMutation, j as jsxRuntimeExports, d as Button, I as Input, T as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label } from "./index-XTnQG40m.js";
...
import { A as AppLayout, e as ChartColumn, k as Stethoscope } from "./AppLayout-CHbvi0yr.js";
...
import { a as printRecordReport } from "./record-report-CZROi13k.js";
import { P as Printer } from "./printer-oU9Pfuzf.js";
import { R as ResponsiveContainer, C as Cell, T as Tooltip, L as Legend, X as XAxis, Y as YAxis, B as Bar } from "./generateCategoricalChart-BxWjSzbM.js";
...
function EquinePage() {
  ...
  const [tab, setTab] = usePersistedTab({ page: "equine", farmId, validIds: ["horses", "health", "analytics"], defaultTab: "horses" });
  ...
  const printHealthEvent = (event) => printRecordReport({
  ...
  const [selectedYear, setSelectedYear] = usePersistedNumberFilter({ page: "equine-analytics", filter: "year", farmId, defaultValue: currentYear, isValid: (y) => y > 2e3 && y <= currentYear + 1 });
  const [compareYear, setCompareYear] = reactExports.useState(null);
  reactExports.useEffect(() => {
    setCompareYear((prev) => prev === selectedYear ? null : prev);
  }, [selectedYear]);
  const eventsFiltered = reactExports.useMemo(
    () => events.filter((e) => e.eventDate.startsWith(String(selectedYear))),
    [events, selectedYear]
  );
  const eventsCompare = reactExports.useMemo(
    () => compareYear ? events.filter((e) => e.eventDate.startsWith(String(compareYear))) : [],
    [events, compareYear]
  );
  const eventTypeData = reactExports.useMemo(() => EVENT_TYPES.map((t) => ({
  ...
  const monthlyEvents = reactExports.useMemo(() => MONTH_LABELS.map((label, i) => {
      ...
      events.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center py-8 text-muted-foreground border rounded-lg", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white border rounded-lg p-4", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 180, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(BarChart, { data: monthlyEvents, children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(Bar, { dataKey: "count", fill: COMPARE_COLORS[0], radius: [4, 4, 0, 0], name: compareYear ? String(selectedYear) : "Events" }),

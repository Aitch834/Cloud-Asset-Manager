import { s as createLucideIcon, r as reactExports, m as useQuery, j as jsxRuntimeExports, L as Label, e as LoaderCircle, I as Input, b as useAppStore, R as Redirect, n as Card, o as CardContent, c as useQueryClient, a as useToast, S as useMutation, d as Button, T as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, C as Checkbox, N as DialogMutationError } from "./index-D4AsNSyV.js";
import { R as RaiseTaskDialog } from "./RaiseTaskDialog-DGDFLmud.js";
import { S as Sprout, e as ChartColumn, G as Gauge, T as TrendingUp, I as Info, A as AppLayout, c as ClipboardList } from "./AppLayout-CZKO83oK.js";
...
import { g as generateCategoricalChart, X as XAxis, Y as YAxis, f as formatAxisMap, R as ResponsiveContainer, T as Tooltip, _ as ReferenceLine } from "./generateCategoricalChart-DtFnfs0d.js";
import { A as Area } from "./Area-CBoifnQ_.js";
...
import { T as Tractor } from "./tractor-CDXDtxgl.js";
import { T as TriangleAlert } from "./triangle-alert-DyT2FlKg.js";
import { F as FileCheck } from "./file-check-BjTKS_Kx.js";
...
function computeScenarios(opts) {
  ...
  const skipLoss14 = yieldLoss(smdIn14);
  const skipBaselineRevLoss = skipLoss14.tha * fieldAreaHa * cropPricePerTonne;
  function buildScenario(label, smdAfterIrrig, irrigMm) {
    const irrigCostPerHa = irrigMm * costPerMmHa;
    const irrigCostTotal = irrigCostPerHa * fieldAreaHa;
    const loss = yieldLoss(smdAfterIrrig);
    const stressRange = Math.max(1, fieldCapacityMm - criticalSmdMm);
    const etaEtm = smdAfterIrrig <= criticalSmdMm ? 1 : Math.max(0, 1 - (smdAfterIrrig - criticalSmdMm) / stressRange);
    ...
    return {
      ...
      projectedSmdAfterMm: smdAfterIrrig,
      etaEtmRatio: etaEtm,
  ...
  const smdNowAfter = Math.max(0, Math.min(fieldCapacityMm, currentSmdMm - irrigateMm));
  const irrigateNow = buildScenario("Irrigate now", smdNowAfter, irrigateMm);
  const smdWait7After = Math.max(0, Math.min(fieldCapacityMm, smdIn7 - irrigateMm));
  const wait7 = buildScenario("Wait 7 days", smdWait7After, irrigateMm);

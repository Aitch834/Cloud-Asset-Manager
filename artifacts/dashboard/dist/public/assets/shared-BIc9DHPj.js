import { m as useQuery, j as jsxRuntimeExports, r as reactExports, d as Button, c as useQueryClient, a as useToast, S as useMutation } from "./index-B8sVot4o.js";
...
async function printHarvest(records, farmName, farmId, blocks, farmMeta) {
  ...
  const summaryRows = groupKeys.map((key) => {
    const grpRecords = groupObj[key];
    const totalYieldKgGrp = grpRecords.reduce((s, r) => s + (parseFloat(String(r.yieldKg ?? 0)) || 0), 0);
    const brixVals = grpRecords.map((r) => parseFloat(String(r.brix ?? ""))).filter((v) => !isNaN(v));
    const avgBrix = brixVals.length > 0 ? brixVals.reduce((a, b) => a + b, 0) / brixVals.length : null;
    const thaVals = grpRecords.map((r) => parseFloat(String(r.yieldTonnesPerHa ?? ""))).filter((v) => !isNaN(v));
    const avgTha = thaVals.length > 0 ? thaVals.reduce((a, b) => a + b, 0) / thaVals.length : null;
    const sortedDates = [...new Set(
      grpRecords.map((r) => r.harvestDate ? new Date(r.harvestDate).toLocaleDateString("en-GB") : "").filter(Boolean)
    )];
    const dateStr = sortedDates.length === 0 ? "—" : sortedDates.length === 1 ? sortedDates[0] : `${sortedDates[0]} – ${sortedDates[sortedDates.length - 1]}`;
    let label;
    let photoCell = "";
    if (groupByVintage) {
      label = key;
      ...
      if (hasPhotos) {
        const du = !isNaN(bid) && bid > 0 ? photoDataUrl[bid] : void 0;
    ...
    return `<tr>
      ...
      <td style="padding:5px 5px;border:1px solid #d1d5db;text-align:right">${avgBrix != null ? avgBrix.toFixed(1) + " °" : "—"}</td>
    </tr>`;
  }).join("");
  const summaryPhotoHeader = hasPhotos && !groupByVintage ? `<th style="background:#7c3d12;color:white;padding:6px 5px;text-align:left;width:76px">Photo</th>` : "";
  const summaryPhotoFooterCell = hasPhotos && !groupByVintage ? `<td style="padding:5px 5px;border:1px solid #fdba74;background:#ffedd5"></td>` : "";
...
export {
  ...
  fmtNum as g,
  printHarvest as h,
  printOperations as i,
  printOrganicWineRecords as j,
  printVineRegister as k,
  printExciseReturn as p,
  today as t,
  useFarmMeta as u
};

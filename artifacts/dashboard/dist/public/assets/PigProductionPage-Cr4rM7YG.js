...
import { r as reactExports, l as useQuery, j as jsxRuntimeExports, b as useAppStore, R as Redirect, c as Button, d as LoaderCircle, m as Card, n as CardContent, t as useQueryClient, a as useToast, O as useMutation, S as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, L as Label, I as Input, C as Checkbox, _ as __vitePreload, M as MapPin } from "./index-BVX7s6g6.js";
...
import { T as TrendingUp, A as AppLayout, Q as LayoutDashboard, V as PiggyBank, j as Truck, q as Stethoscope, f as Scale, c as ClipboardList } from "./AppLayout-BcS87_cw.js";
...
import { C as ChevronDown, T as Trash2 } from "./trash-2-D6XtOi8g.js";
...
import { P as Paperclip } from "./paperclip-C_KhDVuw.js";
...
import "./use-safe-clerk-CQJ-LStw.js";
...
function MovementsTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/pig-movements/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
...
function FciDocumentsTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/pig-fci-documents/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function PigPenConsumptionView({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/pig-feed-consumption/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function VetAssessmentsTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/pig-vet-assessments/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function StockmanshipChecksTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/pig-stockmanship-checks/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function TailBitingRisksTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/pig-tail-biting-risks/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function FarrowingRecordsTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/pig-farrowing-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function PigRedTractorChecklistTab({ farmId }) {
  ...
  const save = useMutation({ mutationFn: (b) => fetch(editing ? api(`farms/${farmId}/pig-red-tractor-checklists/${editing.id}`) : api(`farms/${farmId}/pig-red-tractor-checklists`), { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(b) }), onSuccess: () => {
    qc.invalidateQueries({ queryKey: ["pig-rt-checklist", farmId] });
    setOpen(false);
    setForm({});
    setEditing(null);
  }, onError: () => toast({ title: "Save failed", variant: "destructive" }) });
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/pig-red-tractor-checklists/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open, onOpenChange: setOpen, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogContent, { style: { maxWidth: "54rem" }, children: [
...
function MedicineRegisterTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/pig-medicine-treatments/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open, onOpenChange: (v) => !v && setOpen(false), children: /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogContent, { style: { maxWidth: "44rem" }, children: [
...
function KillRecordsTab({ farmId }) {
  ...
  const { data: allKillRecords = [], isLoading } = useQuery({
    ...
    queryFn: () => fetch(api(`farms/${farmId}/pig-kill-records`), { credentials: "include" }).then((r) => r.json()).then((d) => {
  ...
  const save = useMutation({
    mutationFn: (b) => fetch(editing ? api(`farms/${farmId}/pig-kill-records/${editing.id}`) : api(`farms/${farmId}/pig-kill-records`), {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/pig-kill-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
async function generatePigAuditPDF(farmId) {
  const [jsPDFModule, autoTableModule] = await Promise.all([__vitePreload(() => import("./jspdf.es.min-BYz_k5dn.js"), true ? __vite__mapDeps([0,1,2,3]) : void 0), __vitePreload(() => import("./jspdf.plugin.autotable-rNuKHwSj.js"), true ? [] : void 0)]);
...
function PigProductionPage() {
  ...
  const [tab, setTab] = reactExports.useState(() => {
    ...
    const t = p.get("tab");
    const valid = ["overview", "flocks", "movements", "medicine", "fci", "feed", "vet", "stockmanship", "tail-biting", "farrowing", "red-tractor", "kill-records", "salmonella", "vaccination", "disease-monitoring", "analytics"];
...
function SalmonellaMonitoringTab({ farmId }) {
  ...
  async function del(id) {
    ...
    await fetch(api(`farms/${farmId}/pig-salmonella-monitoring/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function PigVaccinationTab({ farmId }) {
  ...
  async function del(id) {
    ...
    await fetch(api(`farms/${farmId}/pig-vaccination-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function PigDiseaseMonitoringTab({ farmId }) {
  ...
  async function del(id) {
    ...
    await fetch(api(`farms/${farmId}/pig-disease-monitoring/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }

import { c as useQueryClient, a as useToast, m as useQuery, r as reactExports, S as useMutation, j as jsxRuntimeExports, d as Button, T as Plus, O as React, M as MapPin, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, L as Label, I as Input, N as DialogMutationError } from "./index-D-0vFWNm.js";
...
import { A as AppLayout, c as ClipboardList } from "./AppLayout-C8XAlai2.js";
import { T as Textarea } from "./textarea-COafKZrF.js";
import { B as Badge } from "./badge-mnNjXrMG.js";
import { P as PhotoPanel } from "./PhotoPanel-JXDHyLOU.js";
...
function FlyTippingPage({ farmId }) {
  ...
  const mut = useMutation({
    mutationFn: async (vars) => {
      if (vars.action === "create") return fetch(`/api/farms/${farmId}/fly-tipping`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(vars.body) }).then(async (r) => {
        if (!r.ok) {
      ...
      if (vars.action === "update") return fetch(`/api/farms/${farmId}/fly-tipping/${vars.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(vars.body) }).then(async (r) => {
        if (!r.ok) {
          const t = await r.text().catch(() => "");
          throw new Error(t || `Request failed (${r.status})`);
        }
      ...
      return fetch(`/api/farms/${farmId}/fly-tipping/${vars.id}`, { method: "DELETE" }).then(async (r) => {
        if (!r.ok) {
          const t = await r.text().catch(() => "");
          throw new Error(t || `Request failed (${r.status})`);
        }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AppLayout, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { maxWidth: 1100, margin: "0 auto", padding: "0 8px" }, children: [
    ...
    deleteId !== null && /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: true, onOpenChange: (o) => {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(DialogMutationError, { mutation: mut, message: "Failed to delete — please try again." }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogFooter, { children: [

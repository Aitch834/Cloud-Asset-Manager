import { s as createLucideIcon, b as useAppStore, j as jsxRuntimeExports, K as Map, c as useQueryClient, a as useToast, r as reactExports, m as useQuery, O as useMutation, e as LoaderCircle, d as Button, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, I as Input, X as DialogMutationError, J as DialogFooter, S as Plus } from "./index-DxRio-2w.js";
import { u as usePersistedTab } from "./use-persisted-tab-Dfn9Nbn5.js";
import { A as AppLayout } from "./AppLayout-EUHbgKOo.js";
...
import { A as Award } from "./award-CqJtSJnx.js";
import { P as Package } from "./use-safe-clerk-B9lE2-1_.js";
import { L as Leaf } from "./triangle-alert-DwmrbLTo.js";
...
function CertificationTab({ farmId }) {
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-venison/certification/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
...
function LandRegisterTab({ farmId }) {
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-venison/land-register/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function FeedSupplementsTab({ farmId }) {
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-venison/feed-supplements/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function VensDerogCard({ farmId, c: rec, isExpanded, onToggle, qc }) {
  ...
  const saveEdit = useMutation({
    mutationFn: (body) => fetch(api(`farms/${farmId}/organic-venison/derogations/${rec.id}`), { method: "PUT", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const delDerog = useMutation({
    mutationFn: () => fetch(api(`farms/${farmId}/organic-venison/derogations/${rec.id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const delCorr = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-venison/derogation-correspondence/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const delDoc = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-venison/derogation-documents/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  async function handleFileUpload(file, docType) {
    setUploading(true);
    try {
      const presignRes = await fetch(api("uploads/presign"), { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fileName: file.name, contentType: file.type, recordType: "organic_venison_derogation" }) }).then(async (r) => {
        if (!r.ok) {
          const t = await r.text().catch(() => "");
          throw new Error(t || `Request failed (${r.status})`);
        }
      ...
      await fetch(api(`farms/${farmId}/organic-venison/derogations/${rec.id}/documents`), { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fileKey, fileName: file.name, fileSize: file.size, mimeType: file.type, documentType: docType }) }).then(async (r) => {
        if (!r.ok) {
          const t = await r.text().catch(() => "");
          throw new Error(t || `Request failed (${r.status})`);
        }
...
function DerogationsTab({ farmId }) {
  ...
  const mutAdd = useMutation({
    mutationFn: (body) => fetch(api(`farms/${farmId}/organic-venison/derogations`), { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }

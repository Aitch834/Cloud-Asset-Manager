import { b as useAppStore, r as reactExports, j as jsxRuntimeExports, R as Redirect, t as useQueryClient, a as useToast, l as useQuery, O as useMutation, c as Button, S as Plus, d as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, H as DialogDescription, L as Label, I as Input, C as Checkbox, m as Card, n as CardContent } from "./index-D3ay9Ao3.js";
...
import { BcsTab, BulkTankTab, CaeTab, AssuranceTab } from "./GoatDairyPage-B78f3QrB.js";
import { S as SccEquipmentSection, D as DairySuppliesTab } from "./DairyPage-DwuWvmNT.js";
import { A as AbrProcurementSection } from "./DairyEnterpriseReport-_CKJ9coD.js";
...
function OrganicGoatDairyPage() {
  ...
  const [tab, setTab] = reactExports.useState("conversion");
...
function FlockConversionTab({ farmId }) {
  ...
  const remove = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-goat-dairy/flock-conversion/${id}`), { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["org-goat-flock-conv", farmId] });
...
function OrganicCollectionsTab({ farmId }) {
  ...
  const remove = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-goat-dairy/collections/${id}`), { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["org-goat-collections", farmId] });
      toast({ title: "Record deleted" });
    },
...
function FeedNutritionTab({ farmId }) {
  ...
  const remove = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-goat-dairy/feed/${id}`), { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["org-goat-feed", farmId] });
      toast({ title: "Record deleted" });
    },
...
function TreatmentRegisterTab({ farmId }) {
  ...
  const remove = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-goat-dairy/treatments/${id}`), { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["org-goat-treatments", farmId] });
      toast({ title: "Record deleted" });
    },

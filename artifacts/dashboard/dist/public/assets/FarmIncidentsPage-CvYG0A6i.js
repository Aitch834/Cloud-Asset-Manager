import { b as useAppStore, a as useToast, t as useQueryClient, r as reactExports, l as useQuery, Q as React, O as useMutation, j as jsxRuntimeExports, c as Button, S as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, L as Label, I as Input, o as Link } from "./index-LlzrVk-g.js";
import { A as AppLayout, E as Flame, c as ClipboardList } from "./AppLayout-Fd1cmhVY.js";
...
function FarmIncidentsPage() {
  ...
  const filtered = evidenceFilter === "all" ? statusFiltered : statusFiltered.filter((i) => (i.photos ?? []).length === 0 && (evidenceFilter === "missing" || EVIDENCE_REQUIRED_TYPES.includes(i.incidentType)));
  const missingCount = incidents.filter((i) => (i.photos ?? []).length === 0).length;
  const missingHighRiskCount = incidents.filter((i) => (i.photos ?? []).length === 0 && EVIDENCE_REQUIRED_TYPES.includes(i.incidentType)).length;
        ...
        [
          ...
          return /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              ...
              title: o.value === "missing" ? "Show only incidents with zero evidence photos" : "Show only Fire, Theft and Criminal Damage incidents with zero evidence photos",
              style: {
                display: "inline-flex",
                alignItems: "center",
                gap: 5,

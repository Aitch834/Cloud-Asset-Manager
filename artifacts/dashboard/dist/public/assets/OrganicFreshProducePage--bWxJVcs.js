import { b as useAppStore, m as useQuery, j as jsxRuntimeExports, n as Card, T as FlaskConical, c as useQueryClient, a as useToast, r as reactExports, O as useMutation, d as Button, S as Plus, e as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, H as DialogDescription, L as Label, I as Input, X as DialogMutationError, Q as React } from "./index-DxRio-2w.js";
import { u as usePersistedTab } from "./use-persisted-tab-Dfn9Nbn5.js";
import { A as AppLayout, W as Warehouse, I as Info, c as ClipboardList } from "./AppLayout-EUHbgKOo.js";
import { T as TabBar, a as TabButton } from "./tab-button-Ca7Ppcdb.js";
import { B as Badge } from "./badge-Ds9V8OdB.js";
import { CropsTab, WaterTestsTab, HarvestTab, IntakeTab, PackhouseTab, AllergenTab } from "./FreshProducePage-DkGnmSLg.js";
...
import { F as FileText, D as Droplets } from "./shield-alert-BEDUX2R0.js";
...
import "./LabSelector-DksECoGX.js";
...
import "./use-farm-members-VIQpw2iF.js";
import "./use-farm-name-7jCK77cs.js";
import "./api-Dhdsf4oM.js";
import "./layout-grid-Dp2f-sTs.js";
import "./archive-S2rp-BYB.js";
...
function SyntheticHistoryPanel({
  ...
  const addPersisted = useMutation({
    mutationFn: (body) => fetch(api(`farms/${farmId}/organic-fp-block-status/${blockStatusId}/synthetic-history`), {
      method: "POST",
  ...
  const removePersisted = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp-synthetic-history/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function BlockStatusTab({ farmId, farmName }) {
    ...
    onSuccess: async (data) => {
      if (!editing && pendingHistory.length > 0) {
        await Promise.all(pendingHistory.map(
          (entry) => fetch(api(`farms/${farmId}/organic-fp-block-status/${data.id}/synthetic-history`), {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp-block-status/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function InputLogTab({ farmId, farmName }) {
  ...
  const { data: logs = [], isLoading } = useQuery({
    ...
    queryFn: () => {
      const url = yearFilter === "all" ? api(`farms/${farmId}/organic-fp-input-log`) : api(`farms/${farmId}/organic-fp-input-log?cropYear=${yearFilter}`);
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp-input-log/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function CertificatesTab({ farmId, farmName }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp-certificates/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function BuyerDeclarationsTab({ farmId, farmName }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp-buyer-declarations/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function InputDerogationsTab({ farmId, farmName: _farmName }) {
  ...
  const deleteCase = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-fp/input-derogations/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const saveCorresp = useMutation({
    mutationFn: (b) => fetch(
      editingCorresp ? api(`farms/${farmId}/organic-fp/input-derogation-correspondence/${editingCorresp.id}`) : api(`farms/${farmId}/organic-fp/input-derogations/${correspCaseId}/correspondence`),
  ...
  const deleteCorresp = useMutation({
    mutationFn: ({ id, caseId }) => fetch(api(`farms/${farmId}/organic-fp/input-derogation-correspondence/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const deleteDoc = useMutation({
    mutationFn: ({ id, caseId }) => fetch(api(`farms/${farmId}/organic-fp/input-derogation-documents/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1 shrink-0 ml-2", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "ghost", size: "icon", title: "Delete", onClick: (e) => {
                  e.stopPropagation();
                  if (confirm("Delete this derogation case and all its correspondence?")) deleteCase.mutate(c.id);
        ...
        isExp && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "border-t px-4 pb-4 pt-3 space-y-5", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-2", children: corresp.map((cr) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-md border bg-muted/20 px-3 py-2 text-sm flex gap-3 items-start", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-1 shrink-0", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "ghost", size: "icon", onClick: () => {
                  if (confirm("Delete this correspondence entry?")) deleteCorresp.mutate({ id: cr.id, caseId: c.id });
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-1.5", children: docs.map((d) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 text-sm rounded border bg-muted/20 px-3 py-2", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "ghost", size: "icon", onClick: () => {
                if (confirm("Delete this document?")) deleteDoc.mutate({ id: d.id, caseId: c.id });
              }, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { className: "h-3 w-3 text-destructive" }) })
            ] }, d.id)) })
          ] })
        ] })

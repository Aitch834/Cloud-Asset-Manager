import { b as useAppStore, j as jsxRuntimeExports, a as useToast, c as useQueryClient, r as reactExports, m as useQuery, S as useMutation, I as Input, d as Button, T as Plus, X, o as Link, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, N as DialogMutationError, J as DialogFooter, e as LoaderCircle, O as React } from "./index-CTNdWNpd.js";
...
import { A as AppLayout, l as ShoppingBag, e as ChartColumn, Z as Zap, a as Wheat, T as TrendingUp } from "./AppLayout-UnB4Z3KW.js";
...
import { F as FileText, P as PoundSterling } from "./shield-alert-BmiBeqnv.js";
...
function buildAccountantPackPrintTitle(farmName, periodLabel, enterpriseLabel, projectLabel) {
  return `Accountant's Financial Pack — ${[
    farmName,
    periodLabel,
    enterpriseLabel,
    projectLabel
  ].filter((value) => !!value).map(escapeHtml).join(" — ")}`;
}
function filterAccountantPackRecords(transactions, purchases, projectId) {
  if (projectId == null) {
...
function shouldResetAccountantPackProjectFilter(projectFilter, selectedProjectExists, projectsSettled, transactionsSettled) {
  return projectFilter !== "all" && projectsSettled && transactionsSettled && !selectedProjectExists;
}
function buildFullPlCsvRows(incomeByCategory, expenseByCategory, periodLabel) {
...
function AccountantPackTab({ farmId }) {
  ...
  const linkedAgriEnvProjectIds = new Set(
    allTx.filter((r) => r.transactionType === "income" && r.agriEnvProjectId != null).map((r) => Number(r.agriEnvProjectId))
  );
  const linkedAgriEnvProjects = agriEnvProjects.filter((project) => linkedAgriEnvProjectIds.has(Number(project.id)));
  const selectedAgriEnvProject = agriEnvProjectFilter === "all" ? null : linkedAgriEnvProjects.find((project) => String(project.id) === agriEnvProjectFilter) ?? null;
  reactExports.useEffect(() => {
  ...
  const enterpriseTx = enterpriseFilter === "all" ? yearTx : yearTx.filter((r) => (r.enterprise ?? "") === enterpriseFilter);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        ...
        agriEnvProjectLabel && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { display: "inline-block", marginTop: 4, marginLeft: enterpriseLabel ? 6 : 0, fontSize: "0.75rem", fontWeight: 600, background: "#f0fdf4", color: "#166534", border: "1px solid #bbf7d0", borderRadius: 4, padding: "2px 8px" }, children: [
          "Agri-environment project: ",
          agriEnvProjectLabel
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }, children: [
        ...
        linkedAgriEnvProjects.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: selectedAgriEnvProject ? String(selectedAgriEnvProject.id) : "all", onValueChange: setAgriEnvProjectFilter, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(SelectTrigger, { "aria-label": "Agri-environment project", style: { width: 220 }, children: /* @__PURE__ */ jsxRuntimeExports.jsx(SelectValue, {}) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(SelectContent, { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: "all", children: "All agri-environment projects" }),
            linkedAgriEnvProjects.map((project) => /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: String(project.id), children: project.schemeName }, project.id))
          ] })
        ] }),

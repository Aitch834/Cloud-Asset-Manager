...
import { r as reactExports, b as useAppStore, _ as __vitePreload, j as jsxRuntimeExports, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, H as DialogDescription, J as DialogFooter, c as Button, l as useQuery, R as Redirect, m as Card, n as CardContent, t as useQueryClient, a as useToast, O as useMutation, S as Plus, d as LoaderCircle, L as Label, I as Input, C as Checkbox, K as Map } from "./index-BS52_Egk.js";
import { L as LabSelector } from "./LabSelector-DPBqBrWY.js";
import { P as Printer } from "./printer-D-DBpuD7.js";
import { R as ResponsiveContainer, X as XAxis, Y as YAxis, T as Tooltip, L as Legend, B as Bar } from "./generateCategoricalChart-BKbDMbtI.js";
...
import { A as AppLayout, W as Warehouse, T as TrendingUp, R as RotateCcw } from "./AppLayout-BWnF0XqL.js";
...
import { u as useFarmName } from "./use-farm-name-DHPzUBwf.js";
import { S as StaffSelect } from "./staff-select-B_9AKlGw.js";
import { L as LayoutGrid } from "./layout-grid-BIHhvasD.js";
...
import { A as Archive } from "./archive-Bz0OKb05.js";
...
function BlockBoundaryMapDialog({ blockId, blockName, open, onClose, onSaved }) {
  ...
  reactExports.useEffect(() => {
    if (open) {
      __vitePreload(() => import("./leaflet-src-lNH0r8uo.js").then((n) => n.l), true ? __vite__mapDeps([0,1,2]) : void 0).then((L) => {
...
function BlocksTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/horticulture-blocks/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
  ...
  const reactivate = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/horticulture-blocks/${id}/reactivate`), { method: "POST", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function CropsTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/horticulture-crops/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function WaterTestsTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/horticulture-water-tests/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function HarvestTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/horticulture-harvest-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function IntakeTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/fresh-produce-intake/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const delLoc = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/fresh-produce-storage-locations/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function PackhouseTab({ farmId }) {
  ...
  const save = useMutation({
    mutationFn: (b) => fetch(api(`farms/${farmId}/horticulture-packhouse-records`), { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(b) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/horticulture-packhouse-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function AllergenTab({ farmId }) {
  ...
  const save = useMutation({ mutationFn: (b) => fetch(api(`farms/${farmId}/allergen-management`), { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(b) }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/allergen-management/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }

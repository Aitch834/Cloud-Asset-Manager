import { b as useAppStore, r as reactExports, j as jsxRuntimeExports, a as useToast, t as useQueryClient, l as useQuery, O as useMutation, I as Input, c as Button, S as Plus, o as Link, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, J as DialogFooter, $ as X, d as LoaderCircle } from "./index-DKBMd7uR.js";
import { u as useLookupStrings } from "./use-lookup-UXexC4gR.js";
import { A as AppLayout, k as ShoppingBag, e as ChartColumn, Z as Zap, a as Wheat, T as TrendingUp } from "./AppLayout-BPk5oW6I.js";
import { T as Textarea } from "./textarea-D5XSJZyx.js";
import { S as Select, a as SelectTrigger, b as SelectValue, c as SelectContent, d as SelectItem, e as SelectGroup, f as SelectLabel } from "./select-C6O8xv2G.js";
import { B as Badge } from "./badge-Dh2rvif7.js";
import { T as TabBar, a as TabButton } from "./tab-button-B9ah5hIq.js";
import { F as FileText, P as PoundSterling } from "./shield-alert-DfBfD1or.js";
...
import { L as Link2 } from "./link-2-DSwqJuFc.js";
...
function TransactionsTab({ farmId }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/financial-transactions/${id}`, { method: "DELETE" }),
    onSuccess: () => {
...
function AttachmentPanel({ farmId, purchaseId }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/record-attachments/${id}`, { method: "DELETE" }),
...
function LivestockPurchasesTab({ farmId }) {
  ...
  const createMut = useMutation({ mutationFn: (b) => fetch(`/api/farms/${farmId}/livestock-purchases`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) }).then((r) => r.json()), onError: () => toast({ title: "Save failed", variant: "destructive" }) });
  const updateMut = useMutation({ mutationFn: (b) => fetch(`/api/farms/${farmId}/livestock-purchases/${b.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) }).then((r) => r.json()), onSuccess: () => {
  ...
  const deleteMut = useMutation({ mutationFn: (id) => fetch(`/api/farms/${farmId}/livestock-purchases/${id}`, { method: "DELETE" }).then((r) => r.json()), onSuccess: () => {
    toast({ title: "Deleted" });
    invalidate();
    setDeleteId(null);
  }, onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const markPaidMut = useMutation({
    mutationFn: (b) => fetch(`/api/farms/${farmId}/livestock-purchases/${b.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ paymentStatus: "paid", paidDate: b.paidDate, paymentMethod: b.paymentMethod, paymentReference: b.paymentReference }) }).then((r) => r.json()),
    onSuccess: () => {
...
function FinancialPage() {
  ...
  const [tab, setTab] = reactExports.useState("transactions");

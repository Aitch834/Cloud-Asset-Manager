import { b as useAppStore, r as reactExports, j as jsxRuntimeExports, B as Building2, a as useToast, t as useQueryClient, l as useQuery, O as useMutation, c as Button, S as Plus, A as ArrowRight, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, I as Input, J as DialogFooter, $ as X } from "./index-D3ay9Ao3.js";
import { O as OtherSelect } from "./other-select-Bn8gazfk.js";
import { u as useLookupStrings } from "./use-lookup-B-NfA9d5.js";
import { A as AppLayout, c as ClipboardList, a as Wheat, j as Truck } from "./AppLayout-DRFFu3V0.js";
...
import { A as ArrowUpRight } from "./arrow-up-right-I77NHN_m.js";
import { A as ArrowLeftRight } from "./arrow-left-right-D-3VmijC.js";
...
import { I as Image } from "./image-BiGdXEEK.js";
...
function DispatchesTab({ farmId }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/haulage/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      toast({ title: "Deleted" });
      invalidate();
      setDeleteId(null);
  ...
  const confirmMut = useMutation({
    mutationFn: ({ id, body }) => fetch(`/api/farms/${farmId}/haulage/${id}/confirm-dispatch`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }),
    onSuccess: () => {
...
function GrainPositionTab({ farmId, onGoToDispatches }) {
  ...
  const [yearFilter, setYearFilter] = reactExports.useState(curYear);
...
function DispatchPlansTab({ farmId }) {
  ...
  const notifyMut = useMutation({
    mutationFn: (planId) => fetch(`/api/farms/${farmId}/dispatch-plans/${planId}/notify-haulier`, { method: "POST" }).then((r) => r.json()),
    onSuccess: (data) => {
      qc.invalidateQueries({ queryKey: ["dispatch-plans", farmId] });
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/dispatch-plans/${id}`, { method: "DELETE" }),
    onSuccess: () => {
  ...
  const statusMut = useMutation({
    mutationFn: ({ id, status }) => fetch(`/api/farms/${farmId}/dispatch-plans/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) }),
...
function HaulierDirectoryTab({ farmId }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/hauliers/${id}`, { method: "DELETE" }),
    onSuccess: () => {
...
function HaulierInvoicesTab({ farmId }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/haulier-invoices/${id}`, { method: "DELETE" }),
    onSuccess: () => {
...
function HaulagePageFull() {
  ...
  const initialTab = () => {
    if (typeof window !== "undefined") {
      ...
      const valid = ["plans", "dispatches", "transfers", "grain-position", "invoices", "directory"];

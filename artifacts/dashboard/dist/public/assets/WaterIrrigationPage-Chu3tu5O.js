import { q as createLucideIcon, b as useAppStore, r as reactExports, j as jsxRuntimeExports, R as Redirect, m as Card, n as CardContent, t as useQueryClient, a as useToast, l as useQuery, O as useMutation, c as Button, S as Plus, d as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, L as Label, I as Input, C as Checkbox } from "./index-CVAOLzqi.js";
import { R as RaiseTaskDialog } from "./RaiseTaskDialog-JJUgo5JF.js";
import { A as AppLayout, e as ChartColumn, c as ClipboardList } from "./AppLayout-DF-K4gcR.js";
...
import { D as Droplets } from "./shield-alert-CXhiHolV.js";
import { T as Tractor } from "./tractor-DSXeZue8.js";
import { C as CloudRain } from "./cloud-rain-Nvcn_Xdb.js";
import { T as TriangleAlert } from "./triangle-alert-BHMJGNJE.js";
import { F as FileCheck } from "./file-check-BHXBsJcm.js";
import { E as Eye } from "./eye-BVe2YUYv.js";
import { P as Pencil } from "./pencil-XLO8LO4W.js";
import { T as Trash2 } from "./trash-2-DCDsjI9z.js";
import { P as Play } from "./play-B4cRmSul.js";
...
function LicencesTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/water-abstraction-licences/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
...
function MeterReadingsTab({ farmId }) {
  ...
  const save = useMutation({ mutationFn: (b) => fetch(api(`farms/${farmId}/water-meter-readings`), { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(b) }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/water-meter-readings/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function BoreholeTestsTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/borehole-tests/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function IrrigationRecordsTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/irrigation-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function IrrigationEquipmentTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/irrigation-equipment/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function SoilMoistureTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/soil-moisture-readings/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function DroughtManagementTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/drought-management-plans/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
...
function CamsReturnsTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/cams-annual-returns/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }

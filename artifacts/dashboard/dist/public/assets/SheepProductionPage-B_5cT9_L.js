import { r as reactExports, l as useQuery, j as jsxRuntimeExports, b as useAppStore, d as LoaderCircle, t as useQueryClient, O as useMutation, c as Button, S as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, I as Input, C as Checkbox, L as Label } from "./index-BKcsQW2l.js";
import { R as RecordAttachments } from "./RecordAttachments-CLBK5iYR.js";
...
import { T as TrendingUp, A as AppLayout, Y as Scissors, f as Scale, e as ChartColumn, c as ClipboardList } from "./AppLayout-B8_1RkJL.js";
import { T as TrendingDown } from "./trending-down-DuisWie6.js";
import { R as ResponsiveContainer, X as XAxis, Y as YAxis, T as Tooltip, L as Legend, B as Bar, C as Cell } from "./generateCategoricalChart-BGCmmOMj.js";
import { B as BarChart } from "./BarChart-L6mwRNTu.js";
import { C as CartesianGrid } from "./CartesianGrid-CIxwhGaq.js";
import { C as ChevronUp } from "./chevron-up-BvI8mfC2.js";
import { C as ChevronDown, T as Trash2 } from "./trash-2-PNmQtpsZ.js";
...
import { S as StaffSelect } from "./staff-select-mJPBaxiU.js";
...
import "./use-upload-DpLOCcbZ.js";
import "./paperclip-99Y3k6cZ.js";
import "./upload-CVARo2wk.js";
import "./image-DNF8sR07.js";
import "./shield-alert-CPHe-hz7.js";
import "./download-BFCo8F8j.js";
import "./use-safe-clerk-C3AlvsMT.js";
import "./database-BbsdnltO.js";
import "./shield-check-CWBpqMg4.js";
import "./tractor-BHt8CiXS.js";
...
function TuppingTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-tupping-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["sheep-tupping", farmId] }) });
...
function ScanningTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-scanning-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["sheep-scanning", farmId] }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function WeighTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-weigh-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["sheep-weigh", farmId] }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function ShearingTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-shearing-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["sheep-shearing", farmId] }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function HealthTab({ farmId }) {
  ...
  const delVacc = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-vaccination-programmes/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["sheep-vacc", farmId] }) });
  const delDisease = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-disease-monitoring/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["sheep-disease", farmId] }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function RTChecklistTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-rt-checklists/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["sheep-rt", farmId] }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));

...
import { T as TrendingUp, A as AppLayout, f as Scale, e as ChartColumn, c as ClipboardList } from "./AppLayout-BPk5oW6I.js";
...
function MatingTab({ farmId }) {
  ...
  const save = useMutation({
    mutationFn: (body) => {
      const url = editing ? api(`farms/${farmId}/goat-mating-records/${editing.id}`) : api(`farms/${farmId}/goat-mating-records`);
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-mating-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-mating", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
...
function ScanningTab({ farmId }) {
  ...
  const save = useMutation({
    mutationFn: (body) => {
      const url = editing ? api(`farms/${farmId}/goat-scanning-records/${editing.id}`) : api(`farms/${farmId}/goat-scanning-records`);
      return fetch(url, { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(body) });
    },
    onSuccess: () => {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-scanning-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-scanning", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function WeighTab({ farmId }) {
  ...
  const save = useMutation({
    mutationFn: (body) => {
      const url = editing ? api(`farms/${farmId}/goat-weigh-records/${editing.id}`) : api(`farms/${farmId}/goat-weigh-records`);
      return fetch(url, { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(body) });
    },
    onSuccess: () => {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-weigh-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-weigh", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function CullTab({ farmId }) {
  ...
  const save = useMutation({
    mutationFn: (body) => {
      const url = editing ? api(`farms/${farmId}/goat-cull-records/${editing.id}`) : api(`farms/${farmId}/goat-cull-records`);
      return fetch(url, { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(body) });
    },
    onSuccess: () => {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-cull-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-cull", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function HealthTab({ farmId }) {
  ...
  const saveVacc = useMutation({ mutationFn: (body) => {
    const url = editing ? api(`farms/${farmId}/goat-vaccination-programmes/${editing.id}`) : api(`farms/${farmId}/goat-vaccination-programmes`);
    return fetch(url, { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(body) });
  }, onSuccess: () => {
  ...
  const saveDisease = useMutation({ mutationFn: (body) => {
    const url = editing ? api(`farms/${farmId}/goat-disease-monitoring/${editing.id}`) : api(`farms/${farmId}/goat-disease-monitoring`);
    return fetch(url, { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(body) });
  }, onSuccess: () => {
  ...
  const delVacc = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-vaccination-programmes/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-vacc", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const delDisease = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-disease-monitoring/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-disease", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function GoatAnalyticsTab({ farmId }) {
  const { data: mating = [] } = useQuery({ queryKey: ["goat-mating", farmId], queryFn: () => fetch(api(`farms/${farmId}/goat-mating-records`), { credentials: "include" }).then((r) => r.json()) });
  const { data: scanning = [] } = useQuery({ queryKey: ["goat-scanning", farmId], queryFn: () => fetch(api(`farms/${farmId}/goat-scanning-records`), { credentials: "include" }).then((r) => r.json()) });
  const { data: weigh = [] } = useQuery({ queryKey: ["goat-weigh", farmId], queryFn: () => fetch(api(`farms/${farmId}/goat-weigh-records`), { credentials: "include" }).then((r) => r.json()) });
  const { data: cull = [] } = useQuery({ queryKey: ["goat-cull", farmId], queryFn: () => fetch(api(`farms/${farmId}/goat-cull-records`), { credentials: "include" }).then((r) => r.json()) });

import { b as useAppStore, a as useToast, c as useQueryClient, r as reactExports, O as React, m as useQuery, S as useMutation, j as jsxRuntimeExports, p as Link, I as Input, d as Button, T as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, L as Label, X, N as DialogMutationError } from "./index-BOs5CvUd.js";
...
import { A as AppLayout, K as Recycle, j as Truck, c as ClipboardList } from "./AppLayout-BrgepLZj.js";
...
function WasteDisposalPage() {
  ...
  const saveMut = useMutation({
    mutationFn: (body) => {
      ...
      if (editRecord) return fetch(`/api/farms/${farmId}/waste/${editRecord.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(async (r) => {
        if (!r.ok) {
      ...
      return fetch(`/api/farms/${farmId}/waste`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(async (r) => {
        if (!r.ok) {
          const t = await r.text().catch(() => "");
          throw new Error(t || `Request failed (${r.status})`);
        }
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/waste/${id}`, { method: "DELETE" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }

import { r as reactExports, l as useQuery, j as jsxRuntimeExports, b as useAppStore, d as LoaderCircle, t as useQueryClient, O as useMutation, c as Button, S as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, I as Input, C as Checkbox, L as Label } from "./index-DZ3-mP8N.js";
...
import { T as TrendingUp, A as AppLayout, f as Scale, e as ChartColumn, c as ClipboardList } from "./AppLayout-D1dNxWO5.js";
...
function MatingTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-mating-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-mating", farmId] }) });
...
function ScanningTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-scanning-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-scanning", farmId] }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function WeighTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-weigh-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-weigh", farmId] }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function CullTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-cull-records/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-cull", farmId] }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));
...
function HealthTab({ farmId }) {
  ...
  const delVacc = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-vaccination-programmes/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-vacc", farmId] }) });
  const delDisease = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-disease-monitoring/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["goat-disease", farmId] }) });
  const sf = (k, v) => setForm((f) => ({ ...f, [k]: v }));

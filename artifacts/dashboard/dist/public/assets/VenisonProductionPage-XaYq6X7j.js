import { r as reactExports, m as useQuery, j as jsxRuntimeExports, b as useAppStore, c as useQueryClient, a as useToast, S as useMutation, e as LoaderCircle, d as Button, T as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, I as Input, C as Checkbox, N as DialogMutationError, J as DialogFooter } from "./index-D-0vFWNm.js";
...
import { T as Target } from "./target-DRwuvHH7.js";
import { C as ChevronUp } from "./chevron-up-CosWAhNU.js";
import { C as ChevronDown, T as Trash2 } from "./trash-2-pyy4HCw3.js";
import { A as AppLayout, _ as Crosshair, U as Users, H as HeartPulse, e as ChartColumn } from "./AppLayout-C8XAlai2.js";
...
import { S as ShoppingCart } from "./shopping-cart-BDEQ7oDj.js";
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { ref: reportRef, className: "space-y-5", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between flex-wrap gap-3 no-print", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => printElementReport(reportRef.current, { title: "Venison Enterprise Report", subtitle: `${year} enterprise analysis`, farmName }), className: "h-9 px-3 rounded-lg border border-border bg-background text-sm flex items-center gap-1.5 hover:bg-muted/50", children: [
...
function HerdsTab({ farmId }) {
  const { data: herds = [] } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/herd-flock-register`), { credentials: "include" }).then((r) => r.json())
  });
...
function CullRecordsTab({ farmId }) {
  ...
  const { data: records = [], isLoading } = useQuery({
    queryKey: ["venison-cull", farmId],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/venison-cull-records`), { credentials: "include" }).then((r) => r.json())
  });
  const { data: herds = [] } = useQuery({
    queryKey: ["herds", farmId],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/herd-flock-register`), { credentials: "include" }).then((r) => r.json())
  ...
  const mutSave = useMutation({
    mutationFn: (data) => fetch(apiUrl(`farms/${farmId}/venison-cull-records${data.id ? `/${data.id}` : ""}`), {
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/venison-cull-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
...
function CarcassSalesTab({ farmId }) {
  ...
  const { data: records = [], isLoading } = useQuery({
    queryKey: ["venison-sales", farmId],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/venison-carcass-sales`), { credentials: "include" }).then((r) => r.json())
  });
  const mutSave = useMutation({
    mutationFn: (data) => fetch(apiUrl(`farms/${farmId}/venison-carcass-sales${data.id ? `/${data.id}` : ""}`), {
      method: data.id ? "PUT" : "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data)
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/venison-carcass-sales/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function HerdMonitoringTab({ farmId }) {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/venison-herd-monitoring`), { credentials: "include" }).then((r) => r.json())
  });
  const { data: herds = [] } = useQuery({
    queryKey: ["herds", farmId],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/herd-flock-register`), { credentials: "include" }).then((r) => r.json())
  ...
  const mutSave = useMutation({
    mutationFn: (data) => fetch(apiUrl(`farms/${farmId}/venison-herd-monitoring${data.id ? `/${data.id}` : ""}`), {
      method: data.id ? "PUT" : "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data)
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/venison-herd-monitoring/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function HealthRecordsTab({ farmId }) {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/venison-health-records`), { credentials: "include" }).then((r) => r.json())
  });
  const { data: herds = [] } = useQuery({
    queryKey: ["herds", farmId],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/herd-flock-register`), { credentials: "include" }).then((r) => r.json())
  ...
  const mutSave = useMutation({
    mutationFn: (data) => fetch(apiUrl(`farms/${farmId}/venison-health-records${data.id ? `/${data.id}` : ""}`), {
      method: data.id ? "PUT" : "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data)
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/venison-health-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function FirearmsRegisterTab({ farmId }) {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/venison-firearms-register`), { credentials: "include" }).then((r) => r.json())
  });
  const mutSave = useMutation({
    mutationFn: (data) => fetch(apiUrl(`farms/${farmId}/venison-firearms-register${data.id ? `/${data.id}` : ""}`), {
      method: data.id ? "PUT" : "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data)
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/venison-firearms-register/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function VenisonAnalyticsTab({ farmId }) {
  const { data: cull = [] } = useQuery({ queryKey: ["venison-cull", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/venison-cull-records`), { credentials: "include" }).then((r) => r.json()) });
  const { data: sales = [] } = useQuery({ queryKey: ["venison-sales", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/venison-carcass-sales`), { credentials: "include" }).then((r) => r.json()) });
  const { data: monitoring = [] } = useQuery({ queryKey: ["venison-monitoring", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/venison-herd-monitoring`), { credentials: "include" }).then((r) => r.json()) });
...
const VENISON_TAB_IDS = ["herds", "cull", "sales", "monitoring", "health", "firearms", "analytics", "enterprise"];
function VenisonProductionPage() {
  const { selectedFarm } = useAppStore();
  const farmId = selectedFarm?.id;
  const [tab, setTab] = usePersistedTab({ page: "venison-production", farmId, validIds: VENISON_TAB_IDS, defaultTab: "herds" });
  if (!farmId) {

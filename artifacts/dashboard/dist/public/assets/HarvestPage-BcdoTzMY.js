import { b as useAppStore, a as useToast, c as useQueryClient, m as useQuery, j as jsxRuntimeExports, M as MapPin, r as reactExports, S as useMutation, I as Input, d as Button, T as Plus, O as React, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, N as DialogMutationError, J as DialogFooter, p as Link } from "./index-DaWtA8cs.js";
import { b as printFromRef } from "./print-report-ClU8-1P0.js";
...
import { A as AppLayout, a as Wheat, f as Scale, u as useUserRole, j as Truck, W as Warehouse } from "./AppLayout-Dl_MNypY.js";
...
import { C as ChevronRight, T as Tractor } from "./tractor-CFYM5rpU.js";
...
function HarvestLogTab({ harvests, transports, storages, farmRecord, equipment, fieldCrops, farmId, loading, onRefresh, toast }) {
  ...
  const [cropYear, setCropYear] = usePersistedNumberFilter({ page: "harvest-log", filter: "crop-year", farmId, defaultValue: currentCropYear(), isValid: (n) => n === 0 || n >= 2e3 && n <= 2100 });
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/harvests/${id}`, { method: "DELETE" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function TransportTab({ transports, harvests, farmId, loading, onRefresh, toast }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/harvest-transport/${id}`, { method: "DELETE" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function StorageTab({ storages, harvests, farmId, loading, onRefresh, toast }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/harvest-storage/${id}`, { method: "DELETE" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function PrintTab({ harvests, transports, storages, farm }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { ref: printRef, style: { background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: "2rem" }, children: [
      ...
      farm?.contactPhone && /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { style: { fontSize: "0.8rem", color: "#6b7280", marginBottom: 2 }, children: [
        "Tel: ",
        farm.contactPhone
      ] }),

...
import { m as useQuery, r as reactExports, j as jsxRuntimeExports, d as Button, u as useLocation, c as useQueryClient, a as useToast, S as useMutation, _ as __vitePreload } from "./index-DhpNTxyq.js";
import { b as buildViticultureCsvContent } from "./csv-DqFyucvM.js";
...
import { C as ChevronsUpDown } from "./chevrons-up-down-BOhgj4E8.js";
...
import { c as ClipboardList } from "./AppLayout-Bk2nlSfQ.js";
...
function buildBlockColorMap$1(blockNames, blockVarietyByName, varietyColorMap) {
  ...
  for (const bname of blockNames) {
    ...
    map[bname] = v ? varietyColorMap[v] ?? YIELD_CHART_COLORS[0] : fallbackPalette[fbIdx++ % (fallbackPalette.length || YIELD_CHART_COLORS.length)] ?? YIELD_CHART_COLORS[0];
  }
  return map;
}
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border border-amber-200 bg-amber-50 p-3.5", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start gap-2.5 mb-2.5", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm font-semibold text-amber-800", children: "FSA / APPA / WineGB registration incomplete" }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              onClick: () => {
                ...
                const firstMissingTargetId = fields.filter((f) => !f.value || String(f.value).trim() === "").map((f) => FIELD_TARGET_IDS[f.label]).find(Boolean);
                setTimeout(() => {
                  document.getElementById(firstMissingTargetId ?? "settings-appa-ref")?.scrollIntoView({ behavior: "smooth", block: "center" });
                }, 400);
              },
              children: "Farm Settings → Viticulture Registrations"
            }
          ),
          "."
        ] })
...
function buildBlockColorMap(blockIds, bvarietyFn) {
  const blockVarietyByName = {};
  ...
  for (const id of blockIds) {
    ...
    colorMap[String(id)] = v ? varietyColorMap[v] ?? YIELD_CHART_COLORS[0] : fallbackPalette[fbIdx++ % (fallbackPalette.length || YIELD_CHART_COLORS.length)] ?? YIELD_CHART_COLORS[0];
  }
  return colorMap;
}
function buildYieldTrendChartSvg(records, blockLookup, sharedBlockColorMap) {
  ...
  const blockColorMap = sharedBlockColorMap ?? buildBlockColorMap(allBlockIds, bvariety);
  const matrix = {};
...
function buildYieldTrendChartTHaSvg(records, blockLookup, sharedBlockColorMap) {
  ...
  const thaBlockColorMap = sharedBlockColorMap ?? buildBlockColorMap(blockIdsWithArea, bvarietyTha);
  const kgMatrix = {};
  for (const r of linkedRows) {
    const bid = String(r.blockId);
...
async function printHarvest(records, farmName, farmId, blocks, farmMeta, yearLabel) {
  const win = window.open("", "_blank", "width=1100,height=850");
  if (!win) return;
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8">
  ...
  if (namedVarietyKeys2.length >= 2) {
    ...
    const varietyBodyRows = sortedVarietyEntries.map(([variety, e]) => {
      ...
      return `<tr>
        ...
        <td style="padding:5px 5px;border:1px solid #d1d5db;text-align:right;font-family:monospace">${avgBrix != null ? avgBrix.toFixed(1) + " °" : "—"}</td>
  ...
  <table style="width:100%;border-collapse:collapse;font-size:10.5px;margin-bottom:18px">
    ...
    <tfoot><tr>
      ...
      <td style="padding:5px 5px;border:1px solid #fdba74;background:#ffedd5;text-align:right;font-family:monospace">${vsGrandBrix2 != null ? vsGrandBrix2.toFixed(1) + " °" : "—"}</td>
...
async function downloadVineSprayDiaryPdf(records, blocks, farmName, farmMeta, searchQuery) {
  const [jsPDFModule, autoTableModule] = await Promise.all([
    __vitePreload(() => import("./jspdf.es.min-CmZV-Gpp.js"), true ? __vite__mapDeps([0,1,2,3]) : void 0),
...
export {
  buildVarietyColorMap as A,
  BBCH_STAGES as B,
  buildBlockColorMap$1 as C,
  DataTable as D,
  Empty as E,
  ...
  UK_ROOTSTOCKS as c,
  ViewField as d,
  buildRpaMailtoHref as e,
  downloadVineSprayDiaryPdf as f,
  emailHarvestReport as g,
  emailOrganicWineRecords as h,
  emailRpaReference as i,
  emailVineRegister as j,
  exportCSV as k,
  fmt as l,
  fmtDate as m,
  fmtNum as n,
  printExciseReturn as o,
  printDiseaseScouting as p,
  printHarvest as q,
  printOperations as r,
  printOrganicWineRecords as s,
  printPhenology as t,
  useFarmMeta as u,
  printRpaReference as v,
  printSprayRecords as w,
  printVineRegister as x,
  printVineSprayDiaryReport as y,
  today as z
};

...
import { F as FarmLocationSelect } from "./FarmLocationSelect-Coi1P_6o.js";
import { A as AppLayout, Z as Zap, E as Flame, c as ClipboardList } from "./AppLayout-CHbvi0yr.js";
...
function PatTestingTab({ farmId, openId }) {
  ...
  const farm = farmData?.farm ?? farmData?.record ?? farmData;
  function handlePrintLabel(eq) {
    printProReport({
      title: "PAT Equipment Label",
      subtitle: "Portable appliance testing identification label",
      farmName: farm?.name,
      ...
      recordLabel: "label",
      landscape: false,
      tableHtml: `<table><tbody><tr><td><strong>Asset Number</strong></td><td>${escapeHtml(eq.assetNumber)}</td></tr><tr><td><strong>Equipment</strong></td><td>${escapeHtml(eq.itemName)}</td></tr><tr><td><strong>Make / Model</strong></td><td>${escapeHtml([eq.make, eq.model].filter(Boolean).join(" ") || "—")}</td></tr><tr><td><strong>Serial Number</strong></td><td>${escapeHtml(eq.serialNumber || "—")}</td></tr></tbody></table>`
    });
  }
  const { data, isLoading } = useQuery({

import { s as createLucideIcon, b as useAppStore, j as jsxRuntimeExports, K as Map, c as useQueryClient, a as useToast, r as reactExports, m as useQuery, S as useMutation, e as LoaderCircle, d as Button, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, I as Input, N as DialogMutationError, J as DialogFooter, T as Plus } from "./index-BEkyPlmL.js";
...
import { A as Award } from "./award-BIUPximm.js";
import { P as Package } from "./use-safe-clerk-DvzsJU13.js";
import { L as Leaf } from "./triangle-alert-Dy8DCGTJ.js";
...
import "./shield-alert-Crl64Cqn.js";
...
function CertificationTab({ farmId }) {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/organic-venison/certification`), { credentials: "include" }).then((r) => r.json())
  });
  const mutSave = useMutation({
    mutationFn: (data) => fetch(apiUrl(`farms/${farmId}/organic-venison/certification${data.id ? `/${data.id}` : ""}`), {
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/organic-venison/certification/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: dlg.open, onOpenChange: (o) => {
      if (!o) {
        setDlg({ open: false, mode: "add", row: {} });
        mutSave.reset();
      }
      ...
      dlg.mode !== "view" && /* @__PURE__ */ jsxRuntimeExports.jsx(DialogMutationError, { mutation: mutSave, message: "Failed to save — your entries are still here." }),
...
function LandRegisterTab({ farmId }) {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/organic-venison/land-register`), { credentials: "include" }).then((r) => r.json())
  });
  const mutSave = useMutation({
    mutationFn: (data) => fetch(apiUrl(`farms/${farmId}/organic-venison/land-register${data.id ? `/${data.id}` : ""}`), {
      method: data.id ? "PUT" : "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data)
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/organic-venison/land-register/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function FeedSupplementsTab({ farmId }) {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/organic-venison/feed-supplements`), { credentials: "include" }).then((r) => r.json())
  });
  const mutSave = useMutation({
    mutationFn: (data) => fetch(apiUrl(`farms/${farmId}/organic-venison/feed-supplements${data.id ? `/${data.id}` : ""}`), {
      method: data.id ? "PUT" : "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data)
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/organic-venison/feed-supplements/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function VensDerogCard({ farmId, c: rec, isExpanded, onToggle, qc }) {
  ...
  const [pendingDeleteDerog, setPendingDeleteDerog] = reactExports.useState(false);
  const { data: corrData } = useQuery({
    queryKey: ["vens-derog-corr", rec.id],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/organic-venison/derogations/${rec.id}/correspondence`), { credentials: "include" }).then((r) => r.json()),
    enabled: isExpanded
  });
  const { data: docsData } = useQuery({
    queryKey: ["vens-derog-docs", rec.id],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/organic-venison/derogations/${rec.id}/documents`), { credentials: "include" }).then((r) => r.json()),
    enabled: isExpanded
  });
  const corrItems = corrData?.items ?? [];
  const docs = docsData?.items ?? [];
  const saveEdit = useMutation({
    mutationFn: (body) => fetch(apiUrl(`farms/${farmId}/organic-venison/derogations/${rec.id}`), { method: "PUT", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const delDerog = useMutation({
    mutationFn: () => fetch(apiUrl(`farms/${farmId}/organic-venison/derogations/${rec.id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const saveCorr = useMutation({
    mutationFn: (body) => {
      const url = editCorr ? apiUrl(`farms/${farmId}/organic-venison/derogation-correspondence/${editCorr.id}`) : apiUrl(`farms/${farmId}/organic-venison/derogations/${rec.id}/correspondence`);
  ...
  const delCorr = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/organic-venison/derogation-correspondence/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const delDoc = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/organic-venison/derogation-documents/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  async function handleFileUpload(file, docType) {
    setUploading(true);
    try {
      const presignRes = await fetch(apiUrl("uploads/presign"), { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fileName: file.name, contentType: file.type, recordType: "organic_venison_derogation" }) }).then(async (r) => {
        if (!r.ok) {
          const t = await r.text().catch(() => "");
          throw new Error(t || `Request failed (${r.status})`);
        }
      ...
      await fetch(uploadUrl, { method: "PUT", headers: { "Content-Type": file.type }, body: file }).then(async (r) => {
        if (!r.ok) {
          const t = await r.text().catch(() => "");
          throw new Error(t || `Request failed (${r.status})`);
        }
      ...
      await fetch(apiUrl(`farms/${farmId}/organic-venison/derogations/${rec.id}/documents`), { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fileKey, fileName: file.name, fileSize: file.size, mimeType: file.type, documentType: docType }) }).then(async (r) => {
        if (!r.ok) {
          const t = await r.text().catch(() => "");
          throw new Error(t || `Request failed (${r.status})`);
        }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "border rounded-md overflow-hidden", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3 px-4 py-3 bg-card hover:bg-muted/30 cursor-pointer", onClick: onToggle, children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-1 shrink-0 items-center", onClick: (e) => e.stopPropagation(), children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "ghost", size: "icon", onClick: () => setPendingDeleteDerog(true), children: /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { className: "h-4 w-4 text-destructive" }) })
      ] })
    ] }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: recordDecisionOpen, onOpenChange: (o) => {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(DialogMutationError, { mutation: saveDecision, message: "Failed to save — your entries are still here." }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: editOpen, onOpenChange: (o) => {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(DialogMutationError, { mutation: saveEdit, message: "Failed to save — your entries are still here." }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogFooter, { children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      ConfirmDialog,
      {
        ...
        mutation: delDerog,
        onConfirm: () => delDerog.mutate(void 0, { onSuccess: () => setPendingDeleteDerog(false) }),
        onCancel: () => {
          setPendingDeleteDerog(false);
          delDerog.reset();
        }
      }
    )
...
function DerogationsTab({ farmId }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/organic-venison/derogations`), { credentials: "include" }).then((r) => r.json())
  });
  const records = data ?? [];
  const mutAdd = useMutation({
    mutationFn: (body) => fetch(apiUrl(`farms/${farmId}/organic-venison/derogations`), { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: addOpen, onOpenChange: (o) => {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(DialogMutationError, { mutation: mutAdd, message: "Failed to create case — your entries are still here." }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogFooter, { children: [
...
const ORGANIC_VENISON_TAB_IDS = ["certification", "land", "feed", "derogations"];
function OrganicVenisonPage() {
  const { selectedFarm } = useAppStore();
  const farmId = selectedFarm?.id;
  const [tab, setTab] = usePersistedTab({ page: "organic-venison", farmId, validIds: ORGANIC_VENISON_TAB_IDS, defaultTab: "certification" });
  if (!farmId) {

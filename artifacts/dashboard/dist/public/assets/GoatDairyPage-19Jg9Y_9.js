import { b as useAppStore, r as reactExports, j as jsxRuntimeExports, R as Redirect, u as useLocation, t as useQueryClient, a as useToast, l as useQuery, O as useMutation, c as Button, S as Plus, d as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, L as Label, I as Input, m as Card, n as CardContent } from "./index-CH03FTk5.js";
import { A as AppLayout } from "./AppLayout-BxE6YHy9.js";
...
import { A as AbrKitStockSection, S as SccEquipmentSection, D as DairySuppliesTab } from "./DairyPage-Bb8a_gtY.js";
import { A as AbrProcurementSection, D as DairyEnterpriseReport } from "./DairyEnterpriseReport-CzBYrXUD.js";
...
import "./RaiseTaskDialog-dMbVM7-h.js";
...
import "./chevron-left-ABpO6ygp.js";
...
function MilkTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/goat-dairy/milk-records/${id}`), { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["goat-dairy-milk", farmId] });
...
function MastitisTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/goat-dairy/mastitis-records/${id}`), { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["goat-dairy-mastitis", farmId] });
      toast({ title: "Deleted" });
    },
...
function KiddingTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/goat-dairy/kidding-records/${id}`), { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["goat-dairy-kidding", farmId] });
      toast({ title: "Deleted" });
    },
...
function BcsTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/goat-dairy/bcs-records/${id}`), { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["goat-dairy-bcs", farmId] });
      toast({ title: "Deleted" });
    },
...
function BulkTankTab({ farmId }) {
  ...
  const delTankM = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-dairy/bulk-tanks/${id}`), { method: "DELETE" }).then((r) => r.json()), onSuccess: () => {
    qc.invalidateQueries({ queryKey: ["goat-dairy-bulk-tanks", farmId] });
    toast({ title: "Tank removed" });
  }, onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const saveMonM = useMutation({ mutationFn: (d) => editingMon ? fetch(api(`farms/${farmId}/goat-dairy/bulk-tank-records/${editingMon.id}`), { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) }).then((r) => r.json()) : fetch(api(`farms/${farmId}/goat-dairy/bulk-tank-records`), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) }).then((r) => r.json()), onSuccess: () => {
    qc.invalidateQueries({ queryKey: ["goat-dairy-tank-records", farmId] });
    setMonDialog(false);
    toast({ title: editingMon ? "Record updated" : "Record saved" });
  }, onError: () => toast({ title: "Save failed", variant: "destructive" }) });
  const delMonM = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/goat-dairy/bulk-tank-records/${id}`), { method: "DELETE" }).then((r) => r.json()), onSuccess: () => {
    qc.invalidateQueries({ queryKey: ["goat-dairy-tank-records", farmId] });
    toast({ title: "Record deleted" });
  }, onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
...
function CaeTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/goat-dairy/cae-monitoring/${id}`), { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["goat-dairy-cae", farmId] });
      toast({ title: "Deleted" });
    },

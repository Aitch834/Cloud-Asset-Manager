import { b as useAppStore, t as useQueryClient, a as useToast, r as reactExports, l as useQuery, O as useMutation, j as jsxRuntimeExports, c as Button, S as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, I as Input, J as DialogFooter, Q as React, A as ArrowRight, H as DialogDescription, d as LoaderCircle } from "./index-CH03FTk5.js";
...
import { A as AppLayout, B as BookOpen, c as ClipboardList } from "./AppLayout-BxE6YHy9.js";
...
function AccidentPhotoPanel({ recordId, farmId, photos }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (photoId) => fetch(`/api/farms/${farmId}/accident-book/${recordId}/photos/${photoId}`, { method: "DELETE" }),
...
function AccidentBookPage() {
  ...
  const createMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/accident-book`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, body }) => fetch(`/api/farms/${farmId}/accident-book/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/accident-book/${id}`, { method: "DELETE" }),
    onSuccess: () => {

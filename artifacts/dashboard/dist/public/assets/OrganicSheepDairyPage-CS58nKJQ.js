import { b as useAppStore, r as reactExports, j as jsxRuntimeExports, R as Redirect, t as useQueryClient, a as useToast, l as useQuery, O as useMutation, c as Button, S as Plus, d as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, L as Label, I as Input, C as Checkbox, H as DialogDescription, m as Card, n as CardContent } from "./index-D3ay9Ao3.js";
...
import { SheepLambingTab, BcsTab, BulkTankTab, MvTab, AssuranceTab } from "./SheepDairyPage-TRc101pt.js";
...
import "./LineChart-BAKxU_HJ.js";
import "./circle-check-DLf6_bdO.js";
...
function FlockConversionTab({ farmId }) {
  ...
  const remove = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-sheep-dairy/flock-conversion/${id}`), { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["org-sheep-flock-conv", farmId] });
...
function OrganicCollectionsTab({ farmId }) {
  ...
  const remove = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-sheep-dairy/collections/${id}`), { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["org-sheep-collections", farmId] });
      toast({ title: "Record deleted" });
    },
...
function FeedNutritionTab({ farmId }) {
  ...
  const remove = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-sheep-dairy/feed/${id}`), { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["org-sheep-feed", farmId] });
      toast({ title: "Record deleted" });
    },
...
function TreatmentRegisterTab({ farmId }) {
  ...
  const remove = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/organic-sheep-dairy/treatments/${id}`), { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["org-sheep-treatments", farmId] });
      toast({ title: "Record deleted" });
    },

import { q as createLucideIcon, b as useAppStore, r as reactExports, j as jsxRuntimeExports, R as Redirect, m as Card, n as CardContent, c as Button, S as Plus, d as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, L as Label, I as Input, t as useQueryClient, a as useToast, l as useQuery, O as useMutation, C as Checkbox } from "./index-BVX7s6g6.js";
import { o as openPrintWindow } from "./print-report-B_FwCCVJ.js";
import { A as AppLayout, k as ShoppingBag, _ as Crosshair, T as TrendingUp } from "./AppLayout-BcS87_cw.js";
...
import { L as LayoutList } from "./layout-list-DVV5QRRY.js";
import { P as PoundSterling, C as ClipboardCheck } from "./shield-alert-BCEKj9Kd.js";
import { P as PawPrint } from "./paw-print-BZbcetdw.js";
...
import { R as ResponsiveContainer, X as XAxis, Y as YAxis, T as Tooltip, B as Bar, C as Cell } from "./generateCategoricalChart-rQnpzASV.js";
...
function FarmShopTab({ farmId }) {
  ...
  const delProd = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/farm-shop-products/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const adjustStock = useMutation({
    mutationFn: ({ id, adjustment }) => fetch(api(`farms/${farmId}/shop-products/${id}/adjust-stock`), { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ adjustment }) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const delSession = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/shop-sales/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const toggleSuppActive = useMutation({
    mutationFn: ({ id, active }) => fetch(api(`farms/${farmId}/shop-suppliers/${id}/${active ? "reactivate" : "deactivate"}`), { method: "PATCH", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const delPurch = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/shop-purchases/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const createStocktakeMut = useMutation({
    mutationFn: (body) => fetch(api(`farms/${farmId}/farm-shop/stocktakes`), { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(body) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const updateStocktakeItemMut = useMutation({
    mutationFn: ({ sessionId, itemId, countedQty }) => fetch(api(`farms/${farmId}/farm-shop/stocktakes/${sessionId}/items/${itemId}`), { method: "PATCH", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ countedQty }) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const completeStocktakeMut = useMutation({
    mutationFn: (sessionId) => fetch(api(`farms/${farmId}/farm-shop/stocktakes/${sessionId}/complete`), { method: "POST", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const deleteStocktakeMut = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/farm-shop/stocktakes/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function IncomeTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/diversification-income/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }

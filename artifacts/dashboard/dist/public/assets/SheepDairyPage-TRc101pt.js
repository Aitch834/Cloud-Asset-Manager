import { b as useAppStore, r as reactExports, j as jsxRuntimeExports, R as Redirect, u as useLocation, t as useQueryClient, a as useToast, l as useQuery, O as useMutation, c as Button, S as Plus, d as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, I as Input, J as DialogFooter, m as Card, n as CardContent, H as DialogDescription, C as Checkbox } from "./index-D3ay9Ao3.js";
import { A as AppLayout } from "./AppLayout-DRFFu3V0.js";
...
import { A as AbrKitStockSection, S as SccEquipmentSection, D as DairySuppliesTab } from "./DairyPage-DwuWvmNT.js";
import { A as AbrProcurementSection, D as DairyEnterpriseReport } from "./DairyEnterpriseReport-_CKJ9coD.js";
...
import { D as Droplets } from "./shield-alert-B8JTZTuq.js";
...
import "./DocAttach-B1IaB-Wp.js";
import "./RaiseTaskDialog-B1Jrhc98.js";
import "./index-BvRHyAif.js";
import "./use-farm-members-Dfj5e__3.js";
import "./staff-select-B4daJZcN.js";
import "./print-report-B_FwCCVJ.js";
import "./vmdMedicines-mq70NSvP.js";
import "./shopping-cart-CCWFSSYJ.js";
import "./circle-x-mR0df5aY.js";
import "./file-down-Bq5uuUv6.js";
import "./trending-down-BXTkzDSE.js";
import "./chevron-left-DVaDlkyO.js";
import "./sparkles-CiLLJCEp.js";
import "./chart-no-axes-column-Dmx8ykVL.js";
import "./receipt-CJAdAERG.js";
import "./thermometer-CXhMiv4Z.js";
import "./chevrons-up-down-BQncuHmz.js";
import "./badge-check-r-RG0Aja.js";
...
function MilkTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-dairy/milk-records/${id}`), { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["sheep-dairy-milk", farmId] });
...
function MastitisTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-dairy/mastitis-records/${id}`), { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["sheep-dairy-mastitis", farmId] });
      toast({ title: "Record deleted" });
    },
...
function SheepLambingTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-dairy/kidding-records/${id}`), { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["sheep-dairy-kidding", farmId] });
      toast({ title: "Record deleted" });
    },
...
function BcsTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-dairy/bcs-records/${id}`), { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["sheep-dairy-bcs", farmId] });
      toast({ title: "Deleted" });
    },
...
function BulkTankTab({ farmId }) {
  ...
  const saveTankM = useMutation({ mutationFn: (d) => editingTank ? fetch(api(`farms/${farmId}/sheep-dairy/bulk-tanks/${editingTank.id}`), { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) }).then((r) => r.json()) : fetch(api(`farms/${farmId}/sheep-dairy/bulk-tanks`), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) }).then((r) => r.json()), onSuccess: () => {
    qc.invalidateQueries({ queryKey: ["sheep-dairy-bulk-tanks", farmId] });
    setTankDialog(false);
    toast({ title: editingTank ? "Tank updated" : "Tank added" });
  }, onError: () => toast({ title: "Save failed", variant: "destructive" }) });
  const delTankM = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-dairy/bulk-tanks/${id}`), { method: "DELETE" }).then((r) => r.json()), onSuccess: () => {
    qc.invalidateQueries({ queryKey: ["sheep-dairy-bulk-tanks", farmId] });
    toast({ title: "Tank removed" });
  }, onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const saveMonM = useMutation({ mutationFn: (d) => editingMon ? fetch(api(`farms/${farmId}/sheep-dairy/bulk-tank-records/${editingMon.id}`), { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) }).then((r) => r.json()) : fetch(api(`farms/${farmId}/sheep-dairy/bulk-tank-records`), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) }).then((r) => r.json()), onSuccess: () => {
    qc.invalidateQueries({ queryKey: ["sheep-dairy-tank-records", farmId] });
    setMonDialog(false);
    toast({ title: editingMon ? "Record updated" : "Record saved" });
  }, onError: () => toast({ title: "Save failed", variant: "destructive" }) });
  const delMonM = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-dairy/bulk-tank-records/${id}`), { method: "DELETE" }).then((r) => r.json()), onSuccess: () => {
    qc.invalidateQueries({ queryKey: ["sheep-dairy-tank-records", farmId] });
    toast({ title: "Record deleted" });
  }, onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
...
function MvTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-dairy/mv-monitoring/${id}`), { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["sheep-dairy-mv", farmId] });
      toast({ title: "Deleted" });
    },
...
function TuppingTab({ farmId }) {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-tupping-records/${id}`), { method: "DELETE", credentials: "include" }),
...
function TreatmentRegisterTab({ farmId }) {
  ...
  const remove = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/sheep-dairy/treatments/${id}`), { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["sheep-dairy-treatments", farmId] });
      toast({ title: "Record deleted" });
    },

import { q as createLucideIcon, b as useAppStore, t as useQueryClient, a as useToast, l as useQuery, r as reactExports, O as useMutation, j as jsxRuntimeExports, c as Button, S as Plus, d as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, I as Input, L as Label } from "./index-CG7Bde15.js";
...
function PhotoPanel({ incidentId, farmId, photos }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (photoId) => fetch(`/api/farms/${farmId}/encampments/${incidentId}/photos/${photoId}`, { method: "DELETE" }),
...
function EncampmentPage() {
  ...
  const createMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/encampments`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["encampments", farmId] });
      setAddOpen(false);
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, body }) => fetch(`/api/farms/${farmId}/encampments/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["encampments", farmId] });
      setEditItem(null);

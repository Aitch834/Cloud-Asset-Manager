import { b as useAppStore, c as useQueryClient, a as useToast, r as reactExports, m as useQuery, S as useMutation, j as jsxRuntimeExports, d as Button, I as Input, T as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label } from "./index-sfXnbVwN.js";
...
import { A as AppLayout, T as TrendingUp } from "./AppLayout-DLu5E63P.js";
...
import "./triangle-alert-BuUwinGL.js";
...
function LambingRecordsPage() {
  ...
  const [tab, setTab] = usePersistedTab({ page: "lambing-records", farmId, validIds: ["records", "analytics"], defaultTab: "records" });
  ...
  const [selectedYear, setSelectedYear] = usePersistedNumberFilter({ page: "lambing-analytics", filter: "year", farmId, defaultValue: currentYear, isValid: (y) => y > 2e3 && y <= currentYear + 1 });
  const [compareYear, setCompareYear] = reactExports.useState(null);
  reactExports.useEffect(() => {
    setCompareYear((prev) => prev === selectedYear ? null : prev);
  }, [selectedYear]);
  const recordsFiltered = reactExports.useMemo(
    () => records.filter((r) => r.lambingDate.startsWith(String(selectedYear))),
    [records, selectedYear]
  );
  const recordsCompare = reactExports.useMemo(
    () => compareYear ? records.filter((r) => r.lambingDate.startsWith(String(compareYear))) : [],
    [records, compareYear]
  );
  const totalLambs = reactExports.useMemo(() => recordsFiltered.reduce((s, r) => s + r.numberOfLambs, 0), [recordsFiltered]);
  const totalMortalities = reactExports.useMemo(() => recordsFiltered.reduce((s, r) => s + r.mortalityCount, 0), [recordsFiltered]);
  const assisted = reactExports.useMemo(() => recordsFiltered.filter((r) => r.assistanceRequired).length, [recordsFiltered]);
  const survivalRate = totalLambs > 0 ? Math.round((totalLambs - totalMortalities) / totalLambs * 100) : null;
  const easeData = reactExports.useMemo(() => [1, 2, 3, 4, 5].map((e) => ({
    ease: EASE_LABELS[e],
    count: recordsFiltered.filter((r) => r.lambingEase === e).length,
    fill: EASE_COLOURS[e]
  })).filter((d) => d.count > 0), [recordsFiltered]);
  const sexData = reactExports.useMemo(() => [
  ...
  const lambsPerEwe = reactExports.useMemo(() => [1, 2, 3, 4].map((n) => ({
    lambs: `${n} lamb${n > 1 ? "s" : ""}`,
    count: recordsFiltered.filter((r) => r.numberOfLambs === n).length
  })).filter((d) => d.count > 0), [recordsFiltered]);
  const MONTH_LABELS = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
  const lambingsByMonth = reactExports.useMemo(() => MONTH_LABELS.map((label, i) => {
  ...
  const printLambingRecord = (record) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(AppLayout, { title: "Lambing Records", children: [
    ...
    tab === "analytics" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
      ...
      lambingsByMonth.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white border rounded-lg p-4", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 160, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(BarChart, { data: lambingsByMonth, children: [
          ...
          compareYear && /* @__PURE__ */ jsxRuntimeExports.jsx(Legend, { iconSize: 10 }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Bar, { dataKey: "count", fill: COMPARE_COLORS[0], radius: [4, 4, 0, 0], name: compareYear ? String(selectedYear) : "Lambings" }),
          compareYear && /* @__PURE__ */ jsxRuntimeExports.jsx(Bar, { dataKey: "count_cmp", fill: COMPARE_COLORS[1], radius: [4, 4, 0, 0], name: String(compareYear) })
        ] }) })
      ] }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-2 text-xs text-muted-foreground", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("strong", { children: [
              assisted,
              " (",
              recordsFiltered.length > 0 ? Math.round(assisted / recordsFiltered.length * 100) : 0,
              "%)"
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            "Avg lambs/ewe: ",
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: recordsFiltered.length > 0 ? (totalLambs / recordsFiltered.length).toFixed(2) : "—" })
          ] })
        ] })
      ] })
    ] }),

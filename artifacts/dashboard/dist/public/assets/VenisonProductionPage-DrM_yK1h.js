import { r as reactExports, m as useQuery, j as jsxRuntimeExports, b as useAppStore, c as useQueryClient, a as useToast, O as useMutation, e as LoaderCircle, d as Button, S as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, I as Input, C as Checkbox, X as DialogMutationError, J as DialogFooter } from "./index-DxRio-2w.js";
...
import { T as Target } from "./target-D5RDikgB.js";
import { C as ChevronUp } from "./chevron-up-DD1_5qJF.js";
import { C as ChevronDown, T as Trash2 } from "./trash-2-D4oUTQ3S.js";
import { A as AppLayout, _ as Crosshair, U as Users, H as HeartPulse, e as ChartColumn } from "./AppLayout-EUHbgKOo.js";
...
function CullRecordsTab({ farmId }) {
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/venison-cull-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
...
function CarcassSalesTab({ farmId }) {
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/venison-carcass-sales/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function HerdMonitoringTab({ farmId }) {
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/venison-herd-monitoring/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function HealthRecordsTab({ farmId }) {
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/venison-health-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function FirearmsRegisterTab({ farmId }) {
  ...
  const mutDel = useMutation({
    mutationFn: (id) => fetch(api(`farms/${farmId}/venison-firearms-register/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }

import { b as useAppStore, r as reactExports, l as useQuery, j as jsxRuntimeExports, a as useToast, t as useQueryClient, O as useMutation, c as Button, S as Plus, M as MapPin, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, L as Label, I as Input, J as DialogFooter } from "./index-CH03FTk5.js";
...
import { A as AppLayout, g as TreePine, C as CalendarDays } from "./AppLayout-BxE6YHy9.js";
...
import { S as StorageLocationMapPicker } from "./StorageLocationMapPicker-BD7voK_z.js";
...
import { L as Leaf, T as TriangleAlert } from "./triangle-alert-DNK0lqUH.js";
...
import { C as ClipboardCheck, D as Droplets } from "./shield-alert-i7MjCWve.js";
...
function EnvironmentalFeaturesTab({ farmId, schemes }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/environmental-features/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      toast({ title: "Deleted" });
      invalidate();
      setDeleteId(null);
...
function AgriEnvSchemesTab({ farmId }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/agri-schemes/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      toast({ title: "Deleted" });
      invalidate();
      setDeleteId(null);
  ...
  const addSchemeCommMut = useMutation({
    mutationFn: (body) => fetch(`/api/farms/${farmId}/agri-schemes/${viewScheme?.id}/communications`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then((r) => r.json()),
    onSuccess: () => {
...
function AssessmentsTab({ farmId }) {
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/environmental-assessments/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      toast({ title: "Deleted" });
      invalidate();
      setDeleteId(null);
...
function ManagementEventsTab({ farmId, features, schemes }) {
  ...
  const [editRecord, setEditRecord] = reactExports.useState(null);
  const [deleteId, setDeleteId] = reactExports.useState(null);
  const [filterFeature, setFilterFeature] = reactExports.useState("__all__");
  ...
  const deleteMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/environmental-management-events/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      toast({ title: "Deleted" });
      invalidate();
      setDeleteId(null);
...
function SFIActionsTab({ farmId, openId }) {
  ...
  const save = useMutation({
    mutationFn: async (body) => {
      ...
      await Promise.all(deletedActionIds.map(
        (id) => fetch(`/api/farms/${farmId}/sfi-actions/${id}`, { method: "DELETE", credentials: "include" })
      ));
      await Promise.all(actions.map(
        (action) => action.id ? fetch(`/api/farms/${farmId}/sfi-actions/${action.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ ...action, agreementId, farmId }) }) : fetch(`/api/farms/${farmId}/sfi-actions`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ ...action, agreementId, farmId }) })
      ));
  ...
  const del = useMutation({
    mutationFn: async (id) => {
      ...
      await Promise.all(acts.map((a) => fetch(`/api/farms/${farmId}/sfi-actions/${a.id}`, { method: "DELETE", credentials: "include" })));
          ...
          "tr",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { className: "px-4 py-3 text-right space-x-1", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { size: "icon", variant: "ghost", onClick: () => setPendingConfirm({ msg: "Delete this SFI/ELMs agreement and all its action codes? This cannot be undone.", fn: () => del.mutate(r.id) }), children: /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { className: "w-3.5 h-3.5 text-red-500" }) })
              ] })
            ]
          },
          Number(r.id)
...
function EnvironmentalPageFull() {
  ...
  const [tab, setTab] = reactExports.useState(() => {
    ...
    const t = p.get("tab");
    const valid = ["features", "schemes", "assessments", "events", "sfi", "slurry", "silage"];
    return t && valid.includes(t) ? t : "features";
  });
  const openId = (() => {
...
function SlurryTab({ farmId, openId }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-6", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: storeOpen, onOpenChange: setStoreOpen, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogContent, { style: { maxWidth: "40rem" }, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: spreadOpen, onOpenChange: setSpreadOpen, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogContent, { style: { maxWidth: "40rem" }, children: [
...
function SilageTab({ farmId }) {
  ...
  const deleteStockMut = useMutation({
    mutationFn: (id) => fetch(`/api/farms/${farmId}/silage-haylage-stock/${id}`, { method: "DELETE", credentials: "include" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["silage-haylage-balance", farmId] });

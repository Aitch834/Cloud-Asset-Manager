import { C as ConfirmDialog, f as formatDate, H as HerdsSection, A as AnimalsSection, V as VetHealthPlansSection, M as MortalitySection, F as FallenStockContractorsSection, a as FeedSection, W as WaterSection, S as SiresSection, b as StrawInventorySection, c as AIReproductionSection, T as TbTestsSection, d as WelfareOutcomeSection } from "./MortalitySection-u7cjGri8.js";
import { e, g, D, E, h, i, j, k, l, O, P, m, n, o, p, q, r } from "./MortalitySection-u7cjGri8.js";
import { c as useQueryClient, a as useToast, r as reactExports, m as useQuery, O as useMutation, j as jsxRuntimeExports, d as Button, S as Plus, e as LoaderCircle, n as Card, o as CardContent, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, H as DialogDescription, L as Label, I as Input, X as DialogMutationError, b as useAppStore, R as Redirect, T as FlaskConical, a4 as ErrorBoundary } from "./index-CrlmjQAc.js";
import { u as usePersistedFilter } from "./use-persisted-filter-Ba-CYcVh.js";
...
import { e as ChartColumn, A as AppLayout, c as ClipboardList, j as Truck, k as Stethoscope } from "./AppLayout-DtIS2asM.js";
import { R as ResponsiveContainer, X as XAxis, Y as YAxis, T as Tooltip, B as Bar, C as Cell } from "./generateCategoricalChart-DS1IfzbO.js";
...
import { C as ConfirmDialog$1 } from "./confirm-dialog-WNje9E6W.js";
import { C as ClipboardCheck, D as Droplets, F as FileText } from "./shield-alert-ut9-9Kop.js";
...
import "./use-lookup-BptUuonq.js";
import "./search-DbJpHqHP.js";
import "./upload-Dnp9XyVR.js";
import "./RaiseTaskDialog-CIDqnB7D.js";
import "./database-D8kZMEr4.js";
import "./circle-x-B6r4b49A.js";
import "./LabSelector-D4mVJ3Zq.js";
import "./index-U4pQ4OBr.js";
import "./syringe-ZMMdsz06.js";
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: rows.map((r2, i2) => {
        ...
        return /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: "border-b last:border-0", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { className: "py-2 text-right space-x-1 whitespace-nowrap", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { size: "icon", variant: "ghost", onClick: () => setPendingConfirm({ msg: "Delete this prescription record? This cannot be undone.", fn: () => del.mutate(r2.id) }), children: /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { className: "w-3.5 h-3.5 text-red-500" }) })
          ] })
        ] }, i2);
      }) })
    ] }) }) }) }),
...
function SheepDippingSection({ farmId }) {
  ...
  function printReport() {
    const rows = records.map((r2) => `<tr><td>${formatDate(r2.dipDate)}</td><td>${r2.productName}</td><td>${r2.dipType}</td><td>${r2.sheepCount}</td><td>${r2.operatorName}</td><td>${r2.operatorCertNumber ?? "—"}</td><td>${r2.disposalMethod ?? "—"}</td><td>${r2.withdrawalPeriodDays != null ? r2.withdrawalPeriodDays + " days" : "—"}</td><td>${formatDate(r2.withdrawalClearDate)}</td></tr>`).join("");
...
function LivestockAnalyticsSection({ farmId }) {
  ...
  const mortalityByMonth = reactExports.useMemo(() => {
    ...
    mortality.forEach((r2) => {
      const d = new Date(String(r2.dateOfDeath || r2.date || ""));
      if (isNaN(d.getTime()) || d < cutoff) return;
      const k2 = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
      map[k2] = (map[k2] || 0) + 1;
    });
    return Object.entries(map).sort().map(([m2, count]) => ({ month: m2.slice(5), count }));
  }, [mortality]);
  ...
  const tbPassRate = reactExports.useMemo(() => {
    const passed = tbTests.filter((r2) => r2.testResult === "clear" || r2.testResult === "passed" || r2.passed === true).length;
    return tbTests.length ? Math.round(passed / tbTests.length * 100) : null;
  }, [tbTests]);
  const aiConceptionRate = reactExports.useMemo(() => {
    const confirmed = aiRecords.filter((r2) => r2.pregnancyConfirmed === true || r2.status === "pregnant").length;

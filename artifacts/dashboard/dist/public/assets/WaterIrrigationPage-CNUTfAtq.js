import { q as createLucideIcon, b as useAppStore, r as reactExports, j as jsxRuntimeExports, R as Redirect, m as Card, n as CardContent, t as useQueryClient, l as useQuery, O as useMutation, c as Button, S as Plus, d as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, L as Label, I as Input, C as Checkbox } from "./index-DZ3-mP8N.js";
import { R as RaiseTaskDialog } from "./RaiseTaskDialog-C5gzDFxP.js";
import { A as AppLayout, e as ChartColumn, c as ClipboardList } from "./AppLayout-D1dNxWO5.js";
...
import "./chevron-up-CxXjqM4J.js";
import "./use-upload-B6MxrfIf.js";
import "./paperclip-DsoIc3nC.js";
import "./upload-CKIHOkzs.js";
import "./image-VGGDRmjz.js";
import "./download-BzJCYjSm.js";
const __iconNode$1 = [
  [
    "path",
...
function LicencesTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/water-abstraction-licences/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["water-licences", farmId] }) });
  const openEdit = (r) => {
    setEditing(r);
...
function MeterReadingsTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/water-meter-readings/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["water-readings", farmId] }) });
...
function BoreholeTestsTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/borehole-tests/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["borehole-tests", farmId] }) });
  function openAdd() {
    setEditing(null);
    setMode("log");
    setForm({});
...
function IrrigationEquipmentTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/irrigation-equipment/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["irrig-equip", farmId] }) });
  const openEdit = (r) => {
    setEditing(r);
    setForm(Object.fromEntries(Object.entries(r).map(([k, v]) => [k, v == null ? "" : String(v)])));
    setOpen(true);
...
function SoilMoistureTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/soil-moisture-readings/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["soil-moisture", farmId] }) });
  const openEdit = (r) => {
    setEditing(r);
    setForm(Object.fromEntries(Object.entries(r).map(([k, v]) => [k, v == null ? "" : String(v)])));
    setOpen(true);
...
function DroughtManagementTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/drought-management-plans/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["drought-plans", farmId] }) });
  const openEdit = (r) => {
    setEditing(r);
    setForm(Object.fromEntries(Object.entries(r).map(([k, v]) => [k, v == null ? "" : String(v)])));
    setOpen(true);
...
function CamsReturnsTab({ farmId }) {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(api(`farms/${farmId}/cams-annual-returns/${id}`), { method: "DELETE", credentials: "include" }), onSuccess: () => qc.invalidateQueries({ queryKey: ["cams-returns", farmId] }) });
  const openEdit = (r) => {
    setEditing(r);
    setForm(Object.fromEntries(Object.entries(r).map(([k, v]) => [k, v == null ? "" : String(v)])));
    setOpen(true);

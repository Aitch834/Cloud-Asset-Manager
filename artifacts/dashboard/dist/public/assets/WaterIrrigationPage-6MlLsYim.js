import { s as createLucideIcon, r as reactExports, m as useQuery, j as jsxRuntimeExports, L as Label, e as LoaderCircle, I as Input, b as useAppStore, R as Redirect, n as Card, o as CardContent, c as useQueryClient, a as useToast, S as useMutation, d as Button, T as Plus, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, J as DialogFooter, C as Checkbox, N as DialogMutationError } from "./index-2vhv0HyJ.js";
import { R as RaiseTaskDialog } from "./RaiseTaskDialog-BLYOnHCy.js";
import { S as Sprout, e as ChartColumn, G as Gauge, T as TrendingUp, I as Info, A as AppLayout, c as ClipboardList } from "./AppLayout-txQ1w-Fw.js";
...
import { R as RecordAttachments } from "./RecordAttachments-B1-HkhJn.js";
...
import { g as generateCategoricalChart, X as XAxis, Y as YAxis, f as formatAxisMap, R as ResponsiveContainer, T as Tooltip, _ as ReferenceLine } from "./generateCategoricalChart-D9BI37CI.js";
import { A as Area } from "./Area-ogy7ca2J.js";
...
import { T as Tractor } from "./tractor-DBdVYKX4.js";
import { T as TriangleAlert } from "./triangle-alert-B3blsqzW.js";
import { F as FileCheck } from "./file-check-BCNuxcEN.js";
import { E as Eye } from "./eye-B_YZaKq1.js";
import { P as Pencil } from "./pencil-B4L73Szw.js";
import { T as Trash2 } from "./trash-2-C9FDvNy3.js";
import { P as Play } from "./play-DGh5uSgS.js";
...
import "./use-upload-CRph-pdC.js";
import "./paperclip-C2TVBvrz.js";
import "./upload-D3c6obDB.js";
import "./image-CksYKLnm.js";
import "./download-Csvs0zJi.js";
const __iconNode$1 = [
  [
    "path",
...
function computeScenarios(opts) {
  ...
  function yieldLoss(smd) {
    if (smd <= criticalSmdMm) return { fraction: 0, tha: 0 };
    const stressRange = Math.max(1, fieldCapacityMm - criticalSmdMm);
    const etaEtm = Math.max(0, 1 - (smd - criticalSmdMm) / stressRange);
    const fraction = Math.max(0, Math.min(0.5, Ky * (1 - etaEtm)));
    return { fraction, tha: typicalYieldTha * fraction };
  }
  const dailyRainMm = expectedRainfall7dMm / 7;
  const clamp = (v) => Math.max(0, Math.min(fieldCapacityMm, v));
  const smdImmediateAfterNow = clamp(currentSmdMm - irrigateMm);
  const smd14AfterNow = clamp(smdImmediateAfterNow + currentDailyEtcMm * 14 - dailyRainMm * 14);
  const smdAtDay7 = clamp(currentSmdMm + currentDailyEtcMm * 7 - dailyRainMm * 7);
  const smdWait7AfterIrrig = clamp(smdAtDay7 - irrigateMm);
  const smd14AfterWait7 = clamp(smdWait7AfterIrrig + currentDailyEtcMm * 7 - dailyRainMm * 7);
  const smd14Skip = clamp(currentSmdMm + currentDailyEtcMm * 14 - dailyRainMm * 14);
  const skipLoss14 = yieldLoss(smd14Skip);
  const skipBaselineRevLoss = skipLoss14.tha * fieldAreaHa * cropPricePerTonne;
  function buildScenario(label, smd14, irrigMm, immediatePostIrrigSmd) {
    const irrigCostPerHa = irrigMm * costPerMmHa;
    const irrigCostTotal = irrigCostPerHa * fieldAreaHa;
    const loss = yieldLoss(smd14);
    const stressRange = Math.max(1, fieldCapacityMm - criticalSmdMm);
    const etaEtm = smd14 <= criticalSmdMm ? 1 : Math.max(0, 1 - (smd14 - criticalSmdMm) / stressRange);
    const yieldLossRevLoss = loss.tha * fieldAreaHa * cropPricePerTonne;
    const revenueSaved = Math.max(0, skipBaselineRevLoss - yieldLossRevLoss);
    return {
      label,
      irrigationMm: irrigMm,
      irrigationCostPerHa: irrigCostPerHa,
      irrigationCostTotal: irrigCostTotal,
      projectedSmdAfterMm: immediatePostIrrigSmd,
      // SMD immediately post-application
      projectedSmd14Mm: smd14,
      // day-14 SMD used for economic calc
      etaEtmRatio: etaEtm,
      yieldLossFraction: loss.fraction,
      yieldLossTha: loss.tha,
      yieldLossRevenueLoss: yieldLossRevLoss,
      irrigationRevenueSaved: revenueSaved,
      // net benefit: revenue recovered vs. skipping, minus cost of irrigation
      netBenefit: revenueSaved - irrigCostTotal
    };
  }
  const irrigateNow = buildScenario("Irrigate now", smd14AfterNow, irrigateMm, smdImmediateAfterNow);
  const wait7 = buildScenario("Wait 7 days", smd14AfterWait7, irrigateMm, smdWait7AfterIrrig);
  const skip = buildScenario("Don't irrigate", smd14Skip, 0, smd14Skip);
  return { irrigateNow, wait7, skip };
}
...
function collapseReadings(readings) {
  const byDate = {};
  for (const r of readings) {
    const date = String(r.recordedAt).slice(0, 10);
    if (!byDate[date]) byDate[date] = { tmaxs: [], tmins: [], rainfalls: [] };
    const val = typeof r.value === "number" ? r.value : parseFloat(String(r.value));
    if (isNaN(val)) continue;
    const cat = r.sensorCategory?.toLowerCase() ?? "";
    const param = r.parameter?.toLowerCase() ?? "";
    if (cat === "temperature") {
      if (param.includes("max")) byDate[date].tmaxs.push(val);
      else if (param.includes("min")) byDate[date].tmins.push(val);
      else {
        byDate[date].tmaxs.push(val);
        byDate[date].tmins.push(val);
      }
    } else if (cat === "rainfall" || cat === "precipitation" || param.includes("rain") || param.includes("precip")) {
      byDate[date].rainfalls.push(val);
    }
  }
  return Object.entries(byDate).sort(([a], [b]) => a.localeCompare(b)).map(([date, { tmaxs, tmins, rainfalls }]) => ({
    date,
    tmax: tmaxs.length ? Math.max(...tmaxs) : void 0,
    tmin: tmins.length ? Math.min(...tmins) : void 0,
    rainfall: rainfalls.length ? rainfalls.reduce((s, v) => s + v, 0) : void 0
  }));
}
...
function ScenarioCard({ result, accent, icon }) {
...
function IrrigationAdvisorTab({ farmId }) {
  ...
  const smdSeries = reactExports.useMemo(() => {
    ...
    if (useManual || !data?.hasWeatherStation) {
      ...
      const collapsed = collapseReadings(data?.sensorReadings ?? []);
      dailyReadings = padReadings(collapsed);
    }
    return computeSMD(
      dailyReadings,
      cropProfile,
  ...
  const chartData = reactExports.useMemo(() => smdSeries.slice(-30).map((d) => ({
    date: d.date.slice(5),
    // MM-DD
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-6", children: [
    ...
    selectedFieldId && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-lg border bg-muted/30 p-4 grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm", children: [
        ...
        platformConfig?.["irrigation.abstractionSource"] && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground", children: "Abstraction Source" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-medium", children: platformConfig["irrigation.abstractionSource"] })
        ] }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 sm:grid-cols-4 gap-3", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground", children: "Forecast / estimate" })
          ] })
        ] })
      ] }),

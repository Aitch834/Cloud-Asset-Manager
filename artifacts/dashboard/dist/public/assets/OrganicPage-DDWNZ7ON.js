import { b as useAppStore, r as reactExports, c as useQueryClient, a as useToast, m as useQuery, j as jsxRuntimeExports, R as Redirect, d as Button, e as LoaderCircle, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, H as DialogDescription, C as Checkbox, L as Label, J as DialogFooter, U as FlaskConical, S as useMutation, T as Plus, n as Card, I as Input, N as DialogMutationError, aC as Check } from "./index-CTNdWNpd.js";
...
import { A as AppLayout, c as ClipboardList, B as BookOpen, I as Info } from "./AppLayout-UnB4Z3KW.js";
...
import { C as Command, a as CommandInput, b as CommandList, c as CommandEmpty, d as CommandGroup, e as CommandItem } from "./command-Zl8xIF6X.js";
...
import { e as escapeHtml } from "./print-report-ClU8-1P0.js";
...
function getInputRegisterApprovalFilterLabel(approvalStatusFilter) {
...
function buildInputRegisterPrintHtml(records, farmName, cropYear, approvalStatusFilter, printCss, printedDate = (/* @__PURE__ */ new Date()).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })) {
...
function getFieldStatusHeaders() {
  return FIELD_STATUS_COLUMNS.map((column) => column.header);
}
function getFieldStatusCsvValues(record) {
  return FIELD_STATUS_COLUMNS.map((column) => column.csvValue(record));
}
function getFieldStatusPrintHeaderHtml() {
  return FIELD_STATUS_COLUMNS.map((column) => `<th>${column.header}</th>`).join("");
}
function getFieldStatusPrintCellsHtml(record) {
  return FIELD_STATUS_COLUMNS.map((column, index) => {
    const style = index === 1 ? ' style="font-weight:600"' : index === 2 || index === 3 ? ' style="white-space:nowrap"' : "";
...
const ORGANIC_INSPECTION_PRINT_HEADERS = ORGANIC_INSPECTION_CSV_HEADERS;
...
function buildOrganicInspectionPrintRows(records) {
  return buildOrganicInspectionCsvRows(records).slice(1).map((row) => row.map((value) => String(value || "—")));
}
function buildOrganicInspectionPrintTableHtml(records) {
  const headers = ORGANIC_INSPECTION_PRINT_HEADERS.map((header) => `<th>${escapeHtml(header)}</th>`).join("");
  const rows = buildOrganicInspectionPrintRows(records).map((row) => `<tr>${row.map((value, index) => `<td${index === 0 || index === 5 ? ' style="white-space:nowrap"' : ""}>${escapeHtml(value)}</td>`).join("")}</tr>`).join("");
...
function OrganicPage() {
  ...
  const [auditPackDialogOpen, setAuditPackDialogOpen] = reactExports.useState(false);
  const [selectedAuditRegisters, setSelectedAuditRegisters] = reactExports.useState({
    certification: true,
    fields: true,
    inspections: true,
  ...
  const { data: farmData } = useQuery({
    ...
    queryFn: () => fetch(`/api/farms/${farmId}`).then(async (r) => {
  ...
  async function downloadAuditPack() {
    if (!farmId) return;
    const selectedKeys = AUDIT_REGISTER_OPTIONS.filter((option) => selectedAuditRegisters[option.key]).map((option) => option.key);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AppLayout, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-4xl mx-auto px-4 py-6 space-y-6", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: auditPackDialogOpen, onOpenChange: (open) => {
      if (!auditPackBusy) setAuditPackDialogOpen(open);
    }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogContent, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogHeader, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(DialogTitle, { children: "Choose registers for the audit pack" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(DialogDescription, { children: "All registers are included by default. Untick any you do not want to share." })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-3 py-2", children: AUDIT_REGISTER_OPTIONS.map((option) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          Checkbox,
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(Label, { htmlFor: `audit-register-${option.key}`, className: "flex-1 cursor-pointer", children: [
          option.label,
          auditPackCounts[option.key] !== null && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "ml-2 text-xs font-normal text-foreground/60", children: [
            "(",
            auditPackCounts[option.key],
            " record",
            auditPackCounts[option.key] === 1 ? "" : "s",
            ")"
          ] })
        ] })
      ] }, option.key)) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogFooter, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Button, { variant: "outline", onClick: () => setAuditPackDialogOpen(false), disabled: auditPackBusy, children: "Cancel" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          Button,
          {

...
import { r as reactExports, b as useAppStore, _ as __vitePreload, j as jsxRuntimeExports, z as Dialog, E as DialogContent, F as DialogHeader, G as DialogTitle, H as DialogDescription, J as DialogFooter, d as Button, m as useQuery, R as Redirect, b6 as formatAlertIssuedAt, n as Card, o as CardContent, c as useQueryClient, a as useToast, S as useMutation, T as Plus, e as LoaderCircle, L as Label, I as Input, C as Checkbox, N as DialogMutationError, K as Map } from "./index-Jc5ltdxf.js";
import { u as usePersistedNumberFilter, a as usePersistedFilter } from "./use-persisted-filter-UoLBCJrN.js";
import { L as LabSelector } from "./LabSelector-DkMRnjf0.js";
import { a as useRawFarmName, b as useFarmName } from "./use-farm-name-CpiqFbkz.js";
import { a as printElementReport } from "./print-report-ClU8-1P0.js";
import { P as Printer } from "./printer-SPoDe0Kj.js";
import { R as ResponsiveContainer, X as XAxis, Y as YAxis, T as Tooltip, L as Legend, B as Bar } from "./generateCategoricalChart-Cwf5iP_1.js";
import { B as BarChart } from "./BarChart-BroTtQ7g.js";
import { C as CartesianGrid } from "./CartesianGrid-DicgGNby.js";
import { L as Leaf, T as TriangleAlert } from "./triangle-alert-D6q6f7zb.js";
import { C as ChevronUp } from "./chevron-up-ddbIky4g.js";
import { C as ChevronDown, T as Trash2 } from "./trash-2-CdRo6b-V.js";
import { A as AppLayout, W as Warehouse, T as TrendingUp, R as RotateCcw } from "./AppLayout-B3GaN3Ka.js";
...
import { S as ShieldAlert, D as Droplets } from "./shield-alert-CY6v8MhG.js";
import { L as LayoutGrid } from "./layout-grid-BPBMkFTz.js";
...
import { A as Archive } from "./archive-C4R2sxjQ.js";
import "./database-D30OFWWI.js";
...
function BlockBoundaryMapDialog({ blockId, blockName, open, onClose, onSaved }) {
  ...
  reactExports.useEffect(() => {
    if (open) {
      __vitePreload(() => import("./leaflet-src-DSvUflHy.js").then((n) => n.l), true ? __vite__mapDeps([0,1,2]) : void 0).then((L) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { ref: reportRef, className: "space-y-5", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between flex-wrap gap-3 no-print", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => printElementReport(reportRef.current, { title: "Fresh Produce Report", subtitle: `${year} harvest and intake report`, farmName }), className: "h-9 px-3 rounded-lg border border-border bg-background text-sm flex items-center gap-1.5 hover:bg-muted/50", children: [
...
function BlocksTab({ farmId }) {
  ...
  const { data: allBlocks = [], isLoading } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/horticulture-blocks?includeRetired=true`), { credentials: "include" }).then((r) => r.json())
  });
  ...
  const { data: farmFields = [] } = useQuery({
    ...
    queryFn: () => fetch(apiUrl(`farms/${farmId}/fields`), { credentials: "include" }).then((r) => r.ok ? r.json().then((d) => Array.isArray(d) ? d : d.records ?? []) : [])
  });
  reactExports.useEffect(() => {
    if (open && !editing) {
  ...
  const save = useMutation({
    mutationFn: (b) => {
      ...
      setCodeError(null);
      return fetch(editing ? apiUrl(`farms/${farmId}/horticulture-blocks/${editing.id}`) : apiUrl(`farms/${farmId}/horticulture-blocks`), {
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/horticulture-blocks/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
  ...
  const retire = useMutation({
    mutationFn: ({ id, ...body }) => fetch(apiUrl(`farms/${farmId}/horticulture-blocks/${id}/retire`), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
  ...
  const reactivate = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/horticulture-blocks/${id}/reactivate`), { method: "POST", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
    ...
    retireRecord && /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open: true, onOpenChange: (o) => {
      if (!o) {
        setRetireRecord(null);
        retire.reset();
      }
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(DialogMutationError, { mutation: retire, message: "Failed to save — your entries are still here." }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogFooter, { children: [
...
function CropsTab({ farmId }) {
  ...
  const { data: blocks = [] } = useQuery({ queryKey: ["horti-blocks", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/horticulture-blocks`), { credentials: "include" }).then((r) => r.json()) });
  const { data: crops = [], isLoading } = useQuery({ queryKey: ["horti-crops", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/horticulture-crops`), { credentials: "include" }).then((r) => r.json()) });
  const save = useMutation({ mutationFn: (b) => fetch(editing ? apiUrl(`farms/${farmId}/horticulture-crops/${editing.id}`) : apiUrl(`farms/${farmId}/horticulture-crops`), { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(b) }), onSuccess: () => {
    qc.invalidateQueries({ queryKey: ["horti-crops", farmId] });
    setOpen(false);
    setForm({});
    setEditing(null);
  }, onError: () => toast({ title: "Save failed", variant: "destructive" }) });
  const del = useMutation({ mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/horticulture-crops/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
    return r;
  }), onSuccess: () => qc.invalidateQueries({ queryKey: ["horti-crops", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const openEdit = (r) => {
    setEditing(r);
...
function WaterTestsTab({ farmId }) {
  ...
  const { data: tests = [], isLoading } = useQuery({ queryKey: ["horti-water", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/horticulture-water-tests`), { credentials: "include" }).then((r) => r.json()) });
  const save = useMutation({
    mutationFn: (b) => fetch(editing ? apiUrl(`farms/${farmId}/horticulture-water-tests/${editing.id}`) : apiUrl(`farms/${farmId}/horticulture-water-tests`), { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(b) }),
    onSuccess: () => {
  ...
  const del = useMutation({ mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/horticulture-water-tests/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
    return r;
  }), onSuccess: () => qc.invalidateQueries({ queryKey: ["horti-water", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  function openAdd() {
    setEditing(null);
    setMode("log");
    setForm({});
...
function HarvestTab({ farmId }) {
  ...
  const [editing, setEditing] = reactExports.useState(null);
  const [form, setForm] = reactExports.useState({});
  const { data: records = [], isLoading } = useQuery({ queryKey: ["horti-harvest", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/horticulture-harvest-records`), { credentials: "include" }).then((r) => r.json()) });
  const save = useMutation({ mutationFn: (b) => fetch(editing ? apiUrl(`farms/${farmId}/horticulture-harvest-records/${editing.id}`) : apiUrl(`farms/${farmId}/horticulture-harvest-records`), { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(b) }), onSuccess: () => {
    qc.invalidateQueries({ queryKey: ["horti-harvest", farmId] });
    setOpen(false);
    setForm({});
    setEditing(null);
  }, onError: () => toast({ title: "Save failed", variant: "destructive" }) });
  const del = useMutation({ mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/horticulture-harvest-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
    return r;
  }), onSuccess: () => qc.invalidateQueries({ queryKey: ["horti-harvest", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
  const openEdit = (r) => {
    setEditing(r);
    setForm(Object.fromEntries(Object.entries(r).map(([k, v]) => [k, v == null ? "" : String(v)])));
    setOpen(true);
...
function useHarvestRecords(farmId) {
  return useQuery({
    queryKey: ["horti-harvest", farmId],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/horticulture-harvest-records`), { credentials: "include" }).then((r) => r.json())
  });
}
...
function IntakeTab({ farmId }) {
  ...
  const { data: records = [], isLoading } = useQuery({
    queryKey: ["fp-intake", farmId],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/fresh-produce-intake`), { credentials: "include" }).then((r) => r.json())
  });
  const { data: harvestRecords = [] } = useHarvestRecords(farmId);
  const { data: crops = [] } = useQuery({
    queryKey: ["horti-crops", farmId],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/horticulture-crops`), { credentials: "include" }).then((r) => r.json())
  });
  const { data: storageLocs = [], refetch: refetchLocs } = useQuery({
    queryKey: ["fp-storage-locs", farmId],
    queryFn: () => fetch(apiUrl(`farms/${farmId}/fresh-produce-storage-locations`), { credentials: "include" }).then((r) => r.json())
  });
  ...
  const save = useMutation({
    mutationFn: (b) => fetch(apiUrl(`farms/${farmId}/fresh-produce-intake`), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/fresh-produce-intake/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const addLoc = useMutation({
    mutationFn: (name) => fetch(apiUrl(`farms/${farmId}/fresh-produce-storage-locations`), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
  ...
  const delLoc = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/fresh-produce-storage-locations/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open, onOpenChange: (o) => {
      setOpen(o);
      if (!o) save.reset();
    }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogContent, { style: { maxWidth: "42rem" }, className: "max-h-[90vh] overflow-y-auto", children: [
...
function PackhouseTab({ farmId }) {
  ...
  const [viewRecord, setViewRecord] = reactExports.useState(null);
  const [form, setForm] = reactExports.useState({});
  const { data: records = [], isLoading } = useQuery({ queryKey: ["horti-packhouse", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/horticulture-packhouse-records`), { credentials: "include" }).then((r) => r.json()) });
  const { data: harvestRecords = [] } = useHarvestRecords(farmId);
  const { data: intakeRecords = [] } = useQuery({ queryKey: ["fp-intake", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/fresh-produce-intake`), { credentials: "include" }).then((r) => r.json()) });
  const save = useMutation({
    mutationFn: (b) => fetch(apiUrl(`farms/${farmId}/horticulture-packhouse-records`), { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(b) }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
  ...
  const del = useMutation({
    mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/horticulture-packhouse-records/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
      if (!r.ok) {
        const t = await r.text().catch(() => "");
        throw new Error(t || `Request failed (${r.status})`);
      }
...
function AllergenTab({ farmId }) {
  ...
  const [viewRecord, setViewRecord] = reactExports.useState(null);
  const [form, setForm] = reactExports.useState({});
  const { data: records = [], isLoading } = useQuery({ queryKey: ["horti-allergen", farmId], queryFn: () => fetch(apiUrl(`farms/${farmId}/allergen-management`), { credentials: "include" }).then((r) => r.json()) });
  const save = useMutation({ mutationFn: (b) => fetch(apiUrl(`farms/${farmId}/allergen-management`), { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(b) }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
  ...
  const del = useMutation({ mutationFn: (id) => fetch(apiUrl(`farms/${farmId}/allergen-management/${id}`), { method: "DELETE", credentials: "include" }).then(async (r) => {
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      throw new Error(t || `Request failed (${r.status})`);
    }
    return r;
  }), onSuccess: () => qc.invalidateQueries({ queryKey: ["horti-allergen", farmId] }), onError: () => toast({ title: "Delete failed", variant: "destructive" }) });
...
function FreshProducePage() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AppLayout, { title: "Fresh Produce", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
    hortiAlert?.active && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `flex items-start gap-3 p-3 rounded-lg text-white ${hortiAlert.level === "national" ? "bg-red-600" : hortiAlert.level === "regional" ? "bg-orange-500" : "bg-amber-500"}`, children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "font-semibold text-sm", children: [
          "Plant Health Alert",
          hortiAlert.level ? ` — ${hortiAlert.level.charAt(0).toUpperCase() + hortiAlert.level.slice(1)}` : ""
        ] }),
        hortiAlert.message && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs mt-0.5 opacity-90", children: hortiAlert.message }),
        (hortiAlert.issuedAt || hortiAlert.date) && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs opacity-75 mt-0.5", children: formatAlertIssuedAt(hortiAlert.issuedAt, hortiAlert.date) })
      ] })
    ] }),

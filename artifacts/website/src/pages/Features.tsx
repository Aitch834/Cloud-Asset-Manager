    ...
    modules: [
      {
        ...
        features: [
          ...
          "TB Test Register — log every APHA-required bovine tuberculosis test with test date and read date, Official Veterinarian name (auto-populated from your Vet Health Plans with phone and practice address), test type (Routine / Pre-Movement / Post-Movement / Short Interval Test), number of animals tested, result (Pass / Fail / Inconclusive), APHA breakdown reference, and restriction status; view a complete TB test history panel on each herd showing all past test results in chronological order; print an individual test report or the full herd TB test history for APHA submission or Red Tractor audit; upcoming TB test read dates surface automatically in the Farm Planner as reminder task cards",
          "Welfare Outcome Assessments (WOA) — structured animal-based welfare scoring records for Red Tractor Beef & Lamb and Dairy standards; species-specific herd picker filters your herd register to show only herds of the relevant species; outcome indicators and scoring metrics are pre-loaded and auto-calculate totals for the selected assessment type; capture indicator scores, overall outcome (Good / Satisfactory / Action Required), corrective actions taken, and follow-up date; view-before-edit panel with printable assessment reports suitable for Red Tractor assessor presentation",
    ...
    modules: [
      {
        ...
        features: [
          ...
          "Mobility Scoring — herd-wide lameness assessments using the AHDB 0–3 scale; pick animals individually or assess the whole herd; automatic prevalence calculation (% of herd at score 2 or 3) with Red Tractor threshold indicators; assessor name, date, and pen or group captured; printable mobility report for herd health review",
          ...
          "Mobile dairy recording — calving records, mastitis records, DCT records, BCS assessments, bulk tank entries, and mobility scorings can all be captured on the mobile app in the field; records save offline and sync to the dashboard automatically when connectivity is restored",
        ],
      },
    ],
  },
    ...
    modules: [
      ...
      },
      {
        ...
        features: [
          ...
          "Week Ahead Planner integration — Expected Certification Dates for in-conversion herds surface as emerald / amber / red event cards in the Farm Planner, escalating colour as the date approaches or passes; standalone treatment organic withdrawal end dates appear as separate emerald cards; Medicine Register organic treatments surface their doubled withdrawal date as an additional card distinct from the standard withdrawal event",
      ...
      },
      {
        ...
        features: [
          ...
          "Organic Input Log — record every approved input applied to a block: product name, date applied, quantity, unit, and purpose; supplier linked from your Trade Contacts list (with PO reference and GRN reference for audit traceability); applied-by staff lookup from your farm team register",
          ...
          "Print-ready compliance reports on every tab — a Print Report button on each of the four tabs (Block Status Register, Input Log, Certificates, Buyer Declarations) generates a formatted A4 printout with farm name header, current date, and complete record set, ready for certifier inspection presentation",
          ...
          "Fresh Produce hub (non-organic) — the complementary Fresh Produce module at /fresh-produce manages growing blocks, crop records, water quality testing, harvest records, pre-cooling and intake, packhouse and despatch, and allergen management; organic status badges from the Organic Fresh Produce module surface on all related block records",
          ...
          "Mobile outdoor access recording — record pasture and outdoor access events for fresh produce crops in the field; captures date, hours, area, stocking density, and weather conditions",
        ],
      },
      {
        ...
        features: [
          ...
          "Labour & Timesheet Management — five-tab module within Staff & Training covering Timesheets, Rota & Shifts, Holiday & Absence, Pay Summary, and Working Time Regulations compliance; access from the main Staff & Training page or via the dedicated Labour Management menu item",
          "Timesheet recording — log hours worked for every staff member against a specific date, operation type (Cultivation, Drilling, Spraying, Harvesting, Livestock Care, Maintenance, Administration, or Other), and optional linked field or livestock group; monthly view groups hours by staff member and operation type with totals; filter by month, staff member, or department; mobile app allows field staff to log hours offline with automatic sync",
          "Rota & Shifts planning — create shift entries per staff member with start time, end time, break duration, and role or location; month-at-a-glance view shows all shifts with colour-coded staff cards; managers can see coverage at a glance for each day without opening individual records",
          "Holiday & Absence recording — log holiday requests, approved leave, sick days, and other absences per staff member; captures absence type (Annual Leave, Sick, Compassionate, Maternity, Paternity, Unpaid, or Other), start date, end date, and total days; annual leave entitlement balance is tracked alongside recorded absences so remaining days are always visible; staff can submit leave requests (Annual Leave, Compassionate, Unpaid, Training Day, or Other) directly from the mobile app — request arrives on the dashboard as Pending, triggers an SMS to all opted-in managers, and the Holiday & Absence tab shows a pending badge and an amber approval panel where managers approve or decline with one click; on approval or decline the staff member receives an automatic SMS with the outcome; when declining, the manager can add an optional reason which is included in the SMS and stored on the record",
          "Pay Summary — monthly pay summary per staff member showing regular hours, overtime hours, and gross pay calculated from your recorded hourly rate; export as CSV for payroll processing; rate management lets you record different hourly rates per staff member with effective-from dates so historic pay calculations remain accurate if rates change",
          "Working Time Regulations (WTR) compliance — automatic 17-week rolling average hours calculation per staff member; any worker whose rolling average exceeds the 48-hour weekly limit is flagged with an amber or red alert banner; the compliance tab shows the current average for all workers alongside their opt-out agreement status — providing the systematic WTR monitoring required under the Working Time Regulations 1998",
        ],
      },
      {
    ...
    modules: [
      ...
      },
      {
        ...
        features: [
          ...
          "Automatic emissions calculation — fuel consumption from the Fuel & Energy module and fertiliser applications from the NVZ and Spray modules feed directly into the auto-calc tab; diesel, petrol, red diesel, AdBlue, LPG, and gas oil emission factors applied automatically to your logged consumption",

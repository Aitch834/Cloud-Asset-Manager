    ...
    modules: [
      {
        ...
        features: [
          ...
          "Task Assignment — assign tasks to named staff members from multiple sources: the Week Ahead planner, Equipment defect reports, Risk assessments, Medicine withdrawal records, Vet follow-up visits, Spray applications, Work Orders, and Feed bin reorders; the staff member receives an SMS notification instantly with the task title, due date, and any note you add; assignments are tracked with a pending / in progress / completed / cancelled status workflow",
          "Task Board — dedicated manager view of all open and completed task assignments across the farm; filter by staff member, status, or source module; expandable cards show the full assignment history including completion notes and timestamps; tasks carry the source module tag (Equipment, Vet Ledger, Risk, Spray etc.) so the origin of every action is always clear",
    ...
    modules: [
      {
        ...
        features: [
          ...
          "Medicine records & withdrawal tracking — log product, batch number, dose, route of administration, and withdrawal period; BDE Farm Trac calculates the end date automatically and warns if an animal with an active withdrawal is recorded as sold or slaughtered; Raise Task on any active withdrawal record to assign a named staff member to carry out the end-of-withdrawal check with the withdrawal end date pre-set as the task due date",
          ...
          "TB Test Register — log every APHA-required bovine tuberculosis test with test date, read date, Official Veterinarian name, test type (Routine / Pre-Movement / Post-Movement / SIT), number of animals tested, result (Pass / Fail / Inconclusive), APHA breakdown reference, and restriction status; printable individual test reports suitable for APHA submission or Red Tractor audit",
          "Welfare Outcome Assessments (WOA) — structured animal-based welfare scoring records for Red Tractor Beef & Lamb and Dairy standards; capture indicator scores, overall outcome (Good / Satisfactory / Action Required), corrective actions taken, and follow-up date; view-before-edit panel with printable assessment reports",
          "Sheep Dipping Records — COSHH-compliant dipping log capturing product name and MAPP number, operator certificate (PA6AW), number of animals, dip type (plunge / shower / pour-on), bath concentration, disposal method, withholding period end date, and APHA scab notification reference; linked to COSHH register for full audit trail",
    ...
    modules: [
      ...
      },
      {
        ...
        features: [
          ...
          "PPE Issue Register — log every item of personal protective equipment issued to each employee under the PPE at Work Regulations 1992 (amended 2022); captures PPE item, category (head / eye / hearing / respiratory / hand / foot / body / fall), issue date, size/specification, condition, and staff acknowledgement; printable issue record for employee countersignature; items marked Replace Soon highlighted in the register for proactive renewal",
        ],
      },
      {

...
const MODULES = [
  ...
  { id: "safety-risk-audits", name: "Safety, Risk & Audits", price: 20, note: "Combines HS&R and Inspections & Audits; includes Contractor H&S File (PLI & RAMS tracking, induction records), accident book (RIDDOR), COSHH register, PAT testing with BDE-PAT-XXXX asset codes auto-assigned per appliance, QR label printing from the dashboard, and mobile scan-to-test workflow (scan the label → log PAT result on the spot), fire extinguisher register with per-unit service history and certificate log, waste disposal, fly-tipping and encampment records; printable H&S Register covering all four areas in one document; inspection correspondence log — log emails, letters, phone calls, meetings, and site visits against each inspection record with direction (sent/received), type, date, subject, and summary notes, providing a complete date-ordered paper trail for Red Tractor and other scheme assessors" },
  { id: "environment-sustainability", name: "Environment & Sustainability", price: 16, note: "Combines Environmental Management and Carbon & Sustainability; includes dedicated SFI / ELM agreement manager page with action codes, evidence-due deadline alerts, and print-ready compliance report; agri-environment scheme correspondence log — log correspondence with Natural England, RPA, and other scheme administrators directly against each scheme record, with direction (sent/received), type (email, letter, call, meeting, or site visit), date, subject, and summary notes; Slurry & Manure Management with full store register, fill-level progress bars, Fill & Intake Events log, and species-specific storage enforcement — material type locked to store configuration on both the spreading form and fill event dialog, with server-side validation blocking mismatched materials to ensure RB209 and NVZ species-split compliance; mobile app capture for field spreading and store fill events with offline-first sync; Silage & Haylage Stock Tracking — cut records per clamp with forage type, cut number, area (ha), yield (t/ha), total yield (t), DM%, and additive reference; live running stock balance per clamp updated as usage events are recorded; mobile-first offline capture; Straw Bale Inventory — bale batches with crop, bale type, quantity, weight per bale, moisture checks (method, % reading, operator) logged across the storage period, biomass contract fields (scheme name and unique bale reference for RTFO/AD scheme traceability), and usage events with auto-decrement of running balance; Season Reports Forage & Straw tab aggregates silage cut records and straw bale totals by harvest year with CSV export; Carbon & Sustainability: DEFRA 2023 GHG Auto-Calculator — select a year and Pre-fill from Farm Records pulls fuel tank deliveries, fertiliser applications, livestock herd numbers, and grid electricity consumption and applies DEFRA 2023 emission factors to return Scope 1 and Scope 2 totals with source badges; Use these figures → Create Carbon Audit writes pre-filled tCO₂e totals into a new audit record in one click; Sustainability Reports tab tracks carbon and sustainability reports submitted to supply chain customers with certifying body lookup (Carbon Trust, BSI PAS 2060, LRQA, Bureau Veritas, SGS UK, Intertek, ADAS, SAC Consulting, Agrecalc Carbon Assurance, Farm Carbon Toolkit, and more), submission status, PO reference, invoice reference, and document attachments" },
  ...
  { id: "report-builder", name: "Report Builder", price: 20, note: "In-app custom report builder with a 4-step wizard — pick from 10 curated datasets (fields, livestock movements, medicine records, spray applications, soil tests, inspections, staff training, equipment, financial records, risk assessments); select columns; apply date-range and field-level filters with contains / equals / ≥ / ≤ operators; sort by any column; live preview table (50-row in-app, full dataset via export); optional bar, line, or pie chart with configurable X/Y fields and count / sum / average aggregation; CSV download; save named report definitions for reuse; edit and re-run any saved report with one click; no API key required — reports run securely inside the platform" },
  { id: "data-api", name: "Data API Access", price: 15, note: "Secure, read-only REST API giving per-farm API keys for connecting BDE Farm Trac data to Excel Power Query, Power BI, Google Sheets, Python, and R; 10 curated endpoints: field register, livestock movements, medicine records, spray applications, soil test results, inspection records, staff training, equipment register, financial transactions, and risk assessments; JSON and CSV output; date-range filtering (?from / ?to); 100 req/min rate limit per key; named API key management in the dashboard — generate, track last-used timestamp, and revoke instantly" },
  { id: "sheep-production", name: "Sheep Production", price: 25, note: "Flocks registered in Livestock → Herds & Animals (single herd register); tupping records, pregnancy scanning, weigh-in & DLWG performance tracking, shearing records (fleece weight, contractor, BWMB traceability), health plans, vaccination programmes (preset vaccine list includes Gudair / Ovilis Gudair for Johne's Disease / Paratuberculosis — the only UK-licensed Johne's vaccine for sheep; disease-targeted presets include Johne's Disease / Paratuberculosis, Clostridial disease, Pasteurellosis, EAE, Toxoplasmosis, OPA, Footrot, Louping ill, Caseous lymphadenitis, Orf, and Maedi Visna; booster due date tracking and printable compliance report), Red Tractor Sheep Assurance checklist, disease monitoring with APHA advisory for reportable diseases, analytics tab (litter distribution, DLWG by batch, tupping summary, shearing by year), mobile capture for all record types, year filter on all recording tabs, and document attachment on every record row" },
  { id: "goat-production", name: "Goat Production", price: 25, note: "Herds registered in Livestock → Herds & Animals (single herd register); mating records (buck breed, ear tag, owner, mating method, does exposed, expected kidding, CIDR/sponge flag), pregnancy scanning (barren/singles/doubles/triplets with auto scanning %), weigh-in & DLWG with BCS, cull & market records (deadweight, kill-out %, EUROP grade, sale value), vaccination programmes (category presets include Johne's Disease — Paratuberculosis (Gudair), CAE prevention, Clostridial diseases, Pasteurella/pneumonia, Enterotoxaemia, Foot rot, CLA, E. coli (neonatal), and Orf; Gudair is the only UK-licensed Johne's vaccine for goats and sheep — not available for cattle due to TB skin test cross-reactivity), disease monitoring (CAE, CLA, Johne's, foot rot, cryptosporidiosis, mycoplasma, FEC), Analytics tab (kid type distribution, DLWG by batch, cull summary), Enterprise Report tab (financial and production KPIs across the enterprise — gross margin, revenue per doe, cost per kg liveweight gain, kidding rate, mortality %, replacement rate, and year-on-year trend charts), mobile capture for all record types, year filter on all recording tabs, and document attachment on every record row" },
  ...
  { id: "pig-production", name: "Pig Production", price: 25, note: "Herds linked from Livestock → Herds & Animals; farrowing & weaning records, stockmanship checks with view dialog and RecordAttachments (for vet reports and photographic welfare evidence), tail biting risk assessments with view dialog and RecordAttachments, feed consumption & FCR, movements, kill records, mortality records, vet health plan integration, Red Tractor Pigs scheme readiness checklist, Pig Salmonella Monitoring — NSMP quarterly blood serology records (number of samples, APHA / approved laboratory, Category 1 / 2 / 3 result, seroprevalence %, submission reference, and corrective actions; Category 3 results trigger a compliance alert and action plan prompt), Vaccination Programme — disease-category presets (PRRS, PCV2/Circovirus, Enzootic Pneumonia/MH, Erysipelas/PPV, E. coli/Clostridial, APP, Swine Influenza, PED) with licensed UK vaccine presets per category (Ingelvac PRRS MLV, Porcilis PRRS, Ingelvac CircoFLEX, Ingelvac M.hyo. IDAL, Eryseng Parvo, and more); batch number, expiry date, age group, dose, route, withdrawal period, vet-prescribed flag, next due date, booster alert panel; mobile capture, Disease Monitoring Register — PRRS, Enzootic Pneumonia/MH, Aujeszky's Disease, APP, Swine Influenza, PRDC, and general serology; AHDB PRRS and MH Accreditation scheme support; herd status badges (Negative / Positive-Stable / Positive-Unstable / AD-Free), PRRS & MH source herd biosecurity status on Isolation Register, year filter on all recording tabs, and document attachment on every record row" },
  { id: "poultry-production", name: "Poultry Production", price: 25, note: "Flocks registered in Livestock → Herds & Animals (single herd register); placement & depletion records with hatchery traceability, Chick Purchases tab (hatchery, supplier, breed/strain, quantity, cost, delivery reference; year filter and RecordAttachments in view dialog), daily mortality log with year-by-year trend, breed / strain and hatchery / supplier mortality breakdowns, cause analysis, feed & water consumption, egg production, medicine & vaccine records, biosecurity checklist, house cleanout records (farm staff or contractor, DEFRA-approved disinfectant, dilution rate, stock consumption, cost and invoice tracking, photo evidence), Thinning Records tab (year filter and document attachment on every row), Red Tractor Poultry scheme records, Campylobacter Monitoring — FSA National Control Programme records per flock (test type, slaughter house, FSA / NCP submission reference, result, and biosecurity interventions; year filter, document attachment on rows, full RecordAttachments in view dialog, and print report), Vaccination Programme — disease-category presets (Newcastle Disease/ND, Infectious Bronchitis/IB, Marek's Disease, Gumboro/IBD, aMPV/TRT, ILT, EDS, AE/Fowl Typhoid, Salmonella, Mycoplasma/MG, Fowl Pox) with licensed UK vaccine presets per category (Nobilis ND Clone 30, Avinew, Nobilis IB Ma5, Nobilis IB 4-91, Nobilis Rismavac, Nobilis Gumboro D78, Nobilis TRT, Nobilis SalENT, AviPro Salmonella Vac E, and more); batch number, expiry date, age group (Broilers, Layers, Breeders, Pullets, Day-old chicks, Turkeys), dose, route (Drinking water, Eye drop, Spray, Injection, Wing web/stab, In ovo), withdrawal period, vet-prescribed flag, next due date, booster alert panel; mobile capture, Disease Monitoring Register — Avian Influenza surveillance, Marek's Disease monitoring, Newcastle Disease serology, MG surveillance, IB typing, ART surveillance, Salmonella serology; AI risk level classification (Low / Medium / High); flock status badges (Negative/Clear, Low Positive, Positive, Inconclusive, Pending), Marek's Disease & Salmonella NCP source flock status on Isolation Register — colour-coded badges supporting Red Tractor Poultry and BEIC biosecurity audit trail requirements" },
  ...
  { id: "resource-planner", name: "Resource Planner", price: 20, note: "Centralised resource registry for tractors, implements, sprayers, trailers, vehicles, and named staff — each with a colour dot for visual identification; drag-and-drop resource assignment from a collapsible sidebar onto Week Ahead Gantt task bars; inline assignment picker on expanded task cards; inline resource requirements — set the number of tractors, implements, vehicles, sprayers, trailers, and staff needed per task directly from the task card using stepper controls; 'Other' resource descriptions — add multiple named entries for custom or hired resources (water bowser, generator, hired plant) against any task; resource search and type-filter pills on the pool sidebar; jump-to-date calendar picker on the Gantt nav; planner CSV export (tasks, requirements, and allocations for the current week); pinch point analysis — automatic amber alert panel identifying every day where total resource demand exceeds your registered supply with plain-English shortfall summaries; materials tracking — log material requirements (product name, quantity, unit) per task with a weekly Materials Preparation Checklist panel aggregating all materials across the planning week; mobile materials visibility — field workers see required materials in the Task Inbox when they open an assigned task on the mobile app; Planning Status tab — manager view of all upcoming tasks grouped by planning state (Not started / Needs sign-off / Committed) with date range filter (1 week to all upcoming), task name search, past tasks toggle, and CSV export; task commitment with audit trail — manager name and timestamp recorded on every sign-off; Plan vs Actual (As-Built) recording — log actual date, resources used, material quantities, outcome (Completed / Partial / Abandoned), and deviation notes on any past task; Plan vs Actual comparison table showing date slip, resource delta (over/under), and outcome for every task with actuals recorded; Analytics tab — planning status chart, tasks-by-week stacked bar, resource demand by type, resource utilisation, material totals table, and Plan vs Actual variance charts (date slip distribution, resource planned-vs-actual) once actuals are recorded; automatic conflict detection flags any resource double-booked on the same day with an amber warning icon; real-time conflict resolution as assignments are added or removed; colour-coded resource chips on Gantt bars for whole-week utilisation overview at a glance; archive / restore resources without losing allocation history" },
];

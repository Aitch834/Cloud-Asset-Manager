...
const MODULES = [
  ...
  { id: "viticulture", label: "Viticulture (vine register with UK variety & rootstock selects, GI classification (PDO/PGI/Table Wine), vineyard block management with full planting lifecycle (Active / Suspended / Removed states, retire & replant workflows, complete block history), BBCH phenology records, canopy & pruning operations, harvest records with Brix/pH/TA, disease & pest scouting with intelligent notification alerts (critical SMS for Xylella, Phytophthora, Vine Weevil & high disease pressure), GI Compliance tab (PDO/PGI designation register, automated Block Compliance view, APHA Certifications with expiry alerts, Harvest Declarations with auto-populate), CSV export, 4 dedicated mobile screens with active block picker — bundled with Sprays & Inputs, COSHH, Staff & Training, Equipment Management, and Trade Contacts & Stock)" },
  ...
  { id: "resource-planner", label: "Resource Planner (resource registry — tractors, implements, sprayers, trailers, vehicles, staff; drag-and-drop Gantt assignment; inline resource requirements per task with stepper controls; 'Other' named resource descriptions; pinch point analysis — amber alerts when demand exceeds supply; materials tracking per task with weekly preparation checklist; mobile materials visibility on Task Inbox; Planning Status tab — upcoming tasks grouped by Not started / Needs sign-off / Committed with live summary counts; task commitment audit trail — manager name and date recorded on every sign-off; conflict detection with amber double-booking warnings; colour-coded resource chips on task bars)" },

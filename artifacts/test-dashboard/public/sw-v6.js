...
/** Inline no-op stub returned for any @react-refresh request. */
const REACT_REFRESH_STUB = `
// @react-refresh no-op stub (sw-v6 inline — HMR disabled in test-dashboard)
console.log('[TD sw-v6] @react-refresh no-op stub loaded (SW inline)');
...
self.addEventListener('fetch', event => {
  ...
  // ── 4. @react-refresh → return no-op stub inline ────────────────────────────
  //
  // Catches ALL URL forms:
  //   bare:      /test-dashboard/@react-refresh
  //   tokenized: /test-dashboard/@td/TOKEN/@xfs/@react-refresh
  //
  // Why inline (no fetch): the Replit proxy may have the REAL @react-refresh
  // module cached from a pre-stub session.  Any network request risks a proxy
  // HIT returning the real module.  Serving the stub directly from the SW
  // memory eliminates all proxy and network involvement. ✓
  //
  // This case is placed FIRST so it fires before Cases 0-3 for tokenized URLs
  // (which would otherwise send a network fetch that the proxy might intercept).
  if (pathname.includes('@react-refresh')) {
    console.log('[SW v6] @react-refresh intercepted →', pathname);
    event.respondWith(

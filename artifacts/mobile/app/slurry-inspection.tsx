...
export default function SlurryInspectionScreen() {
  ...
  const [freeboardOk, setFreeboardOk] = useState(true);
  const [freeboardMm, setFreeboardMm] = useState("");
  ...
  const doSave = async () => {
    ...
    if (!selectedStoreId) {
      Alert.alert("Store Required", "Please select the store being inspected.");
      return;
    }
    if (!inspectionDate) {
    ...
    if ((outcome === "Advisory" || outcome === "Fail") && actionsRequired.trim()) {
      ...
      Alert.alert("Saved", "Inspection record saved and queued for sync.", [
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.title}>Slurry Store Inspection</Text>
            <Text style={styles.subtitle}>SSAFO / CIRIA 126 compliance</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          {loadingStores ? (
            <Text style={styles.hintText}>Loading stores…</Text>
          ) : stores.length === 0 ? (
            <View style={[styles.alertBox, { borderColor: "#F59E0B", backgroundColor: "#FFFBEB" }]}>
              <Feather name="alert-triangle" size={15} color="#B45309" />
              <Text style={[styles.alertText, { color: "#92400E" }]}>No stores registered. Add stores in the dashboard first.</Text>
            </View>
          ) : (
            <View style={styles.optionRow}>
              {stores.map(s => (
                ...
                >
                  ...
                  <Text style={[styles.optionPillSub, selectedStoreId === s.id && { color: "#fff" }]}>
                    {s.storeType}
                  </Text>
                </Pressable>
              ))}
          ...
          {outcome !== "Pass" && (
            <View style={[styles.alertBox, { borderColor: outcome === "Fail" ? colors.error : "#F59E0B", backgroundColor: outcome === "Fail" ? "#FEF2F2" : "#FFFBEB" }]}>
              ...
              <Text style={[styles.alertText, { color: outcome === "Fail" ? "#991B1B" : "#92400E" }]}>
                {outcome === "Fail" ? "Fail — this store has deficiencies requiring urgent action." : "Advisory — minor issues noted; monitor and take action where required."}
              </Text>
            </View>
          )}
        </View>
        ...
        {/* Structural checks */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Structural Checks</Text>
          <View style={styles.toggleRow}>
            <Text style={styles.toggleLabel}>Freeboard adequate</Text>
            <Switch value={freeboardOk} onValueChange={v => { setFreeboardOk(v); Haptics.selectionAsync(); }} trackColor={{ true: colors.success }} />
          </View>
          {!freeboardOk && (
            <Input
              value={freeboardMm}
              onChangeText={setFreeboardMm}
              placeholder="Freeboard measurement (mm)"
...
const styles = StyleSheet.create({
  ...
  optionPillSub: { fontFamily: fonts.regular, fontSize: fontSize.xs, color: colors.textSecondary, marginTop: 2 },

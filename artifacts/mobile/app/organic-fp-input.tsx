...
import type { OrganicFpInput } from "@/lib/types";
...
export default function OrganicFpInputScreen() {
  ...
  const handleSave = async () => {
    if (!blockName.trim()) {
      Alert.alert("Required Field", "Please enter the block or field name.");
      return;
    }
    if (!productName.trim()) {
    ...
    await appendToList(STORAGE_KEYS.ORGANIC_FP_INPUTS, record);
    await refreshPendingCount();
    setSaving(false);

    Alert.alert(
      "FP Input Logged",
      "Fresh produce input saved and will sync when online.",
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Log FP Input</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        ...
        >
          <View style={styles.infoBox}>
            <Feather name="layers" size={14} color="#16a34a" />
            <Text style={styles.infoText}>
              Record any input applied to organically-managed horticulture blocks — fertilisers, crop protection, seed treatments. Syncs to the Fresh Produce Input Register.
            </Text>
          </View>

          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Block / Field</Text>

            <View style={styles.field}>
              <Text style={styles.label}>Block or Field Name *</Text>
              <Input
                placeholder="e.g. Block A — Strawberries, Field 3"
                value={blockName}
          ...
          <View style={styles.section}>
            ...
            <View style={styles.field}>
              ...
              <Input
                placeholder="e.g. Rock Phosphate, Permitted Spray"
                value={productName}
                ...
                {APPROVAL_STATUS.map((s) => (
                  <Pressable
                    ...
                    style={[
                      styles.approvalChip,
                      {
                        borderColor: approvalStatus === s.key ? s.color : colors.border,
                        backgroundColor: approvalStatus === s.key ? s.bg : colors.surface,
                      },
                    ]}
                    onPress={() => { Haptics.selectionAsync(); setApprovalStatus(s.key); }}
                  >
                    <View style={[styles.approvalDot, { backgroundColor: s.color }]} />
                    <Text style={[styles.approvalLabel, { color: approvalStatus === s.key ? s.color : colors.textSecondary }]}>
                      {s.label}
                    </Text>
                  </Pressable>
                ))}
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Quantity Applied</Text>
            <View style={styles.row}>
              <View style={[styles.field, { flex: 1 }]}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.field}>
              <Input
                placeholder="Reason for use, application method, operator name..."
                value={notes}
          ...
          <Button title={saving ? "Saving…" : "Save FP Input Record"} onPress={handleSave} disabled={saving} />

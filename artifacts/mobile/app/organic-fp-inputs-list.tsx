...
export default function OrganicFpInputsListScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const pendingItems = allPending.filter(item => item.record_type === STORAGE_KEYS.ORGANIC_FP_INPUTS);
    ...
    if (apiBase) {
      try {
        ...
        const res = await fetch(`${apiBase}/api/farms/${farmId}/organic-fp-input-log`, { headers });
        if (res.ok) {
          const data = await res.json() as ServerFpInputRecord[];
          server = data.map(r => ({
            key: `server-${r.id}`,
            productName: r.inputName,
      ...
      >
        {loading ? (
          ...
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptyTitle}>No FP inputs recorded</Text>
            <Text style={styles.emptySubtext}>
              Tap + to log a fresh produce input — fertilisers, crop protection, seed treatments.
            </Text>
          </View>
        ) : (
                      ...
                      <View style={styles.metaRow}>
                        ...
                        {r.blockName ? (
                          <View style={styles.metaItem}>
                            ...
                            <Text style={styles.metaText}>{r.blockName}</Text>
                          </View>
                        ) : null}
                      </View>

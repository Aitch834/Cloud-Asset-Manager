...
const CULL_REASONS = ["Age", "Teeth / Dentition", "Mastitis", "Poor Performance", "Lameness / Foot Rot", "Infertility", "CAE / Joint Issues", "Condition", "Market Opportunity", "Other"];
...
export default function GoatCullRecordScreen() {
  ...
  const { dismissed: bannerDismissed, dismiss: dismissBanner } = useIdentifierBannerDismiss("goat-cull", farmId, user?.id);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        notes: notes || null,
        syncEndpoint: `/api/farms/${currentFarm?.id}/goat-cull-records`,
        syncMethod: "POST",
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={[styles.header, { paddingTop: insets.top + spacing.sm }]}>
        ...
        <Text style={styles.headerTitle}>Goat Cull / Market Record</Text>
        <View style={{ width: 36 }} />
      </View>

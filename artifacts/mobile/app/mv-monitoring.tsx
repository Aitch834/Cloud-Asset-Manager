...
const TEST_TYPES = ["ELISA Blood Test", "AGID (Agar Gel Immunodiffusion)", "PCR", "Post-mortem", "Other"];
const ACCREDITATION_STATUSES = ["Accredited — MV Free", "Accredited — Monitoring", "In Progress", "Not Accredited", "Pending Results"];
const ACCREDITATION_BODIES = ["MV Accreditation Scheme (MVA)", "Scrapie Monitored Flock Scheme", "Other", "None"];
...
export default function MvMonitoringScreen() {
  ...
  const [mvAccreditationStatus, setMvAccreditationStatus] = useState("");
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        mvAccreditationStatus: mvAccreditationStatus || null,
        vetName: vetName || null,
        laboratory: laboratory || null,
        labRef: labRef || null,
        nextTestDue: nextTestDue || null,
        notes: notes || null,
        syncEndpoint: `/api/farms/${currentFarm?.id}/sheep-dairy/mv-monitoring`,
        syncMethod: "POST",
  ...
  return (
    ...
    >
      <View style={[styles.header, { paddingTop: insets.top + spacing.sm }]}>
        ...
        <Text style={styles.headerTitle}>MV Monitoring</Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      >
        ...
        <View style={styles.field}>
          ...
          <SmallRuminantPicker species="sheep" value={flockId} onChange={setFlockId} flocks={flocks} loading={flocksLoading} fromCache={flocksCached} error={flocksError} />
        </View>

        <View style={styles.row}>
          <View style={[styles.field, { flex: 1, marginRight: spacing.sm }]}>
            <Text style={styles.label}>Animals Tested *</Text>
            <Input value={animalsTestedCount} onChangeText={setAnimalsTestedCount} placeholder="e.g. 45" keyboardType="number-pad" />
          </View>
          <View style={[styles.field, { flex: 1 }]}>
        ...
        <View style={styles.chips}>
          {ACCREDITATION_STATUSES.map(s => (
            <Chip key={s} label={s} selected={mvAccreditationStatus === s} onPress={() => setMvAccreditationStatus(s === mvAccreditationStatus ? "" : s)} />
          ))}
        </View>
        ...
        <Button title={saving ? "Saving…" : "Save MV Test Record"} onPress={handleSave} disabled={saving} style={styles.saveBtn} />
      </ScrollView>
    </KeyboardAvoidingView>
  );
}

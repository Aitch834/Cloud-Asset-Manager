...
export default function PigMovementScreen() {
  ...
  const { dismissed: bannerDismissed, dismiss: dismissBanner } = useIdentifierBannerDismiss("pig-movement", currentFarm?.id);
      ...
      {missingIdentifiers && !bannerDismissed && (
        ...
        >
          ...
          <Text style={styles.identifierBannerText}>
            {!cphNumber && !sbiNumber
              ? "CPH and SBI are missing from your farm profile — pig movements cannot be submitted without them."
              : !cphNumber
              ? "CPH number is missing from your farm profile — required for pig movement submissions."
              : "SBI number is missing from your farm profile — required for pig movement submissions."}

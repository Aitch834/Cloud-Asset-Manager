...
const CATEGORIES = ["Clostridial", "Pasteurella", "Footrot / CLA", "Orf", "Enterotoxaemia", "Ectoparasites", "Combination", "Other"];
const ROUTES = ["Subcutaneous (SC)", "Intramuscular (IM)", "Intranasal", "Oral", "Pour-On"];
const AGE_CLASSES = ["Kids", "Weanlings", "Does", "Bucks", "All Stock"];
...
export default function GoatVaccinationRecordScreen() {
  ...
  const { dismissed: bannerDismissed, dismiss: dismissBanner } = useIdentifierBannerDismiss("goat-vaccination", farmId, user?.id);
  ...
  const [ageClass, setAgeClass] = useState("Does");
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        vetPrescribed,
        notes: notes || null,
        syncEndpoint: `/api/farms/${currentFarm?.id}/goat-vaccination-programmes`,
        syncMethod: "POST",
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={[styles.header, { paddingTop: insets.top + spacing.sm }]}>
        ...
        <Text style={styles.headerTitle}>Goat Vaccination Record</Text>
        <View style={{ width: 36 }} />
      </View>
      <IdentifierBanner
        justSaved={justSaved}
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.toggleRow}>
          ...
          <Switch value={vetPrescribed} onValueChange={setVetPrescribed} trackColor={{ false: colors.borderLight, true: "#0891b2" }} thumbColor="#fff" />
        </View>

...
export default function OrganicPoultryFeedScreen() {
  ...
  const needsCertifierRef = organicApprovalStatus === "Approved for Organic Use" || organicApprovalStatus === "Conventional — Derogation";

  const handleSave = async () => {
    if (!deliveryDate.trim()) { Alert.alert("Date required", "Please enter the delivery date."); return; }
    if (!productName.trim()) { Alert.alert("Product required", "Please enter the product name."); return; }
    if (isNotPermitted) { Alert.alert("Not Permitted", "This product is not permitted under UK Organic Regulations. Do not use it on an organic holding."); return; }
    setSaving(true);
    ...
    try {
      const payload = {
        ...
        supplierLotNumber: supplierLotNumber || null,
        invoiceReference: invoiceReference || null,
        flock: flock || null,
        notes: notes || null,
        syncEndpoint: `/api/farms/${currentFarm?.id}/organic-poultry/feed-records`,
        syncMethod: "POST",
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={[styles.header, { paddingTop: insets.top + spacing.sm }]}>
        ...
        <Text style={styles.headerTitle}>Organic Poultry Feed Record</Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoText}>
            UK Organic Regulations require 100% organically produced feed for poultry. Synthetic amino acids, GMO ingredients and growth promoters are prohibited. Keep all delivery notes and certificates of conformity.
          </Text>
        </View>
        ...
        <View style={styles.row}>
          <View style={[styles.field, { flex: 1, marginRight: spacing.sm }]}>
            ...
            <Input value={deliveryDate} onChangeText={setDeliveryDate} placeholder="YYYY-MM-DD" />
          </View>
          <View style={[styles.field, { flex: 1 }]}>
            <Text style={styles.label}>Flock / House Ref</Text>
            <Input value={flock} onChangeText={setFlock} placeholder="e.g. Layer House A" />
          </View>
        </View>
        <View style={styles.field}>
          <Text style={styles.label}>Product Name *</Text>
          <Input value={productName} onChangeText={setProductName} placeholder="e.g. Organic Layer Mash 16%" />
        </View>

        <Text style={styles.sectionTitle}>Product Type</Text>
        <View style={styles.chips}>
          {PRODUCT_TYPES.map((t) => (
            <Chip key={t} label={t} selected={productType === t} onPress={() => { Haptics.selectionAsync(); setProductType(t); }} />
          ))}
        </View>
        ...
        <View style={styles.chips}>
          {APPROVAL_STATUSES.map((s) => (
            <Chip key={s} label={s} selected={organicApprovalStatus === s}
              ...
              chipColor={s === "Certified Organic" ? "#16a34a" : s === "Not Permitted" ? "#dc2626" : s === "Conventional — Derogation" ? "#d97706" : CHIP_COLOR} />
          ))}
        </View>
        {isNotPermitted && (
          <View style={styles.alertBox}>
            <Feather name="alert-triangle" size={14} color="#dc2626" />
            <Text style={styles.alertText}>NOT PERMITTED under UK Organic Regulations. This product must not be fed to organic poultry.</Text>
          </View>
        )}
        {needsCertifierRef && (
          <View style={styles.field}>
            <Text style={styles.label}>Certifier Approval / Derogation Reference *</Text>
            <Input value={certifierApprovalReference} onChangeText={setCertifierApprovalReference} placeholder="e.g. SA/2025/0042 or DERG-2025-007" />
          </View>
        )}

        <Text style={styles.sectionTitle}>Supplier & Quantity</Text>
        <View style={styles.row}>
          <View style={[styles.field, { flex: 1, marginRight: spacing.sm }]}>
          ...
          <View style={[styles.field, { flex: 1 }]}>
            ...
            <Input value={supplierName} onChangeText={setSupplierName} placeholder="Supplier" />
          </View>
        </View>
        <View style={styles.row}>
          <View style={[styles.field, { flex: 1, marginRight: spacing.sm }]}>
            <Text style={styles.label}>Lot / Batch Number</Text>
            <Input value={supplierLotNumber} onChangeText={setSupplierLotNumber} placeholder="Batch ref" />
          </View>
          <View style={[styles.field, { flex: 1 }]}>
            <Text style={styles.label}>Invoice Reference</Text>
            <Input value={invoiceReference} onChangeText={setInvoiceReference} placeholder="Invoice no." />
          </View>
        </View>

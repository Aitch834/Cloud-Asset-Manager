...
export default function OrganicGoatFlockConversionScreen() {
  ...
  const [numberOfDoes, setNumberOfDoes] = useState("");
  ...
  const handleSave = async () => {
    if (!flockName.trim()) {
      Alert.alert("Herd name required", "Please enter the herd name.");
      return;
    }
    ...
    try {
      const payload = {
        ...
        type: "organic-goat-flock-conversion",
        flockGroup: flockGroup || null,
        flockName,
        breed: breed || null,
        numberOfDoes: numberOfDoes ? Number(numberOfDoes) : null,
        conversionStartDate,
        ...
        parallelProduction,
        notes: notes || null,
        syncEndpoint: `/api/farms/${currentFarm?.id}/organic-goat-dairy/flock-conversion`,
        syncMethod: "POST",
      ...
      Alert.alert(
        ...
        `Herd "${flockName}" conversion record saved. Will sync when connected.`,
  ...
  return (
    ...
    >
      <View style={[styles.header, { paddingTop: insets.top + spacing.sm }]}>
        ...
        <Text style={styles.headerTitle}>Goat Herd Conversion</Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      >
        <View style={styles.hint}>
          ...
          <Text style={styles.hintText}>Organic conversion for goat dairy milk requires a minimum 6-month conversion period before milk can be sold as organic</Text>
        </View>

        <Text style={styles.sectionTitle}>Herd Details</Text>

        <View style={styles.field}>
          <Text style={styles.label}>Herd Name *</Text>
          <Input value={flockName} onChangeText={setFlockName} placeholder="e.g. Home herd — British Saanen does" />
        </View>

        <View style={styles.field}>
          <Text style={styles.label}>Herd Group / Lot</Text>
          <SmallRuminantPicker species="goat" label="Herd Group / Lot" value={flockGroup} onChange={setFlockGroup} flocks={flocks} loading={flocksLoading} fromCache={flocksCached} error={flocksError} />
        </View>
        ...
        <View style={styles.row}>
          ...
          <View style={[styles.field, { flex: 1 }]}>
            <Text style={styles.label}>Number of Does</Text>
            <Input value={numberOfDoes} onChangeText={setNumberOfDoes} placeholder="e.g. 80" keyboardType="number-pad" />
          </View>
        </View>
        ...
        <View style={styles.field}>
          ...
          <Input value={certificationRef} onChangeText={setCertificationRef} placeholder="e.g. OFG/G/56789" autoCapitalize="characters" />
        </View>

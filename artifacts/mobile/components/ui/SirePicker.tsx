...
import type { ApiSire } from "@/lib/hooks/useApiSires";

interface SirePickerProps {
  value: string;
  onChange: (name: string) => void;
  onChangeSire?: (sire: ApiSire) => void;
  sires: ApiSire[];
  loading: boolean;
  fromCache: boolean;
  error: string | null;
...
export function SirePicker({
  ...
  const filtered = search.trim()
    ? sires.filter((s) => s.name.toLowerCase().includes(search.toLowerCase()) || (s.breed ?? "").toLowerCase().includes(search.toLowerCase()))
    : sires;

  const select = (sire: ApiSire) => {
  ...
  const clear = () => {
    Haptics.selectionAsync();
    onChange("");
    onChangeSire?.(undefined as unknown as ApiSire);
  };
    ...
    <>
      <View style={styles.wrapper}>
        ...
        <View style={styles.row}>
          ...
          >
            <Feather name="target" size={15} color={value ? colors.primary : colors.textSecondary} />
            <Text style={[styles.selectorText, !value && styles.selectorPlaceholder]} numberOfLines={1}>
              {value || "Select from sire register…"}
            </Text>
            {loading
        ...
        {fromCache && (
          <Text style={styles.cacheNote}>Offline — using cached list</Text>
        )}
      </View>
      ...
      <Modal visible={modalOpen} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setModalOpen(false)}>
        <View style={[styles.modal, { paddingTop: insets.top + spacing.sm }]}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Select Sire / Ram</Text>
          ...
          {loading && sires.length === 0 ? (
            <View style={styles.centre}>
              ...
              <Text style={styles.centreText}>Loading sire register…</Text>
            </View>
          ) : sires.length === 0 ? (
            <View style={styles.centre}>
              <Feather name="target" size={40} color={colors.border} />
              <Text style={styles.centreTitle}>No sires registered</Text>
              <Text style={styles.centreText}>
                {error
                  ? "Could not connect. Open the app online to sync your sire register."
                  : "Add your bulls and rams in the web dashboard under Livestock → Sires & Rams."}
              </Text>
            </View>
          ) : filtered.length === 0 ? (
            <View style={styles.centre}>
              <Text style={styles.centreText}>No sires match "{search}"</Text>
            </View>
          ) : (
            <FlatList
              data={filtered}
              ...
              renderItem={({ item }) => (
                <Pressable
                  onPress={() => select(item)}
                  style={[styles.itemRow, item.name === value && styles.itemRowSelected]}
                >
                  <View style={[styles.itemIcon, item.name === value && styles.itemIconSelected]}>
                    <Feather name="target" size={14} color={item.name === value ? colors.textInverse : colors.primary} />
                  </View>
                  <View style={styles.itemInfo}>
                    ...
                    <Text style={styles.itemMeta}>
                      ...
                      {item.ownershipType === "hired_in" ? " · Hired In" : item.ownershipType === "loaned" ? " · Loaned" : ""}
                    </Text>
                    {item.tagNumber && (
                      <Text style={styles.itemMeta}>Tag: {item.tagNumber}</Text>
                    )}
                  </View>
                  {item.name === value && (
...
const styles = StyleSheet.create({
  ...
  itemIcon: { width: 32, height: 32, borderRadius: radius.sm, backgroundColor: "#dcfce7", alignItems: "center", justifyContent: "center" },

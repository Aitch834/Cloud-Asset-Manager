...
import type { ApiPigFlock } from "@/lib/hooks/useApiPigFlocks";

const PINK = "#db2777";
const PINK_BG = "#fce7f3";

interface PigPenPickerProps {
  value: string;
  onChange: (flockName: string) => void;
  onChangeFlock?: (flock: ApiPigFlock) => void;
  flocks: ApiPigFlock[];
  loading: boolean;
  fromCache: boolean;
  error: string | null;
...
export function PigPenPicker({
  ...
  const filtered = search.trim()
    ? flocks.filter(
        (f) =>
          f.flockName.toLowerCase().includes(search.toLowerCase()) ||
          f.productionType.toLowerCase().includes(search.toLowerCase())
      )
    : flocks;

  const select = (flock: ApiPigFlock) => {
    Haptics.selectionAsync();
    onChange(flock.flockName);
    onChangeFlock?.(flock);
    setModalOpen(false);
    setSearch("");
  };
  ...
  return (
    <>
      <View style={styles.wrapper}>
        ...
        >
          <Feather name="grid" size={15} color={value ? PINK : colors.textSecondary} />
          <Text style={[styles.selectorText, !value && styles.selectorPlaceholder]} numberOfLines={1}>
            {value || "Select pig group / pen…"}
          </Text>
          {loading
      ...
      <Modal visible={modalOpen} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setModalOpen(false)}>
        <View style={[styles.modal, { paddingTop: insets.top + spacing.sm }]}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Select Group / Pen</Text>
          ...
          {loading && flocks.length === 0 ? (
            <View style={styles.centre}>
              ...
              <Text style={styles.centreText}>Loading groups…</Text>
            </View>
          ) : flocks.length === 0 ? (
            <View style={styles.centre}>
              <Feather name="grid" size={40} color={colors.border} />
              <Text style={styles.centreTitle}>No pig groups registered</Text>
              <Text style={styles.centreText}>
                {error
                  ? "Could not connect. Open the app online to sync your pig groups."
                  : "Add your pig groups in the Pig Production section of the web dashboard."}
              </Text>
            </View>
          ) : filtered.length === 0 ? (
            <View style={styles.centre}>
              <Text style={styles.centreText}>No groups match "{search}"</Text>
            </View>
          ) : (
            <FlatList
              data={filtered}
              ...
              renderItem={({ item }) => (
                <Pressable
                  onPress={() => select(item)}
                  style={[styles.itemRow, item.flockName === value && styles.itemRowSelected]}
                >
                  <View style={[styles.itemIcon, item.flockName === value && styles.itemIconSelected]}>
                    <Feather name="grid" size={14} color={item.flockName === value ? colors.textInverse : PINK} />
                  </View>
                  <View style={styles.itemInfo}>
                    <Text style={[styles.itemName, item.flockName === value && styles.itemNameSelected]}>
                      {item.flockName}
                    </Text>
                    <Text style={styles.itemMeta}>
                      {[item.productionType, item.breed].filter(Boolean).join(" · ")}
                    </Text>
                    {item.location && (
                      <Text style={styles.itemMeta}>Location: {item.location}</Text>
                    )}
                    <Text style={styles.itemMeta}>
                      Count: {item.currentCount?.toLocaleString() ?? "—"} pigs
                      {item.herdNumber ? ` · Herd: ${item.herdNumber}` : ""}
                      {item.cphNumber ? ` · CPH: ${item.cphNumber}` : ""}
                    </Text>
                  </View>
                  {item.flockName === value && (
                    <Feather name="check" size={16} color={PINK} />
                  )}
                </Pressable>
              )}
            />
...
const styles = StyleSheet.create({
  ...
  itemRowSelected: { borderColor: PINK, backgroundColor: "#fff1f8" },
  itemIcon: { width: 32, height: 32, borderRadius: radius.sm, backgroundColor: PINK_BG, alignItems: "center", justifyContent: "center" },
  itemIconSelected: { backgroundColor: PINK },
  itemInfo: { flex: 1 },
  itemName: { fontFamily: fonts.semiBold, fontSize: fontSize.md, color: colors.text },
  itemNameSelected: { color: PINK },

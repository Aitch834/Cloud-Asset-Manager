...
interface HerdPickerProps {
  value: string;
  onChange: (name: string) => void;
  onChangeHerd?: (herd: ApiHerd) => void;
  herds: ApiHerd[];
  loading: boolean;
  fromCache: boolean;
  error: string | null;
...
export function HerdPicker({
  ...
  const filtered = search.trim()
    ? herds.filter((h) => h.name.toLowerCase().includes(search.toLowerCase()))
    : herds;

  const select = (herd: ApiHerd) => {
  ...
  return (
    <>
      <View style={styles.wrapper}>
        ...
        >
          <Feather name="users" size={15} color={value ? colors.primary : colors.textSecondary} />
          <Text style={[styles.selectorText, !value && styles.selectorPlaceholder]} numberOfLines={1}>
            {value || "Select herd or flock…"}
          </Text>
          {loading
        ...
        <View style={[styles.modal, { paddingTop: insets.top + spacing.sm }]}>
          ...
          {loading && herds.length === 0 ? (
            <View style={styles.centre}>
              ...
              <Text style={styles.centreText}>Loading herds…</Text>
            </View>
          ) : herds.length === 0 ? (
            <View style={styles.centre}>
              <Feather name="users" size={40} color={colors.border} />
              <Text style={styles.centreTitle}>No herds registered</Text>
              <Text style={styles.centreText}>
                {error
                  ? "Could not connect to load herds. Open the app online to sync your list."
                  : "Add your herds and flocks in the web dashboard under Livestock, then they will appear here."}
              </Text>
            </View>
          ) : filtered.length === 0 ? (
            <View style={styles.centre}>
              <Text style={styles.centreText}>No herds match "{search}"</Text>
            </View>
          ) : (
            <FlatList
              data={filtered}
              ...
              renderItem={({ item }) => (
                ...
                >
                  <View style={[styles.itemIcon, item.name === value && styles.itemIconSelected]}>
                    <Feather name="users" size={14} color={item.name === value ? colors.textInverse : colors.primary} />
                  </View>
                  <View style={styles.itemInfo}>
                    ...
                    {item.herdNumber && (
                      <Text style={styles.itemMeta}>Herd No: {item.herdNumber}</Text>
                    )}
                  </View>
                  {item.name === value && (

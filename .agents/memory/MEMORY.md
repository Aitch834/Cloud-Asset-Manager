...
- [Mobile preview routing](mobile-preview-routing.md) — use router=expo-domain so the mobile workflow serves directly; gateway-only routing returns 502 when the website is stopped
...
- [Dashboard build memory baseline](dashboard-build-oom.md) — serial production builds include dashboard; tracked dist remains the post-merge fallback
...
- [Expo Jest ESM transforms](expo-jest-esm-transforms.md) — custom Jest project transforms must cover `.js` as well as TS, or Expo SDK ESM dependencies fail before tests run

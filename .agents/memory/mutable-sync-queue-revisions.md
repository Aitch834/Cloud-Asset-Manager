...
Payload replacement must merge queue-owned reconciliation metadata (such as the resolved server ID or discard tombstone) from the current row into the user's edited payload. Replacing the whole object can erase an ID written moments earlier and turn the next reconciliation PUT into a duplicate POST.
...
**How to apply:** Any sync type that updates mutable queued data must capture a revision or payload hash before upload, preserve queue-owned metadata while replacing user fields, alter queue status only if it is unchanged, and immediately schedule another pass when the attempted revision was superseded. For create-then-edit flows, retain the returned server ID and make the reconciliation request update that row. On non-transactional stores, write durable recovery state before advancing queue state. Persist retry deadlines and exclude not-yet-due rows from immediate passes. Add interleaving and interruption regression tests that assert final server state, not only queue state.
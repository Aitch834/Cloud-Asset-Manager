...
---
Each winery tab that shows an SO₂ verdict on multiple surfaces (badge, CSV, dialog) has a single verdict helper: cellarSo2Verdict (CellarOpsTab), so2TestVerdict (So2TestingTab). bottlingSo2Verdict and so2Ceiling(colour, isOrganic) now live in winery/shared.tsx and are consumed by the Bottling tab, Batch Trail dialog/CSV and print.tsx — new SO₂ surfaces should import these rather than the raw MAX_SO2 maps.

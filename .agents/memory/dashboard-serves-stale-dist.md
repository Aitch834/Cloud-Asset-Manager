...
`cd artifacts/dashboard && PORT=3000 BASE_PATH=/dashboard/ pnpm run build` then restart `artifacts/dashboard: web`. Verify by grepping a new string in `dist/public/assets/`. Browser testers that reuse an open page keep the old JavaScript in memory after a rebuild; use a fresh browser context or hard reload before judging the rebuilt behavior.

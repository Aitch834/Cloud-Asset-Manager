...
export const seedPurchaseOrdersTable = pgTable("seed_purchase_orders", {
  id: serial("id").primaryKey(),
  farmId: integer("farm_id").notNull().references(() => farmsTable.id, { onDelete: "cascade" }),
  ...
  varietyId: integer("variety_id").references(() => cropVarietiesTable.id),
...
export const seedBatchesTable = pgTable("seed_batches", {
  ...
  supplierId: integer("supplier_id").references(() => suppliersTable.id),
  poId: integer("po_id").references(() => seedPurchaseOrdersTable.id, { onDelete: "set null" }),
